...
from llm.proxy._types import (
    ...
    LLM_EndUserTable,
    LLM_JWTAuth,
    LLM_OrganizationMembershipTable,
...
all_routes = LLMRoutes.openai_routes.value + LLMRoutes.management_routes.value
...
def _is_ui_route(
    ...
    allowed_routes = LLMRoutes.ui_routes.value
...
def _is_user_proxy_admin(user_obj: Optional[LLM_UserTable]):
    if user_obj is None:
        return False

    if (
        user_obj.user_role is not None
        and user_obj.user_role == LLMUserRoles.PROXY_ADMIN.value
    ):
        return True
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|LLMRoutes] - the list of allowed routes for the user.
    """

    for allowed_route in allowed_routes:
        if (
            allowed_route in LLMRoutes.__members__
            and user_route in LLMRoutes[allowed_route].value
        ):
            return True
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = LLMRoutes[route_name].value
...
async def get_end_user_object(
    ...
    def check_in_budget(end_user_obj: LLM_EndUserTable):
    ...
    if cached_user_obj is not None:
        if isinstance(cached_user_obj, dict):
            return_obj = LLM_EndUserTable(**cached_user_obj)
            check_in_budget(end_user_obj=return_obj)
            return return_obj
        elif isinstance(cached_user_obj, LLM_EndUserTable):
    ...
    try:
        ...
        _response = LLM_EndUserTable(**response.dict())
...
async def get_user_object(
    ...
    - if valid, return LLM_UserTable object with defined limits
    ...
    if cached_user_obj is not None:
        if isinstance(cached_user_obj, dict):
            return LLM_UserTable(**cached_user_obj)
        elif isinstance(cached_user_obj, LLM_UserTable):
    ...
    try:
        ...
        if (
            ...
            # dump each organization membership to type LLM_OrganizationMembershipTable
            _dumped_memberships = [
                LLM_OrganizationMembershipTable(**membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = LLM_UserTable(**dict(response))
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response = LLM_TeamTableCachedObj(**response.dict())
...
async def _get_team_object_from_cache(
...
) -> Optional[LLM_TeamTableCachedObj]:
    cached_team_obj: Optional[LLM_TeamTableCachedObj] = None
    ...
    if cached_team_obj is not None:
        if isinstance(cached_team_obj, dict):
            return LLM_TeamTableCachedObj(**cached_team_obj)
        elif isinstance(cached_team_obj, LLM_TeamTableCachedObj):
...
async def get_team_object(
    ...
    - if valid, return LLM_TeamTable object with defined limits
    - if not, then raise an error
...
async def _handle_failed_db_connection_for_get_key_object(
    ...
    """
    Handles httpx.ConnectError when reading a Virtual Key from LLM DB
...
async def get_org_object(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[LLM_OrganizationTable]:
    """
    - Check if org id in proxy Org Table
    - if valid, return LLM_OrganizationTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    if cached_org_obj is not None:
        if isinstance(cached_org_obj, dict):
            return LLM_OrganizationTable(**cached_org_obj)
        elif isinstance(cached_org_obj, LLM_OrganizationTable):
