...
# Budget Routing
LLM Supports setting the following budgets:
...
router_settings:
  ...
  # OPTIONAL: Set Redis Host, Port, and Password if using multiple instance of LLM
...
LLM will emit the following metric on Prometheus to track the remaining budget for each provider
...
If you are using a multi-instance setup, you will need to set the Redis host, port, and password in the `proxy_config.yaml` file. Redis is used to sync the spend across LLM instances.
...
router_settings:
  ...
  # 👇 Add this: Set Redis Host, Port, and Password if using multiple instance of LLM
  redis_host: os.environ/REDIS_HOST
  redis_port: os.environ/REDIS_PORT
  redis_password: os.environ/REDIS_PASSWORD
