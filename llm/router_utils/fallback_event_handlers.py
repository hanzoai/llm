...
from llm.types.router import HanzoParamsTypedDict
...
async def log_success_fallback_event(
    ...
    for _callback in llm.callbacks:
        if isinstance(_callback, CustomLogger) or (
            _callback in llm._known_custom_logger_compatible_callbacks
        ):
            try:
                _callback_custom_logger: Optional[CustomLogger] = None
                if _callback in llm._known_custom_logger_compatible_callbacks:
...
def _check_non_standard_fallback_format(fallbacks: Optional[List[Any]]) -> bool:
    ...
    - List[Dict[<HanzoParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for key in HanzoParamsTypedDict.__annotations__.keys():
