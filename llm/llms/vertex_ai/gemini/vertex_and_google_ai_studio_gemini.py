...
from llm.utils import CustomStreamWrapper, ModelResponse
...
class VertexGeminiConfig(VertexAIBaseConfig, BaseConfig):
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            ...
            raise llm.utils.UnsupportedParamsError(
                message="VertexAI doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `llm.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
        ...
        for tool in value:
            ...
            if tool.get("googleSearch", None) is not None:
                ...
                verbose_logger.warning(
                    "Invalid tool={}. Use `llm.set_verbose` or `llm --detailed_debug` to see raw request."
                )
    ...
    def map_openai_params(
        ...
        if llm.vertex_ai_safety_settings is not None:
            optional_params["safety_settings"] = llm.vertex_ai_safety_settings
        return optional_params
...
class ModelResponseIterator:
    ...
    def handle_accumulated_json_chunk(self, chunk: str) -> GenericStreamingChunk:
        chunk = llm.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or ""
