...
from llm.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
from llm.proxy.management_endpoints.common_utils import (
    _is_user_team_admin,
...
from llm.proxy.management_helpers.utils import management_endpoint_wrapper
from llm.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from llm.types.router import Deployment
...
def _is_team_key(data: Union[GenerateKeyRequest, LLM_VerificationToken]):
    return data.team_id is not None


def _get_user_in_team(
    team_table: LLM_TeamTableCachedObj, user_id: Optional[str]
) -> Optional[Member]:
    if user_id is None:
        return None
...
def _is_allowed_to_make_key_request(
    ...
    Relevant issue: https://github.com/hanzoai/llm/issues/7336
    """
    ## BASE CASE - PROXY ADMIN
    if (
        user_api_key_dict.user_role is not None
        and user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN.value
    ):
        return True
    ...
    if team_id is not None:
        if (
            ...
            return True  # handle https://github.com/hanzoai/llm/issues/7482
        assert (
...
def _team_key_generation_team_member_check(
    ...
    team_table: LLM_TeamTableCachedObj,
...
def _team_key_generation_check(
    ...
    if user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN.value:
        return True
    if (
        llm.key_generation_settings is not None
        and "team_key_generation" in llm.key_generation_settings
    ):
        _team_key_generation = llm.key_generation_settings["team_key_generation"]
    else:
...
def _personal_key_membership_check(
    ...
    if user_api_key_dict.user_role not in personal_key_generation["allowed_user_roles"]:
        raise HTTPException(
            status_code=400,
            detail=f"Personal key creation has been restricted by admin. Allowed roles={llm.key_generation_settings['personal_key_generation']['allowed_user_roles']}. Your role={user_api_key_dict.user_role}",  # type: ignore
        )
...
def _personal_key_generation_check(
    ...
    if (
        llm.key_generation_settings is None
        or llm.key_generation_settings.get("personal_key_generation") is None
    ):
        return True

    _personal_key_generation = llm.key_generation_settings["personal_key_generation"]  # type: ignore
...
def key_generation_check(
    team_table: Optional[LLM_TeamTableCachedObj],
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
) -> bool:
    """
    ...
    if is_team_key:
        if team_table is None and llm.key_generation_settings is not None:
            raise HTTPException(
                status_code=400,
...
async def generate_key_fn(  # noqa: PLR0915
    ...
    - metadata: Optional[dict] - Metadata for key, store information for key. Example metadata = {"team": "core-infra", "app": "app2", "email": "ishaan@hanzo.ai" }
    ...
    try:
        ...
        team_table: Optional[LLM_TeamTableCachedObj] = None
        ...
        if llm.default_key_generate_params is not None:
            for elem in data:
                key, value = elem
                if value is None and key in [
                    ...
                    setattr(
                        data, key, llm.default_key_generate_params.get(key, None)
                    )
                elif key == "models" and value == []:
                    setattr(data, key, llm.default_key_generate_params.get(key, []))
                elif key == "metadata" and value == {}:
                    setattr(data, key, llm.default_key_generate_params.get(key, {}))

        # check if user set default key/generate params on config.yaml
        if llm.upperbound_key_generate_params is not None:
            for elem in data:
                key, value = elem
                upperbound_value = getattr(
                    llm.upperbound_key_generate_params, key, None
                )
        ...
        for field in LLM_ManagementEndpoint_MetadataFields_Premium:
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.generate_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def prepare_metadata_fields(
    ...
    """
    Check LLM_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    try:
        for k, v in data_json.items():
            if k in LLM_ManagementEndpoint_MetadataFields:
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.prepare_metadata_fields(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def update_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.update_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def delete_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.delete_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
-H "Authorization: Bearer sk-02Wr4IAlN3NvPXvL5JVvDA"
    ...
    try:
        ...
        key_info = await prisma_client.db.llm_verificationtoken.find_unique(
        ...
        if llm_router._is_model_access_group_for_wildcard_route(
            ...
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Setting a model access group on a wildcard model is only available for LLM Enterprise users.{}".format(
...
async def generate_key_helper_fn(  # noqa: PLR0915
    ...
    budget_id: Optional[float] = None,  # budget id <-> LLM_BudgetTable
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if (
                ...
                raise ValueError(
                    "get_spend_routes permission is only available for LLM Enterprise users"
                )
        ...
        if prisma_client is not None:
            ...
            if user_id == llm_proxy_budget_name or (
                table_name is not None and table_name == "user"
            ):
                # do not create a key for llm_proxy_budget_name or if table name is set to just 'user'
                # we only need to ensure this exists in the user table
                # the LLM_VerificationToken table will increase in size if we don't do this check
                return user_data
            ...
            key_data["llm_budget_table"] = getattr(
                create_key_response, "llm_budget_table", None
            )
    except Exception as e:
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.generate_key_helper_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def _team_key_deletion_check(
    user_api_key_dict: UserAPIKeyAuth,
    key_info: LLM_VerificationToken,
    ...
    if is_team_key and key_info.team_id is not None:
        ...
        if (
            ...
            _team_key_generation = llm.key_generation_settings[
                "team_key_generation"
            ]
        else:
            _team_key_generation = TeamUIKeyGenerationConfig(
...
async def delete_verification_tokens(
    ...
    user_api_key_cache: DualCache,
    user_api_key_dict: UserAPIKeyAuth,
) -> Tuple[Optional[Dict], List[LLM_VerificationToken]]:
    """
    ...
    try:
        if prisma_client:
            ...
            _keys_being_deleted: List[LLM_VerificationToken] = (
                await prisma_client.db.llm_verificationtoken.find_many(
            ...
            # else
                ...
                for key in _keys_being_deleted:
                    ...
                    async def _delete_key(key: LLM_VerificationToken):
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.delete_verification_tokens(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_key_aliases(
    ...
    _keys_being_deleted = await prisma_client.db.llm_verificationtoken.find_many(
...
async def _rotate_master_key(
    ...
    try:
        models: Optional[List] = (
            await prisma_client.db.llm_proxymodeltable.find_many()
        )
    except Exception:
        models = None
    ...
    if models:
        ...
        await prisma_client.db.llm_proxymodeltable.delete_many()
        verbose_proxy_logger.info("Creating %s models", len(new_models))
        await prisma_client.db.llm_proxymodeltable.create_many(
            data=new_models,
        )
...
async def regenerate_key_fn(
    ...
    try:
        ...
        _key_in_db = await prisma_client.db.llm_verificationtoken.find_unique(
        ...
        updated_token = await prisma_client.db.llm_verificationtoken.update(
            where={"token": hashed_api_key},
...
async def validate_key_list_check(
    ...
    complete_user_info = LLM_UserTable(**complete_user_info_db_obj.model_dump())
...
async def get_admin_team_ids(
    complete_user_info: Optional[LLM_UserTable],
    user_api_key_dict: UserAPIKeyAuth,
    prisma_client: PrismaClient,
) -> List[str]:
    """
    ...
    teams: Optional[List[BaseModel]] = (
        await prisma_client.db.llm_teamtable.find_many(
    ...
    teams_pydantic_obj = [LLM_TeamTable(**team.model_dump()) for team in teams]
...
async def list_keys(
    ...
    try:
        ...
        if user_id is None and user_api_key_dict.user_role not in [
            ...
            LLMUserRoles.PROXY_ADMIN_VIEW_ONLY.value,
        ]:
            user_id = user_api_key_dict.user_id
...
async def _list_key_helper(
    ...
    keys = await prisma_client.db.llm_verificationtoken.find_many(
    ...
    total_count = await prisma_client.db.llm_verificationtoken.count(
        where=where  # type: ignore
    )
...
async def block_key(
    ...
    if llm.store_audit_logs is True:
        # make an audit log for key update
        record = await prisma_client.db.llm_verificationtoken.find_unique(
    ...
    record = await prisma_client.db.llm_verificationtoken.update(
...
def _can_user_query_key_info(
    ...
    """
    if (
        user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN.value
        or user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN_VIEW_ONLY.value
    ):
        return True
...
async def test_key_logging(
    ...
    from llm.proxy.proxy_server import general_settings, proxy_config
    ...
    try:
        data = {
            "model": "openai/llm-key-health-test",
            "messages": [
                {
                    "role": "user",
                    "content": "Hello, this is a test from llm /key/health. No LLM API call was made for this",
                }
            ],
...
async def _enforce_unique_key_alias(
    ...
    if key_alias is not None and prisma_client is not None:
        ...
        existing_key = await prisma_client.db.llm_verificationtoken.find_first(
            where=where_clause
        )
