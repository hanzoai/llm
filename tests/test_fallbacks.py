...
async def test_chat_completion_with_retries():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert headers["x-llm-attempted-retries"] == "1"
        assert headers["x-llm-max-retries"] == "50"
...
async def test_chat_completion_with_fallbacks():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"headers: {headers}")
        assert headers["x-llm-attempted-fallbacks"] == "1"
