...
async def new_organization(
    ...
    organization_row = Hanzo_OrganizationTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=Hanzo_OrganizationTableWithMembers,
)
async def update_organization(
    data: Hanzo_OrganizationTableUpdate,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
):
    """
...
@router.delete(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[Hanzo_OrganizationTableWithMembers],
)
async def delete_organization(
    ...
    for organization_id in data.organization_ids:
        ...
        await prisma_client.db.llm_teamtable.delete_many(
...
async def info_organization(organization_id: str):
    ...
    response: Optional[Hanzo_OrganizationTableWithMembers] = (
    ...
    response_pydantic_obj = Hanzo_OrganizationTableWithMembers(
        **response.model_dump()
    )
...
async def organization_member_add(
    ...
    2. Creates a new Internal User if the user_id or user_email is not found in Hanzo_UserTable
...
async def find_member_if_email(
    ...
    """
    Find a member if the user_email is in Hanzo_UserTable
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Unique user not found for user_email={user_email}. Potential duplicate OR non-existent user_email in Hanzo_UserTable. Use 'user_id' instead."
            },
        )
    existing_user_email_row_pydantic = Hanzo_UserTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=Hanzo_OrganizationMembershipTable,
)
@management_endpoint_wrapper
async def organization_member_update(
    ...
    try:
        ...
        final_organization_membership_pydantic = Hanzo_OrganizationMembershipTable(
...
) -> Tuple[Hanzo_UserTable, Hanzo_OrganizationMembershipTable]:
    """
    Add a member to an organization

    - Checks if member.user_id or member.user_email is in Hanzo_UserTable
    - If not found, create a new user in Hanzo_UserTable
    - Add user to organization in Hanzo_OrganizationMembership
    """

    try:
        user_object: Optional[Hanzo_UserTable] = None
        existing_user_id_row = None
        existing_user_email_row = None
        ## Check if user exists in Hanzo_UserTable - user exists - either the user_id or user_email is in Hanzo_UserTable
        ...
        if existing_user_id_row is None and member.user_email is not None:
            try:
                ...
                raise ValueError(
                    f"Potential NON-Existent or Duplicate user email in DB: Error finding a unique instance of user_email={member.user_email} in Hanzo_UserTable.: {e}"
                )
        ...
        if existing_user_id_row is None and existing_user_email_row is None:
            ...
            if _returned_user is not None:
                user_object = Hanzo_UserTable(**_returned_user.model_dump())
            ...
            user_object = Hanzo_UserTable(**existing_user_email_row.model_dump())
        elif existing_user_id_row is not None:
            user_object = Hanzo_UserTable(**existing_user_id_row.model_dump())
        else:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if user_object is None:
            raise ValueError(
                f"User does not exist in Hanzo_UserTable. user_id={member.user_id} and user_email={member.user_email}"
            )
        ...
        organization_membership = Hanzo_OrganizationMembershipTable(
