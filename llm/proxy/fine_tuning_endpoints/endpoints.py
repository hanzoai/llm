...
from llm.types.llms.openai import HanzoFineTuningJobCreate
        ...
        if isinstance(element, dict):
            for key, value in element.items():
                if isinstance(value, str) and value.startswith("os.environ/"):
                    element[key] = llm.get_secret(value)
...
async def create_fine_tuning_job(
    ...
    fine_tuning_request: HanzoFineTuningJobCreate,
    ...
    try:
        ...
        response = await llm.acreate_fine_tuning_job(**data)
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.create_fine_tuning_job(): Exception occurred - {}".format(
                str(e)
            )
        )
...
async def retrieve_fine_tuning_job(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.list_fine_tuning_jobs(): Exception occurred - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def list_fine_tuning_jobs(
    ...
    try:
        ...
        response = await llm.alist_fine_tuning_jobs(
            **data,
            after=after,
            limit=limit,
        )
