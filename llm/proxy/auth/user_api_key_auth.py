"""
This file handles authentication for the Hanzo Proxy.

it checks if the user passed a valid API Key to the Hanzo Proxy
...
from llm.proxy.utils import PrismaClient, ProxyLogging, _to_ns
...
def _is_ui_route(
    ...
    allowed_routes = HanzoRoutes.ui_routes.value
...
async def _user_api_key_auth_builder(  # noqa: PLR0915
    ...
    try:
        ...
        # if user wants to pass Hanzo_Master_Key as a custom header, example pass llm keys as X-Hanzo-Key: Bearer sk-1234
        ...
        if (
            route in HanzoRoutes.public_routes.value  # type: ignore
        ...
        for mapped_route in HanzoRoutes.mapped_pass_through_routes.value:  # type: ignore
        ...
        if (
            ...
            try:
                team_obj: Hanzo_TeamTableCachedObj = await get_team_object(
        ...
        if route in HanzoRoutes.master_key_only_routes.value:  # type: ignore
            raise Exception(
        ...
        if isinstance(
            ...
            assert api_key.startswith(
                "sk-"
            ), "Hanzo Virtual Key expected. Received={}, expected to start with 'sk-'.".format(
                api_key
        ...
        if valid_token is None:
            try:
                ...
                verbose_logger.info(
                    "llm.proxy.auth.user_api_key_auth.py::user_api_key_auth() - Unable to find token={} in cache or `Hanzo_VerificationTokenTable`. Defaulting 'valid_token' to None'".format(
                        api_key
                    )
                )
                valid_token = None
        ...
        if valid_token is None:
            raise Exception(
                "Invalid proxy server token passed. Received API Key (hashed)={}. Unable to find token in cache or `Hanzo_VerificationTokenTable`".format(
                    api_key
                )
            )
        ...
        if valid_token is not None:
            ...
            if valid_token.team_id is not None:
                _team_obj: Optional[Hanzo_TeamTable] = Hanzo_TeamTable(
                    team_id=valid_token.team_id,
        ...
        # check if token is from llm-ui, llm ui makes keys to allow users to login with sso. These keys can only be used for Hanzo UI functions
        # sso/login, ui/login, /key functions and /user functions
        # this will never be allowed to call /chat/completions
        token_team = getattr(valid_token, "team_id", None)
        token_type: Literal["ui", "api"] = (
            "ui"
            if token_team is not None and token_team == "llm-dashboard"
            else "api"
        )
...
async def _return_user_api_key_auth_obj(
    user_obj: Optional[Hanzo_UserTable],
    api_key: str,
...
def _is_user_proxy_admin(user_obj: Optional[Hanzo_UserTable]):
    if user_obj is None:
        return False
...
def get_api_key_from_custom_header(
    ...
    if custom_api_key:
        ...
        verbose_proxy_logger.exception(
            f"No Hanzo Virtual Key pass. Please set header={custom_llm_key_header_name}: Bearer <api_key>"
        )
    return api_key
