...
| Provider Route on Hanzo | `azure/`, [`azure/o_series/`](#azure-o-series-models) |
...
<a target="_blank" href="https://colab.research.google.com/github/BerriAI/llm/blob/main/cookbook/Hanzo_Azure_OpenAI.ipynb">
...
Here's how to call Azure OpenAI models with the Hanzo Proxy Server
...
Set `llm.enable_preview_features = True` to use Azure O1 Models with streaming support. 
...
llm.enable_preview_features = True # 👈 KEY CHANGE
...
Example video of using `tenant_id`, `client_id`, `client_secret` with Hanzo Proxy Server
...
| `custom_llm_provider` on Hanzo | `azure/` |
...
| Cost Tracking, Logging Support | ✅ Hanzo will log, track cost for Batch API Requests |
...
Azure OpenAI O-Series models are supported on Hanzo. 

Hanzo routes any deployment name with `o1` or `o3` in the model name, to the O-Series [transformation](https://github.com/BerriAI/llm/blob/91ed05df2962b8eee8492374b048d27cc144d08c/llm/llms/azure/chat/o1_transformation.py#L4) logic.
