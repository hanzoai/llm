...
def organization_role_based_access_check(
    ...
    2. IF route is a HanzoRoutes.org_admin_only_routes, then check if user is an Org Admin for that organization
    ...
    if route == "/organization/new":
        if user_object.user_role != LLMUserRoles.PROXY_ADMIN.value:
    ...
    if route in HanzoRoutes.org_admin_only_routes.value:
        ...
        user_role: Optional[LLMUserRoles] = _user_organization_role_mapping.get(
        ...
        if user_role != LLMUserRoles.ORG_ADMIN.value:
            raise ProxyException(
        ...
        if (
            ...
            if _user_role_in_passed_org != LLMUserRoles.ORG_ADMIN.value:
                raise ProxyException(
...
def get_user_organization_info(
    ...
    Args:
        user_object (Hanzo_UserTable): The user object containing organization memberships.
...
def _user_is_org_admin(
    ...
    for _membership in user_object.organization_memberships:
        if _membership.organization_id == request_data.get("organization_id", None):
            if _membership.user_role == LLMUserRoles.ORG_ADMIN.value:
                return True
