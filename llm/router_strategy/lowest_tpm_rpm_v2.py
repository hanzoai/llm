...
from llm.types.utils import LLMPydanticObjectBase, StandardLoggingPayload
from llm.utils import get_utc_datetime, print_verbose
...
class RoutingArgs(LLMPydanticObjectBase):
...
class LowestTPMLoggingHandler_v2(BaseRoutingStrategy, CustomLogger):
    ...
    def pre_call_check(self, deployment: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            deployment_name = deployment.get("llm_params", {}).get("model")
            ...
            if deployment_rpm is None:
                deployment_rpm = deployment.get("llm_params", {}).get("rpm")
            if deployment_rpm is None:
            ...
            if isinstance(e, llm.RateLimitError):
                raise e
    ...
    async def async_pre_call_check(
        ...
        Why? solves concurrency issue - https://github.com/BerriAI/llm/issues/2994
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            model = standard_logging_object["hidden_params"].get("llm_model_name")
            ...
            verbose_logger.exception(
                "llm.proxy.hooks.lowest_tpm_rpm_v2.py::log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if standard_logging_object is None:
                raise ValueError("standard_logging_object not passed in.")
            model_group = standard_logging_object.get("model_group")
            model = standard_logging_object["hidden_params"]["llm_model_name"]
            id = standard_logging_object.get("model_id")
            ...
            verbose_logger.exception(
                "llm.proxy.hooks.lowest_tpm_rpm_v2.py::async_log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def _return_potential_deployments(
        ...
        for item, item_tpm in all_deployments.items():
            ...
            if _deployment_tpm is None:
                _deployment_tpm = _deployment.get("llm_params", {}).get("tpm")
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = _deployment.get("llm_params", {}).get("rpm")
            if _deployment_rpm is None:
    ...
    async def async_get_available_deployments(
        ...
        for m in healthy_deployments:
            if isinstance(m, dict):
                ...
                deployment_name = m.get("llm_params", {}).get("model")
        ...
        try:
            ...
            for index, _deployment in enumerate(healthy_deployments):
                if isinstance(_deployment, dict):
                    ...
                    if _deployment_tpm is None:
                        _deployment_tpm = _deployment.get("llm_params", {}).get(
                            "tpm", None
                        )
                    if _deployment_tpm is None:
                    ...
                    if _deployment_rpm is None:
                        _deployment_rpm = _deployment.get("llm_params", {}).get(
                            "rpm", None
                        )
                    if _deployment_rpm is None:
