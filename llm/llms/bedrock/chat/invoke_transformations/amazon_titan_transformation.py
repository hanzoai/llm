...
from llm.llms.bedrock.chat.invoke_transformations.base_invoke_transformation import (
    AmazonInvokeConfig,
)
...
class AmazonTitanConfig(AmazonInvokeConfig, BaseConfig):
    ...
    def _map_and_modify_arg(
        ...
        """
        filter params to fit the required provider format, drop those that don't fit if user sets `llm.drop_params = True`.
        """
        filtered_stop = None
        if "stop" in supported_params and llm.drop_params:
