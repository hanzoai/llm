...
from llm.types.utils import CallTypes, StandardLoggingPayload
from llm.utils import is_prompt_caching_valid_prompt
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        if (
            ...
            verbose_logger.debug(
                "llm.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, CALL TYPE IS NOT COMPLETION"
            )
            return
        ...
        if messages is None or not isinstance(messages, list):
            verbose_logger.debug(
                "llm.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, MESSAGES IS NOT A LIST"
            )
            return
        if model_id is None:
            verbose_logger.debug(
                "llm.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, MODEL ID IS NONE"
            )
            return
