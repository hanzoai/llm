...
/sso/callback - returns JWT Redirect Response that redirects to Hanzo UI
"""
...
from llm.proxy.auth.auth_checks import get_user_object
from llm.proxy.auth.auth_utils import _has_user_setup_sso
from llm.proxy.auth.handle_jwt import JWTHandler
from llm.proxy.auth.user_api_key_auth import user_api_key_auth
from llm.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    html_form,
    show_missing_vars_in_env,
)
from llm.proxy.management_endpoints.internal_user_endpoints import new_user
from llm.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from llm.proxy.management_endpoints.team_endpoints import team_member_add
from llm.proxy.management_endpoints.types import CustomOpenID
...
async def add_missing_team_member(
    user_info: Union[NewUserResponse, Hanzo_UserTable], sso_teams: List[str]
):
    """
...
def get_disabled_non_admin_personal_key_creation():
    key_generation_settings = llm.key_generation_settings
...
async def auth_callback(request: Request):  # noqa: PLR0915
    ...
    user_info = None
    user_id_models: List = []
    max_internal_user_budget = llm.max_internal_user_budget
    internal_user_budget_duration = llm.internal_user_budget_duration
    ...
    try:
        if prisma_client is not None:
            ...
            verbose_proxy_logger.debug(
                f"user_info: {user_info}; llm.default_internal_user_params: {llm.default_internal_user_params}"
            )
            ...
            if user_info is not None:
                ...
                verbose_proxy_logger.info(
                    "user not in DB, inserting user into Hanzo DB"
                )
                # user not in DB, insert User into Hanzo DB
                ...
                user_role = (
                    user_info.user_role or LLMUserRoles.INTERNAL_USER_VIEW_ONLY
                )
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in Hanzo DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): Hanzo SSOValues / fields that were read
    ...
    if llm.default_internal_user_params:
        user_defined_values.update(llm.default_internal_user_params)  # type: ignore

    # Set budget for internal users
    if user_defined_values.get("user_role") == LLMUserRoles.INTERNAL_USER.value:
        if user_defined_values.get("max_budget") is None:
            user_defined_values["max_budget"] = llm.max_internal_user_budget
    ...
    if user_defined_values["user_role"] is None:
        user_defined_values["user_role"] = LLMUserRoles.INTERNAL_USER_VIEW_ONLY
...
async def get_ui_settings(request: Request):
    from llm.proxy.proxy_server import general_settings, proxy_state
