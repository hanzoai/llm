...
from llm.proxy._types import HanzoPromptInjectionParams, UserAPIKeyAuth
...
class _OPTIONAL_PromptInjectionDetection(CustomLogger):
    ...
    def __init__(
        self,
        prompt_injection_params: Optional[HanzoPromptInjectionParams] = None,
    ):
    ...
    async def async_moderation_hook(  # type: ignore
        ...
        if (
            ...
            if isinstance(response, llm.ModelResponse) and isinstance(
                response.choices[0], llm.Choices
            ):
