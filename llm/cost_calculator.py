...
from llm.llm_core_utils.llm_cost_calc.utils import _generic_cost_per_character
from llm.llms.anthropic.cost_calculation import (
    cost_per_token as anthropic_cost_per_token,
)
from llm.llms.azure.cost_calculation import (
    cost_per_token as azure_openai_cost_per_token,
)
from llm.llms.bedrock.image.cost_calculator import (
    cost_calculator as bedrock_image_cost_calculator,
)
from llm.llms.databricks.cost_calculator import (
    cost_per_token as databricks_cost_per_token,
)
from llm.llms.deepseek.cost_calculator import (
    cost_per_token as deepseek_cost_per_token,
)
from llm.llms.fireworks_ai.cost_calculator import (
    cost_per_token as fireworks_ai_cost_per_token,
)
from llm.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from llm.llms.openai.cost_calculation import (
    cost_per_second as openai_cost_per_second,
)
from llm.llms.openai.cost_calculation import cost_per_token as openai_cost_per_token
from llm.llms.together_ai.cost_calculator import get_model_params_and_category
from llm.llms.vertex_ai.cost_calculator import (
    cost_per_character as google_cost_per_character,
)
from llm.llms.vertex_ai.cost_calculator import (
    cost_per_token as google_cost_per_token,
)
from llm.llms.vertex_ai.cost_calculator import cost_router as google_cost_router
from llm.llms.vertex_ai.image_generation.cost_calculator import (
...
def cost_per_token(  # noqa: PLR0915
    ...
    if custom_llm_provider is not None:
        ...
        _, custom_llm_provider, _, _ = llm.get_llm_provider(model=model)
    ...
    if len(model_parts) > 1:
        ...
        model_without_prefix = model
    """
    Code block that formats model to lookup in llm.model_cost
...
def get_replicate_completion_pricing(completion_response: dict, total_time=0.0):
    ...
    # for all llm currently supported LLMs, almost all requests go to a100_80gb
...
def _get_provider_for_cost_calc(
    ...
    try:
        ...
        verbose_logger.debug(
            f"llm.cost_calculator.py::_get_provider_for_cost_calc() - Error inferring custom_llm_provider - {str(e)}"
        )
        return None
...
def _is_known_usage_objects(usage_obj):
    ...
    return isinstance(usage_obj, llm.Usage) or isinstance(
...
def completion_cost(  # noqa: PLR0915
    ...
    """
    Calculate the cost of a given completion call fot GPT-3.5-turbo, llama2, any llm supported llm.

    Parameters:
        completion_response (llm.ModelResponses): [Required] The response received from a LLM completion request.
    ...
    Exceptions:
        Raises exception if model not in the llm model cost map. Register model, via custom pricing or PR - https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json
    ...
    try:
        ...
        if completion_response is not None and (
            ...
            if isinstance(usage_obj, BaseModel) and not _is_known_usage_objects(
                ...
                setattr(
                    completion_response,
                    "usage",
                    llm.Usage(**usage_obj.model_dump()),
                )
        ...
        if custom_llm_provider is None:
            try:
                ...
                verbose_logger.debug(
                    "llm.cost_calculator.py::completion_cost() - Error inferring custom_llm_provider - {}".format(
                        str(e)
                    )
                )
        if (
            ...
            prompt_characters = llm.utils._count_characters(text=prompt)
        elif (
        ...
        # see https://replicate.com/pricing
            ...
            model in llm.replicate_models or "replicate" in model
        ) and model not in llm.model_cost:
        ...
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
            ...
            if len(messages) > 0:
                prompt_string = llm.utils.get_formatted_prompt(
                    data={"messages": messages}, call_type="completion"
                )

                prompt_characters = llm.utils._count_characters(text=prompt_string)
            if completion_response is not None and isinstance(
                completion_response, ModelResponse
            ):
                completion_string = llm.utils.get_response_string(
                    response_obj=completion_response
                )
                completion_characters = llm.utils._count_characters(
...
def get_response_cost_from_hidden_params(
    ...
    if additional_headers and "x-llm-response-cost" in additional_headers:
        response_cost = additional_headers["x-llm-response-cost"]
        if response_cost is None:
            return None
        return float(additional_headers["x-llm-response-cost"])
    return None
...
def rerank_cost(
    ...
    """
    _, custom_llm_provider, _, _ = llm.get_llm_provider(
        model=model, custom_llm_provider=custom_llm_provider
    )
    ...
    try:
        ...
        try:
            model_info: Optional[ModelInfo] = llm.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
        except Exception:
            model_info = None
...
def default_image_cost_calculator(
    ...
    if model_name_with_quality in llm.model_cost:
        cost_info = llm.model_cost[model_name_with_quality]
    elif base_model_name in llm.model_cost:
        cost_info = llm.model_cost[base_model_name]
    else:
        ...
        if model_with_quality_without_provider in llm.model_cost:
            cost_info = llm.model_cost[model_with_quality_without_provider]
        elif model_without_provider in llm.model_cost:
            cost_info = llm.model_cost[model_without_provider]
        else:
            raise Exception(
