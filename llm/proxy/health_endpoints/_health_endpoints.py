...
from llm.constants import HEALTH_CHECK_TIMEOUT_SECONDS
...
from llm.proxy.health_check import (
    _clean_endpoint_data,
    _update_llm_params_for_health_check,
...
async def health_services_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if (
            ...
            or (service in llm.success_callback and service != "langfuse")
        ):
            _ = await llm.acompletion(
                model="openai/llm-mock-response-model",
                messages=[{"role": "user", "content": "Hey, how's it going?"}],
                user="llm:/health/services",
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.health_services_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def health_endpoint(
    ...
    try:
        if llm_model_list is None:
            # if no router set, check if user set a model using llm --model ollama/llama2
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.py::health_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def active_callbacks():
    ...
    from llm.proxy.proxy_server import general_settings, proxy_logging_obj
...
async def health_readiness():
    ...
    """
    from llm.proxy.proxy_server import prisma_client, version
    ...
    try:
        ...
        try:
            ...
            success_callback_names = [
                callback_name(x) for x in llm.success_callback
            ]
        except AttributeError:
            # don't let this block the /health/readiness response, if we can't convert to str -> return llm.success_callback
            success_callback_names = llm.success_callback

        # check Cache
        cache_type = None
        if llm.cache is not None:
            from llm.caching.caching import RedisSemanticCache

            cache_type = llm.cache.type
...
@router.get(
    "/health/liveliness",  # Historical LLM name; doesn't match k8s terminology but kept for backwards compatibility
    tags=["health"],
    dependencies=[Depends(user_api_key_auth)],
)
@router.get(
...
async def test_model_connection(
    ...
    llm_params: Dict = fastapi.Body(
        None,
        description="Parameters for llm.completion, llm.embedding for the health check",
    ),
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
):
    """
    ...
    try:
        ...
        llm_params = _update_llm_params_for_health_check(
            model_info={},
            llm_params=llm_params,
        )
        mode = mode or llm_params.pop("mode", None)
        result = await run_with_timeout(
            llm.ahealth_check(
                model_params=llm_params,
                mode=mode,
                prompt="test from llm",
                input=["test from llm"],
            ),
        ...
        cleaned_result = _clean_endpoint_data(
            {**llm_params, **result}, details=True
        )
        ...
        verbose_proxy_logger.error(
            f"llm.proxy.health_endpoints.test_model_connection(): Exception occurred - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
