...
from llm._logging import _is_debugging_on, verbose_logger
from llm.batches.batch_utils import _handle_completed_batch
from llm.caching.caching import DualCache, InMemoryCache
...
from llm.integrations.arize.arize import ArizeLogger
...
from llm.llm_core_utils.get_llm_params import get_llm_params
from llm.llm_core_utils.llm_cost_calc.tool_call_cost_tracking import (
...
from llm.responses.utils import ResponseAPILoggingUtils
...
from llm.types.router import SPECIAL_MODEL_INFO_PARAMS
...
from llm.utils import _get_base_model_from_metadata, executor, print_verbose
...
try:
    ...
    verbose_logger.debug(
        f"[Non-Blocking] Unable to import GenericAPILogger - LLM Enterprise Feature - {str(e)}"
    )
...
class ServiceTraceIDCache:
    ...
    def get_cache(self, llm_call_id: str, service_name: str) -> Optional[str]:
        key_name = "{}:{}".format(service_name, llm_call_id)
        response = self.cache.get_cache(key=key_name)
        return response

    def set_cache(self, llm_call_id: str, service_name: str, trace_id: str) -> None:
        key_name = "{}:{}".format(service_name, llm_call_id)
...
class Logging(LLMLoggingBaseClass):
    ...
    def __init__(
        ...
        if kwargs is not None:
            ...
            llm_params = scrub_sensitive_keys_in_metadata(llm_params)
    ...
    def process_dynamic_callbacks(self):
        ...
        If a callback is in llm._known_custom_logger_compatible_callbacks, it needs to be intialized and added to the respective dynamic_* callback list.
        """
    ...
    def _process_dynamic_callback_list(
        ...
        - If a callback is in llm._known_custom_logger_compatible_callbacks,
        ...
        for callback in callback_list:
            if (
                ...
                and callback in llm._known_custom_logger_compatible_callbacks
            ):
    ...
    def update_environment_variables(
        ...
        **additional_params,
        ...
        self.llm_params = {
            **self.llm_params,
            **scrub_sensitive_keys_in_metadata(llm_params),
        }
        self.logger_fn = llm_params.get("logger_fn", None)
        ...
        if (
            llm_params.get("input_cost_per_token") is not None
            or llm_params.get("input_cost_per_second") is not None
            or llm_params.get("output_cost_per_token") is not None
            or llm_params.get("output_cost_per_second") is not None
        ):
    ...
    def get_custom_logger_for_prompt_management(
        ...
        for callback_name in llm._known_custom_logger_compatible_callbacks:
        ...
        prompt_management_loggers = (
            llm.logging_callback_manager.get_custom_loggers_for_type(
    ...
    def _pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        self.model_call_details["llm_params"]["api_base"] = (
    ...
    def pre_call(self, input, api_key, model=None, additional_args={}):  # noqa: PLR0915
        ...
        llm.error_logs["PRE_CALL"] = locals()
        try:
            self._pre_call(
                input=input,
                api_key=api_key,
            ...
            if (
                ...
                _llm_params = self.model_call_details.get("llm_params", {})
                _metadata = _llm_params.get("metadata", {}) or {}
                try:
            ...
            if self.logger_fn and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "LLM.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(
                            str(e)
                        )
                    )
            ...
            callbacks = llm.input_callback + (self.dynamic_input_callbacks or [])
                ...
                try:
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.input_log_event(
                            ...
                            llm_call_id=self.llm_params["llm_call_id"],
                    ...
                    verbose_logger.exception(
                        "llm.Logging.pre_call(): Exception occured - {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
                        f"LLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
    ...
    def _print_llm_call_debugging_log(
        ...
        Prints the RAW curl command sent from LLM
        """
        if _is_debugging_on():
            if json_logs:
    ...
    def _get_masked_headers(
        ...
        Masks the headers of the request sent from LLM
        """
    ...
    def post_call(
        ...
        llm.error_logs["POST_CALL"] = locals()
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "LLM.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
    ...
    def _response_cost_calculator(
        ...
        custom_pricing = use_custom_pricing_for_model(
            llm_params=(
                self.llm_params if hasattr(self, "llm_params") else None
            )
        )
    ...
    def should_run_callback(
        self, callback: llm.CALLBACK_TYPES, llm_params: dict, event_hook: str
    ) -> bool:

        if llm.global_disable_no_log_param:
            return True

        if llm_params.get("no-log", False) is True:
    ...
    def _success_handler_helper_fn(
        ...
        try:
            ...
            if (
                ...
                if (
                    ...
                    if hidden_params:
                        ...
                        if self.model_call_details.get("llm_params") is not None:
                            self.model_call_details["llm_params"].setdefault(
                                "metadata", {}
                            )
                            if (
                                self.model_call_details["llm_params"]["metadata"]
                                is None
                            ):
                                self.model_call_details["llm_params"][
                                    "metadata"
                                ] = {}

                            self.model_call_details["llm_params"]["metadata"][  # type: ignore
                                "hidden_params"
                            ] = getattr(
            ...
            if (
                ...
                llm._current_cost += llm.completion_cost(
                    model=self.model,
                    prompt="",
            ...
            raise Exception(f"[Non-Blocking] LLM.Success_Call Error: {str(e)}")

    def success_handler(  # noqa: PLR0915
        self, result=None, start_time=None, end_time=None, cache_hit=None, **kwargs
    ):
        verbose_logger.debug(
            f"Logging Details LLM-Success Call: Cache_hit={cache_hit}"
        )
        ...
        try:
            ...
            if complete_streaming_response is not None:
                verbose_logger.debug(
                    "Logging Details LLM-Success Call streaming complete"
                )
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=llm.success_callback,
            )
            ...
            for callback in callbacks:
                try:
                    llm_params = self.model_call_details.get("llm_params", {})
                    ...
                    if (
                        callback == "openmeter"
                        and self.model_call_details.get("llm_params", {}).get(
                            "acompletion", False
                        )
                        is not True
                        and self.model_call_details.get("llm_params", {}).get(
                    ...
                    print_verbose(
                        f"LLM.LoggingError: [Non-Blocking] Exception occurred while success logging with integrations {traceback.format_exc()}"
                    )
                    print_verbose(
                        f"LLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "LLM.LoggingError: [Non-Blocking] Exception occurred while success logging {}".format(
                    str(e)
                ),
            )
    ...
    async def async_success_handler(  # noqa: PLR0915
        ...
        """
        print_verbose(
            "Logging Details LLM-Async Success Call, cache_hit={}".format(cache_hit)
        )
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=llm._async_success_callback,
        )
        ...
        for callback in callbacks:
            if isinstance(callback, CustomGuardrail):
                ...
                result = redact_message_input_output_from_custom_logger(
                    result=result, llm_logging_obj=self, custom_logger=callback
                )
                self.model_call_details, result = await callback.async_logging_hook(
                    kwargs=self.model_call_details,
                    result=result,
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.error(
                    f"LLM.LoggingError: [Non-Blocking] Exception occurred while success logging {traceback.format_exc()}"
                )
                pass
    ...
    def _failure_handler_helper_fn(
        ...
        if hasattr(exception, "headers") and isinstance(exception.headers, dict):
            self.model_call_details.setdefault("llm_params", {})
            metadata = (
                self.model_call_details["llm_params"].get("metadata", {}) or {}
            )
    ...
    async def special_failure_handlers(self, exception: Exception):
        ...
        llm_params: dict = self.model_call_details.get("llm_params") or {}
        ...
        for callback in llm._async_failure_callback:
            if isinstance(callback, CustomLogger):  # custom logger class
    ...
    def failure_handler(  # noqa: PLR0915
        ...
        verbose_logger.debug(
            f"Logging Details LLM-Failure Call: {llm.failure_callback}"
        )
        try:
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=llm.failure_callback,
            )
                ...
                try:
                    ...
                    if callback == "sentry":
                        ...
                        supabaseClient.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            llm_call_id=self.model_call_details["llm_call_id"],
                            print_verbose=print_verbose,
                        )
                    if (
                        callable(callback) and customLogger is not None
                    ...
                    print_verbose(
                        f"LLM.LoggingError: [Non-Blocking] Exception occurred while failure logging with integrations {str(e)}"
                    )
                    print_verbose(
                        f"LLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "LLM.LoggingError: [Non-Blocking] Exception occurred while failure logging {}".format(
                    str(e)
                )
            )
    ...
    async def async_failure_handler(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=llm._async_failure_callback,
        )
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(
                    "LLM.LoggingError: [Non-Blocking] Exception occurred while failure \
                        logging {}\nCallback={}".format(
                        str(e), callback
                    )
                )
    ...
    def _get_trace_id(self, service_name: Literal["langfuse"]) -> Optional[str]:
        ...
        if service_name == "langfuse":
            trace_id = in_memory_trace_id_cache.get_cache(
                llm_call_id=self.llm_call_id, service_name=service_name
            )
    ...
    def _remove_internal_llm_callbacks(self, callbacks: List) -> List:
        ...
        """
        filtered = [
            cb for cb in callbacks if not self._is_internal_llm_proxy_callback(cb)
        ]
    ...
    def _is_internal_llm_proxy_callback(self, cb) -> bool:
    ...
    def _remove_internal_custom_logger_callbacks(self, callbacks: List) -> List:
        ...
        for _c in callbacks:
            if isinstance(_c, CustomLogger):
                ...
                isinstance(_c, str)
                and _c in llm._known_custom_logger_compatible_callbacks
            ):
                continue
    ...
    def _handle_anthropic_messages_response_logging(self, result: Any) -> ModelResponse:
        ...
        result = llm.AnthropicConfig().transform_response(
...
def _init_custom_logger_compatible_class(  # noqa: PLR0915
    ...
    llm_router: Optional[
        Any
    ],  # expect llm.Router, but typing errors due to circular import
    ...
    try:
        ...
        if logging_integration == "lago":
            ...
            from llm.integrations.opentelemetry import (
                OpenTelemetry,
                OpenTelemetryConfig,
            )
            ...
            if llm_router is not None and isinstance(llm_router, llm.Router):
...
def _get_custom_logger_settings_from_proxy_server(callback_name: str) -> Dict:
    ...
    """
    from llm.proxy.proxy_server import callback_settings
...
def use_custom_pricing_for_model(llm_params: Optional[dict]) -> bool:
    """
    Check if the model uses custom pricing

    Returns True if any of `SPECIAL_MODEL_INFO_PARAMS` are present in `llm_params` or `model_info`
    """
    if llm_params is None:
        return False

    metadata: dict = llm_params.get("metadata", {}) or {}
    model_info: dict = metadata.get("model_info", {}) or {}

    for _custom_cost_param in SPECIAL_MODEL_INFO_PARAMS:
        if llm_params.get(_custom_cost_param, None) is not None:
            return True
...
class StandardLoggingPayloadSetup:
    ...
    def get_standard_logging_metadata(
        ...
        if llm_params is not None:
            prompt_id = cast(Optional[str], llm_params.get("prompt_id", None))
            prompt_variables = cast(
                Optional[dict], llm_params.get("prompt_variables", None)
            )
        ...
        if isinstance(metadata, dict):
            ...
            _potential_requester_metadata = metadata.get(
                "metadata", None
            )  # check if user passed metadata in the sdk request - e.g. metadata for langsmith logging - https://docs.hanzo.ai/docs/observability/langsmith_integration#set-langsmith-fields
            if (
    ...
    def get_model_cost_information(
        ...
        if model_cost_name is None:
            ...
            try:
                _model_cost_information = llm.get_model_info(
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        llm_params = kwargs.get("llm_params", {})
        proxy_server_request = llm_params.get("proxy_server_request") or {}

        metadata: dict = (
            llm_params.get("llm_metadata")
            or llm_params.get("metadata", None)
            or {}
        )
        ...
        id = response_obj.get("id", kwargs.get("llm_call_id"))
        ...
        custom_pricing = use_custom_pricing_for_model(llm_params=llm_params)
        ...
        payload: StandardLoggingPayload = StandardLoggingPayload(
            id=str(id),
            trace_id=kwargs.get("llm_trace_id"),  # type: ignore
...
def scrub_sensitive_keys_in_metadata(llm_params: Optional[dict]):
    if llm_params is None:
        llm_params = {}
