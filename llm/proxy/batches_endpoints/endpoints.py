...
from llm.proxy.openai_files_endpoints.files_endpoints import is_known_model
...
async def create_batch(
    ...
    try:
        ...
        if (
            ...
            response = await llm.acreate_batch(
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.create_batch(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def retrieve_batch(
    ...
    try:
        ...
        # setup logging
        data["llm_call_id"] = request.headers.get(
        ...
        if llm.enable_loadbalancing_on_batch_endpoints is True:
            ...
            response = await llm.aretrieve_batch(
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.retrieve_batch(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def list_batches(
    ...
    ```
    """
    from llm.proxy.proxy_server import proxy_logging_obj, version
    ...
    try:
        ...
        response = await llm.alist_batches(
...
async def cancel_batch(
    ...
    try:
        ...
        response = await llm.acancel_batch(
            custom_llm_provider=custom_llm_provider,  # type: ignore
            **_cancel_batch_data
        )
