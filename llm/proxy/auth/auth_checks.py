...
from llm.caching.dual_cache import LimitedSizeOrderedDict
...
from llm.proxy._types import (
    ...
    Hanzo_EndUserTable,
    Hanzo_JWTAuth,
    Hanzo_OrganizationMembershipTable,
    Hanzo_OrganizationTable,
    Hanzo_TeamTable,
...
from llm.proxy.utils import PrismaClient, ProxyLogging, log_db_metrics
...
all_routes = HanzoRoutes.openai_routes.value + HanzoRoutes.management_routes.value
...
async def common_checks(
    ...
    7. [OPTIONAL] If 'llm.max_budget' is set (>0), is proxy under budget
    ...
    if end_user_object is not None and end_user_object.llm_budget_table is not None:
        end_user_budget = end_user_object.llm_budget_table.max_budget
    ...
    if (
        ...
        if global_proxy_spend > llm.max_budget:
            raise llm.BudgetExceededError(
                current_cost=global_proxy_spend, max_budget=llm.max_budget
            )
    ...
    if _request_metadata.get("guardrails"):
        ...
        from llm.proxy.guardrails.guardrail_helpers import can_modify_guardrails
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|HanzoRoutes] - the list of allowed routes for the user.
    """

    for allowed_route in allowed_routes:
        if (
            allowed_route in HanzoRoutes.__members__
            and user_route in HanzoRoutes[allowed_route].value
        ):
            return True
...
def allowed_routes_check(
    ...
    user_route: str,
    llm_proxy_roles: Hanzo_JWTAuth,
) -> bool:
    """
    Check if user -> not admin - allowed to access these routes
    """

    if user_role == LLMUserRoles.PROXY_ADMIN:
        is_allowed = _allowed_routes_check(
            user_route=user_route,
            allowed_routes=llm_proxy_roles.admin_allowed_routes,
        )
        return is_allowed

    elif user_role == LLMUserRoles.TEAM:
        if llm_proxy_roles.team_allowed_routes is None:
            """
            ...
            return is_allowed
        elif llm_proxy_roles.team_allowed_routes is not None:
            is_allowed = _allowed_routes_check(
                user_route=user_route,
                allowed_routes=llm_proxy_roles.team_allowed_routes,
            )
            return is_allowed
    return False
...
def allowed_route_check_inside_route(
    ...
    ret_val = True
    if (
        user_api_key_dict.user_role != LLMUserRoles.PROXY_ADMIN
        and user_api_key_dict.user_role != LLMUserRoles.PROXY_ADMIN_VIEW_ONLY
    ):
        ret_val = False
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = HanzoRoutes[route_name].value
...
async def get_end_user_object(
    ...
    def check_in_budget(end_user_obj: Hanzo_EndUserTable):
        if end_user_obj.llm_budget_table is None:
            return
        end_user_budget = end_user_obj.llm_budget_table.max_budget
    ...
    if cached_user_obj is not None:
        if isinstance(cached_user_obj, dict):
            return_obj = Hanzo_EndUserTable(**cached_user_obj)
            check_in_budget(end_user_obj=return_obj)
            return return_obj
        elif isinstance(cached_user_obj, Hanzo_EndUserTable):
    ...
    try:
        ...
        _response = Hanzo_EndUserTable(**response.dict())
...
async def get_user_object(
    ...
    - if valid, return Hanzo_UserTable object with defined limits
    ...
    if cached_user_obj is not None:
        if isinstance(cached_user_obj, dict):
            return Hanzo_UserTable(**cached_user_obj)
        elif isinstance(cached_user_obj, Hanzo_UserTable):
    ...
    try:
        ...
        if (
            ...
            # dump each organization membership to type Hanzo_OrganizationMembershipTable
            _dumped_memberships = [
                Hanzo_OrganizationMembershipTable(**membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = Hanzo_UserTable(**dict(response))
...
async def _get_team_db_check(
    ...
    response = await prisma_client.db.llm_teamtable.find_unique(
        where={"team_id": team_id}
    )

    if response is None and team_id_upsert:
        response = await prisma_client.db.llm_teamtable.create(
...
async def _get_team_object_from_db(team_id: str, prisma_client: PrismaClient):
    return await prisma_client.db.llm_teamtable.find_unique(
        where={"team_id": team_id}
    )
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response = Hanzo_TeamTableCachedObj(**response.dict())
...
async def _get_team_object_from_cache(
...
) -> Optional[Hanzo_TeamTableCachedObj]:
    cached_team_obj: Optional[Hanzo_TeamTableCachedObj] = None
    ...
    if cached_team_obj is not None:
        if isinstance(cached_team_obj, dict):
            return Hanzo_TeamTableCachedObj(**cached_team_obj)
        elif isinstance(cached_team_obj, Hanzo_TeamTableCachedObj):
...
async def get_team_object(
    ...
    - if valid, return Hanzo_TeamTable object with defined limits
    - if not, then raise an error
...
async def _handle_failed_db_connection_for_get_key_object(
    ...
    """
    Handles httpx.ConnectError when reading a Virtual Key from Hanzo DB
...
async def get_org_object(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[Hanzo_OrganizationTable]:
    """
    - Check if org id in proxy Org Table
    - if valid, return Hanzo_OrganizationTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    if cached_org_obj is not None:
        if isinstance(cached_org_obj, dict):
            return Hanzo_OrganizationTable(**cached_org_obj)
        elif isinstance(cached_org_obj, Hanzo_OrganizationTable):
...
async def _can_object_call_model(
    ...
    """
    if model in llm.model_alias_map:
...
async def _virtual_key_max_budget_check(
    valid_token: UserAPIKeyAuth,
    proxy_logging_obj: ProxyLogging,
    user_obj: Optional[Hanzo_UserTable] = None,
):
    """
    Raises:
