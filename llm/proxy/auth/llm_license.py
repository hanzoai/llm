# What is this?
## If llm license in env, checks if it's valid
import base64
import json
import os
...
class LicenseCheck:
    ...
    def is_premium(self) -> bool:
        ...
        2. _verify: checks if license is valid calling llm API. This is the old way we were generating/validating license
        """
        try:
            verbose_proxy_logger.debug(
                "llm.proxy.auth.llm_license.py::is_premium() - ENTERING 'IS_PREMIUM' - Hanzo License={}".format(
                    self.license_str
                )
            )
