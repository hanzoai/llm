...
from llm.types.utils import ChatCompletionMessageToolCall, Choices, Delta
from llm.types.utils import GenericStreamingChunk as GChunk
...
from llm.utils import CustomStreamWrapper, get_secret
...
async def make_call(
    ...
    bedrock_invoke_provider: Optional[llm.BEDROCK_INVOKE_PROVIDERS_LITERAL] = None,
):
    try:
        if client is None:
        ...
        if fake_stream:
            model_response: (
                ModelResponse
            ) = llm.AmazonConverseConfig()._transform_response(
                model=model,
                response=response,
...
class BedrockLLM(BaseAWSLLM):
    ...
    def process_response(  # noqa: PLR0915
            ...
            if provider == "cohere":
                ...
                if model.startswith("anthropic.claude-3"):
                    ...
                    if (
                        ...
                        # streaming_model_response.choices = [llm.utils.StreamingChoices()]
                        streaming_choice = llm.utils.StreamingChoices()
                        ...
                        if isinstance(model_response.choices[0], llm.Choices):
                            if getattr(
        ...
        prompt_tokens = int(
            bedrock_input_tokens or llm.token_counter(messages=messages)
        )

        completion_tokens = int(
            bedrock_output_tokens
            or llm.token_counter(
    ...
    def completion(  # noqa: PLR0915
        ...
        if aws_region_name is None:
            # check env #
            llm_aws_region_name = get_secret("AWS_REGION_NAME", None)

            if llm_aws_region_name is not None and isinstance(
                llm_aws_region_name, str
            ):
                aws_region_name = llm_aws_region_name
        ...
        if provider == "cohere":
            if model.startswith("cohere.command-r"):
                ## LOAD CONFIG
                config = llm.AmazonCohereChatConfig().get_config()
                ...
                ## LOAD CONFIG
                config = llm.AmazonCohereConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            if model.startswith("anthropic.claude-3"):
                ...
                ## LOAD CONFIG
                config = llm.AmazonAnthropicClaude3Config.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
                ...
                ## LOAD CONFIG
                config = llm.AmazonAnthropicConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = llm.AmazonAI21Config.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = llm.AmazonMistralConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
            ...
            ## LOAD CONFIG
            config = llm.AmazonTitanConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > amazon_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = llm.AmazonLlamaConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
    ...
    def _get_provider_from_model_path(
        model_path: str,
    ) -> Optional[llm.BEDROCK_INVOKE_PROVIDERS_LITERAL]:
        """
        ...
        if len(parts) >= 1:
            provider = parts[0]
            if provider in get_args(llm.BEDROCK_INVOKE_PROVIDERS_LITERAL):
                return cast(llm.BEDROCK_INVOKE_PROVIDERS_LITERAL, provider)
        return None

    def get_bedrock_model_id(
        self,
        optional_params: dict,
        provider: Optional[llm.BEDROCK_INVOKE_PROVIDERS_LITERAL],
        model: str,
    ) -> str:
...
class AmazonDeepSeekR1StreamDecoder(AWSEventStreamDecoder):
    def __init__(
        ...
        super().__init__(model=model)
        from llm.llms.bedrock.chat.invoke_transformations.amazon_deepseek_transformation import (
...
class MockResponseIterator:  # for returning ai21 streaming responses
    ...
    def _handle_json_mode_chunk(
        ...
        if self.json_mode is True and tool_calls is not None:
            message = llm.AnthropicConfig()._convert_tool_response_to_message(
