...
class AnthropicConfig(BaseConfig):
    ...
    def _map_stop_sequences(
        ...
        if isinstance(stop, str):
            if (
                stop.isspace() and llm.drop_params is True
            ...
            for v in stop:
                if (
                    v.isspace() and llm.drop_params is True
                ):  # anthropic doesn't allow whitespace characters as stop-sequences
                    continue
                new_v.append(v)
            if len(new_v) > 0:
    ...
    def transform_request(
        ...
        if (
            ...
            if llm.modify_params:
                ...
                raise llm.UnsupportedParamsError(
                    message="Anthropic doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `llm.modify_params = True` // `llm_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
        ...
        ## Load Config
        config = llm.AnthropicConfig.get_config()
        ...
        _llm_metadata = llm_params.get("metadata", None)
        if (
            _llm_metadata
            and isinstance(_llm_metadata, dict)
            and "user_id" in _llm_metadata
        ):
            optional_params["metadata"] = {"user_id": _llm_metadata["user_id"]}
        ...
        try:
            if json_mode_content_str is not None:
                ...
                if (
                    ...
                    _message = llm.Message(content=json.dumps(values))
                    return _message
                else:
            ...
            return llm.Message(content=json_mode_content_str)
        return None
