...
class ReplicateConfig(BaseConfig):
    ...
    def transform_request(
        ...
        ## Load Config
        config = llm.ReplicateConfig.get_config()
        ...
        if model in llm.custom_prompt_dict:
            # check if the model has a registered custom prompt
            model_prompt_details = llm.custom_prompt_dict[model]
        ...
        if prompt is None or not isinstance(prompt, str):
            raise ReplicateError(
                status_code=400,
                message="Hanzo Error - prompt is not a string - {}".format(prompt),
                headers={},
            )
    ...
    def transform_response(
        ...
        if raw_response_json.get("status") != "succeeded":
            raise ReplicateError(
                status_code=422,
                message="Hanzo Error - prediction not succeeded - {}".format(
                    raw_response_json
                ),
    ...
    def get_prediction_url(self, response: httpx.Response) -> str:
        ...
        if prediction_url is None:
            raise ReplicateError(
                status_code=400,
                message="Hanzo Error - prediction url is None - {}".format(
                    response_json
                ),
