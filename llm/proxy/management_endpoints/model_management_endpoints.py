...
from llm.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper
from llm.proxy.management_endpoints.team_endpoints import (
    team_model_add,
    update_team,
)
from llm.proxy.management_helpers.audit_logs import create_object_audit_log
from llm.proxy.utils import PrismaClient
from llm.types.router import (
    Deployment,
    DeploymentTypedDict,
    HanzoParamsTypedDict,
    updateDeployment,
)
...
async def get_db_model(
    ...
    db_model = cast(
        Optional[BaseModel],
        await prisma_client.db.llm_proxymodeltable.find_unique(
...
def update_db_model(
    ...
    merged_deployment_dict = DeploymentTypedDict(
        ...
        llm_params=HanzoParamsTypedDict(
            **db_model.llm_params.model_dump(exclude_none=True)  # type: ignore
        ),
    )
    # update model name
    ...
    # update llm params
    if updated_patch.llm_params:
        # Encrypt any sensitive values
        encrypted_params = {
            k: encrypt_value_helper(v)
            for k, v in updated_patch.llm_params.model_dump(
                exclude_none=True
            ).items()
        }

        merged_deployment_dict["llm_params"].update(encrypted_params)  # type: ignore
    ...
    if "llm_params" in merged_deployment_dict:
        prisma_compatible_model_dict["llm_params"] = json.dumps(
            merged_deployment_dict["llm_params"]
        )
...
async def patch_model(
    ...
    try:
        ...
        update_data["updated_by"] = (
            user_api_key_dict.user_id or llm_proxy_admin_name
        )
        update_data["updated_at"] = cast(str, get_utc_datetime())

        # Perform partial update
        updated_model = await prisma_client.db.llm_proxymodeltable.update(
            where={"model_id": model_id},
            data=update_data,
        )
...
async def _add_model_to_db(
...
) -> Optional[Hanzo_ProxyModelTable]:
    # encrypt llm params #
    _llm_params_dict = model_params.llm_params.dict(exclude_none=True)
    _orignal_llm_model_name = model_params.llm_params.model
    for k, v in _llm_params_dict.items():
        encrypted_value = encrypt_value_helper(
            value=v, new_encryption_key=new_encryption_key
        )
        model_params.llm_params[k] = encrypted_value
    _data: dict = {
        "model_id": model_params.model_info.id,
        "model_name": model_params.model_name,
        "llm_params": model_params.llm_params.model_dump_json(exclude_none=True),  # type: ignore
    ...
    if should_create_model_in_db:
        model_response = await prisma_client.db.llm_proxymodeltable.create(
            data=_data  # type: ignore
        )
    else:
        model_response = Hanzo_ProxyModelTable(**_data)
...
async def delete_model(
    ...
    try:
        ...
        if store_model_in_db is True:
            ...
            """
            # encrypt llm params #
            result = await prisma_client.db.llm_proxymodeltable.delete(
...
async def add_new_model(
    ...
    try:
        ...
        model_response: Optional[Hanzo_ProxyModelTable] = None
        # update DB
        if store_model_in_db is True:
            """
            - store model_list in db
            ...
            try:
                _original_llm_model_name = model_params.model_name
                ...
                if "slack" in _alerting:
                    ...
                    await proxy_logging_obj.slack_alerting_instance.model_added_alert(
                        model_name=model_params.model_name,
                        llm_model_name=_original_llm_model_name,
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.add_new_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_model(
    ...
    try:
        ...
        if store_model_in_db is True:
            ...
            _existing_llm_params = (
                await prisma_client.db.llm_proxymodeltable.find_unique(
                    where={"model_id": _model_id}
                )
            )
            if _existing_llm_params is None:
                if (
            ...
            _existing_llm_params_dict = dict(
                _existing_llm_params.llm_params
            )

            if model_params.llm_params is None:
                raise Exception("llm_params not provided")

            _new_llm_params_dict = model_params.llm_params.dict(
                exclude_none=True
            )

            ### ENCRYPT PARAMS ###
            for k, v in _new_llm_params_dict.items():
                encrypted_value = encrypt_value_helper(value=v)
                model_params.llm_params[k] = encrypted_value

            ### MERGE WITH EXISTING DATA ###
            merged_dictionary = {}
            _mp = model_params.llm_params.dict()

            for key, value in _mp.items():
                if value is not None:
                    merged_dictionary[key] = value
                elif (
                    key in _existing_llm_params_dict
                    and _existing_llm_params_dict[key] is not None
                ):
                    merged_dictionary[key] = _existing_llm_params_dict[key]
                else:
                    pass

            _data: dict = {
                "llm_params": json.dumps(merged_dictionary),  # type: ignore
                "updated_by": user_api_key_dict.user_id or LLM_PROXY_ADMIN_NAME,
            }
            model_response = await prisma_client.db.llm_proxymodeltable.update(
                where={"model_id": _model_id},
                data=_data,  # type: ignore
            )
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.update_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
