...
def route_in_additonal_public_routes(current_route: str):
    ...
    general_settings:
        master_key: sk-1234
        public_routes: ["LLMRoutes.public_routes", "/spend/calculate"]
    ```
    """
...
def should_run_auth_on_pass_through_provider_route(route: str) -> bool:
    """
    Use this to decide if the rest of the LLM Virtual Key auth checks should run on /vertex-ai/{endpoint} routes
    Use this to decide if the rest of the LLM Virtual Key auth checks should run on provider pass through routes
    ...
    # only enabled for LLM Enterprise
    return True
