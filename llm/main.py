...
from llm.exceptions import LLMUnknownProvider
from llm.integrations.custom_logger import CustomLogger
from llm.llm_core_utils.audio_utils.utils import get_audio_file_for_health_check
from llm.llm_core_utils.health_check_utils import (
...
from llm.llm_core_utils.mock_functions import (
    mock_embedding,
...
from llm.realtime_api.main import _realtime_health_check
from llm.secret_managers.main import get_secret_str
from llm.types.router import GenericLLMParams
from llm.types.utils import RawRequestTypedDict
...
class LLM:
    def __init__(
        ...
        *,
        ...
        max_retries: Optional[int] = llm.num_retries,
...
async def acompletion(
    ...
    **kwargs,
    ...
    fallbacks = fallbacks or llm.model_fallbacks
    if fallbacks is not None:
        ...
        if response is None:
            raise Exception(
                "No response from fallbacks. Got none. Turn on `llm.set_verbose=True` to see more details."
            )
        return response
    ...
    try:
        ...
        if (
            ...
            response = llm.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
...
def _handle_mock_potential_exceptions(
    ...
    if isinstance(mock_response, Exception):
    ...
    elif isinstance(mock_response, str) and mock_response == "llm.RateLimitError":
        ...
        and mock_response == "llm.ContextWindowExceededError"
    ):
        ...
        isinstance(mock_response, str)
        and mock_response == "llm.InternalServerError"
    ):
...
def mock_completion(
    ...
    **kwargs,
    ...
    Returns:
        llm.ModelResponse: A ModelResponse simulating a completion response with the specified model, messages, and mock response.
    ...
    try:
        ...
        if isinstance(mock_response, str) and mock_response.startswith(
            ...
            mock_response = llm.MockException(
        ...
        try:
            _, custom_llm_provider, _, _ = llm.utils.get_llm_provider(model=model)
...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    custom_llm_provider = kwargs.get("custom_llm_provider", None)
    llm_logging_obj = kwargs.get("llm_logging_obj", None)
    ...
    ### COPY MESSAGES ### - related issue https://github.com/hanzoai/llm/discussions/4489
    ...
    llm_params = {}  # used to prevent unbound var errors
    ## PROMPT MANAGEMENT HOOKS ##

    if isinstance(llm_logging_obj, LLMLoggingObj) and prompt_id is not None:
    ...
    try:
        ...
        logging = llm_logging_obj
        fallbacks = fallbacks or llm.model_fallbacks
        if fallbacks is not None:
            return completion_with_fallbacks(**args)
        if model_list is not None:
            deployments = [
                m["llm_params"] for m in model_list if m["model_name"] == model
            ]
            return llm.batch_completion_models(deployments=deployments, **args)
        if llm.model_alias_map and model in llm.model_alias_map:
            model = llm.model_alias_map[
                model
            ]  # update the model to the actual value if an alias has been passed in
        model_response = ModelResponse()
        setattr(model_response, "usage", llm.Usage())
        if (
        ...
        if llm.add_function_to_prompt and optional_params.get(
        ...
        llm_params = get_llm_params(
            ...
            custom_llm_provider=custom_llm_provider,
            api_base=api_base,
            llm_call_id=kwargs.get("llm_call_id", None),
            model_alias_map=llm.model_alias_map,
        ...
        if custom_llm_provider == "azure":
            ...
            api_base = api_base or llm.api_base or get_secret("AZURE_API_BASE")
            ...
            azure_ad_token_provider = llm_params.get(
                "azure_ad_token_provider", None
            )

            headers = headers or llm.headers
            ...
            if llm.AzureOpenAIO1Config().is_o_series_model(model=model):
                ...
                ## LOAD CONFIG - if set
                config = llm.AzureOpenAIO1Config.get_config()
                ...
                ## LOAD CONFIG - if set
                config = llm.AzureOpenAIConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in optional_params
                    ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_version = (
                api_version or llm.api_version or get_secret("AZURE_API_VERSION")
            )
            ...
            api_key = (
                api_key
                or llm.api_key  # for deepinfra/perplexity/anyscale/friendliai we check in get_llm_provider and pass in the api key from there
                or llm.openai_key
            ...
            in llm.openai_text_completion_compatible_providers
            ...
            ## LOAD CONFIG - if set
            config = llm.OpenAITextCompletionConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                    ...
                    optional_params[k] = v
            if llm.organization:
                openai.organization = llm.organization
            ...
            if (
                ...
                _response = llm.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
            ...
            ## LOAD CONFIG - if set
            config = llm.GroqChatConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
            ...
            or model in llm.replicate_models
        ):
            # Setting the relevant API KEY for replicate, replicate defaults to using os.environ.get("REPLICATE_API_TOKEN")
            replicate_key = (
                api_key
                or llm.replicate_key
                or llm.api_key
            ...
            custom_prompt_dict = custom_prompt_dict or llm.custom_prompt_dict
            ...
            or model in llm.clarifai_models
        ):
            clarifai_key = None
            clarifai_key = (
                api_key
            ...
            api_base = llm.ClarifaiConfig()._convert_model_to_url(model, api_base)
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
            ...
            response = anthropic_chat_completions.completion(
                ...
                api_base=api_base,
                acompletion=acompletion,
                custom_prompt_dict=llm.custom_prompt_dict,
                model_response=model_response,
                print_verbose=print_verbose,
                optional_params=optional_params,
                llm_params=llm_params,
            ...
            model_response = aleph_alpha.completion(
                ...
                logger_fn=logger_fn,
                encoding=encoding,
                default_max_tokens_to_sample=llm.max_tokens,
            ...
            huggingface_key = (
                api_key
                or llm.huggingface_key
                or os.environ.get("HF_TOKEN")
                or os.environ.get("HUGGINGFACE_API_KEY")
                or llm.api_key
            )
            hf_headers = headers or llm.headers
            ...
            api_key = (
                api_key
                or llm.api_key  # for databricks we check in get_llm_provider and pass in the api key from there
                or llm.databricks_key
            ...
            _headers = headers or llm.headers
            if _headers:
            ...
            ## Load Config
            config = llm.OpenrouterConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            or (model in llm.together_ai_models)
        ):
            """
            ...
            response = vertex_chat_completion.completion(  # type: ignore
                ...
                llm_params=llm_params,  # type: ignore
                logger_fn=logger_fn,
                encoding=encoding,
            ...
            if (
                ...
                llm_params.get("base_model") is not None
                and "gemini" in llm_params["base_model"]
            ):
            ...
            api_key = api_key or llm.api_key or get_secret("CODESTRAL_API_KEY")

            text_completion_model_response = llm.TextCompletionResponse(
                stream=stream
            )
            ...
            api_base = llm.api_base or api_base
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
                messages=messages,
            ...
            or llm.api_base == "https://app.baseten.co"
        ):
            custom_llm_provider = "baseten"
            baseten_key = (
                api_key
            ...
            if inspect.isgenerator(model_response) or (
                ...
                return response
            response = model_response
        elif custom_llm_provider == "petals" or model in llm.petals_models:
            ...
            if stream is True:  ## [BETA]
                ...
                return response
            response = model_response
        elif custom_llm_provider == "snowflake" or model in llm.snowflake_models:
            try:
                client = (
            ...
            url = llm.api_base or api_base or ""
            if url is None or url == "":
                raise ValueError(
                    "api_base not set. Set api_base or llm.api_base for custom endpoints"
                )

            """
            assume input to custom LLM api bases follow this format:
            resp = llm.module_level_client.post(
                api_base,
                json={
            ...
            custom_llm_provider in llm._custom_providers
        ):  # Assume custom LLM provider
            # Get the Custom Handler
            custom_handler: Optional[CustomLLM] = None
            for item in llm.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]

            if custom_handler is None:
                raise LLMUnknownProvider(
...
def completion_with_retries(*args, **kwargs):
    """
    Executes a llm.completion() with 3 retries
    """
    try:
        import tenacity
    except Exception as e:
...
def embedding(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    - llm_call_id: The call ID for llm logging.
    - llm_logging_obj: The llm logging object.
    ...
    max_retries = kwargs.get("max_retries", None)
    llm_logging_obj: LLMLoggingObj = kwargs.get("llm_logging_obj")  # type: ignore
    ...
    default_params = openai_params + llm_params
    non_default_params = {
    ...
    logging: Logging = llm_logging_obj  # type: ignore
    logging.update_environment_variables(
        model=model,
        user=user,
        optional_params=optional_params,
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            api_base = api_base or llm.api_base or get_secret_str("AZURE_API_BASE")
            ...
            model in llm.open_ai_embedding_models
            or custom_llm_provider == "openai"
            or custom_llm_provider == "together_ai"
            or custom_llm_provider == "nvidia_nim"
            or custom_llm_provider == "llm_proxy"
        ):
            api_base = (
                api_base
                or llm.api_base
            ...
            api_base = api_base or llm.api_base or get_secret("DATABRICKS_API_BASE")  # type: ignore
            ...
            api_base = (
                api_base or llm.api_base or get_secret_str("OPENAI_LIKE_API_BASE")
            )
            ...
            gemini_api_key = (
                api_key or get_secret_str("GEMINI_API_KEY") or llm.api_key
            )
            ...
            api_key = api_key or llm.api_key or get_secret_str("MISTRAL_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = (
                api_key or llm.api_key or get_secret_str("FIREWORKS_AI_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
    ...
    *args, **kwargs
    ...
    try:
        ...
        if (
            ...
            text_completion_response = llm.utils.LLMResponseObjectHandler.convert_chat_to_text_completion(
...
def text_completion(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    # default values for all optional params are none, llm only passes them to the llm when they are set to non None values
    ...
    if isinstance(prompt, list) and len(prompt) > 0 and isinstance(prompt[0], str):
        ...
        raise Exception(
            f"Unmapped prompt format. Your prompt is neither a list of strings nor a string. prompt={prompt}. File an issue - https://github.com/hanzoai/llm/issues"
        )
    ...
    if _model is not None and (
        ...
        if _model not in llm.open_ai_chat_completion_models:
    ...
    text_completion_response = (
        llm.utils.LLMResponseObjectHandler.convert_chat_to_text_completion(
            response=response,
            text_completion_response=text_completion_response,
        )
    )
...
async def aadapter_completion(
    *, adapter_id: str, **kwargs
    ...
    try:
        ...
        for item in llm.adapters:
            if item["id"] == adapter_id:
                translation_obj = item["adapter"]

        if translation_obj is None:
            raise ValueError(
                "No matching adapter given. Received 'adapter_id'={}, llm.adapters={}".format(
                    adapter_id, llm.adapters
                )
            )
...
def image_generation(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    try:
        ...
        if extra_headers is not None:
            headers.update(extra_headers)
        model_response: ImageResponse = llm.utils.ImageResponse()
        ...
        logging: Logging = llm_logging_obj
        logging.update_environment_variables(
            model=model,
            user=user,
            optional_params=optional_params,
...
def image_variation(
    ...
    **kwargs,
    ...
    client = kwargs.get("client", None)
    # get logging object
    llm_logging_obj = cast(LLMLoggingObj, kwargs.get("llm_logging_obj"))
    ...
    model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
        model=model,
        custom_llm_provider=llm_params.get("custom_llm_provider", None),
        api_base=llm_params.get("api_base", None),
        api_key=llm_params.get("api_key", None),
    )
    ...
    if image_variation_provider == LLM_IMAGE_VARIATION_PROVIDERS.OPENAI:
        ...
        response = openai_image_variations.image_variations(
            ...
            model=model,
            image=image,
            timeout=llm_params.get("timeout", None),
            custom_llm_provider=custom_llm_provider,
            logging_obj=llm_logging_obj,
            optional_params={},
            llm_params=llm_params,
...
def transcription(
    ...
    **kwargs,
    ...
    if llm_logging_obj:
        llm_logging_obj.model_call_details["client"] = str(client)

    if max_retries is None:
        max_retries = openai.DEFAULT_MAX_RETRIES

    model_response = llm.utils.TranscriptionResponse()
    ...
    if custom_llm_provider == "azure":
        ...
        api_version = (
            api_version or llm.api_version or get_secret_str("AZURE_API_VERSION")
        )
        ...
        # set API KEY
        api_key = api_key or llm.api_key or llm.openai_key or get_secret("OPENAI_API_KEY")  # type: ignore
...
def speech(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    if max_retries is None:
        max_retries = llm.num_retries or openai.DEFAULT_MAX_RETRIES
    llm_params_dict = get_llm_params(**kwargs)
    logging_obj = kwargs.get("llm_logging_obj", None)
    ...
    if (
        ...
        api_base = api_base or llm.api_base or get_secret("AZURE_API_BASE")  # type: ignore

        api_version = api_version or llm.api_version or get_secret("AZURE_API_VERSION")  # type: ignore
        ...
        generic_optional_params = GenericLLMParams(**kwargs)
    ...
    if response is None:
        raise Exception(
            "Unable to map the custom llm provider={} to a known provider={}.".format(
                custom_llm_provider, llm.provider_list
            )
        )
    return response
...
async def ahealth_check_wildcard_models(
    ...
    if len(cheapest_models) == 0:
        raise Exception(
            f"Unable to health check wildcard model for provider {custom_llm_provider}. Add a model on your config.yaml or contribute here - https://github.com/hanzoai/llm/blob/main/model_prices_and_context_window.json"
        )
...
async def ahealth_check(
    ...
    try:
        ...
        if model in llm.model_cost and mode is None:
            mode = llm.model_cost[model].get("mode")
        ...
        model_params["llm_logging_obj"] = llm_logging_obj

        mode_handlers = {
            "chat": lambda: llm.acompletion(
                **model_params,
            ),
            "completion": lambda: llm.atext_completion(
                **_filter_model_params(model_params),
                prompt=prompt or "test",
            ),
            "embedding": lambda: llm.aembedding(
                **_filter_model_params(model_params),
                input=input or ["test"],
            ),
            "audio_speech": lambda: llm.aspeech(
                **_filter_model_params(model_params),
                input=prompt or "test",
                voice="alloy",
            ),
            "audio_transcription": lambda: llm.atranscription(
                **_filter_model_params(model_params),
...
####### HELPER FUNCTIONS ################
## Set verbose to true -> ```llm.set_verbose = True```
...
def config_completion(**kwargs):
    if llm.config_path is not None:
        config_args = read_config_args(llm.config_path)
        # overwrite any args passed in with config args
        return completion(**kwargs, **config_args)
    else:
        raise ValueError(
            "No config path set, please set a config path using `llm.config_path = 'path/to/config.json'`"
        )
...
def stream_chunk_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if isinstance(
            chunks[0]["choices"][0], llm.utils.TextChoices
        ):  # route to the text completion logic
        ...
        verbose_logger.exception(
            "llm.main.py::stream_chunk_builder() - Exception occurred - {}".format(
                str(e)
            )
        )
        raise llm.APIError(
