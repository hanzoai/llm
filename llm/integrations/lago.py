...
class LagoLogger(CustomLogger):
    ...
    def _common_logic(self, kwargs: dict, response_obj) -> dict:
        response_obj.get("id", kwargs.get("llm_call_id"))
        ...
        if (
            isinstance(response_obj, llm.ModelResponse)
        ...
        user_id = llm_params["metadata"].get("user_api_key_user_id", None)
        team_id = llm_params["metadata"].get("user_api_key_team_id", None)
        llm_params["metadata"].get("user_api_key_org_id", None)
