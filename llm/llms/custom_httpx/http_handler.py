...
class AsyncHTTPHandler:
    ...
    def create_client(
        ...
        # /path/to/client.pem
        cert = os.getenv("SSL_CERTIFICATE", llm.ssl_certificate)
    ...
    async def post(
        ...
        timeout: Optional[Union[float, httpx.Timeout]] = None,
        stream: bool = False,
        logging_obj: Optional[HanzoLoggingObject] = None,
    ):
    ...
    def _create_async_transport(self) -> Optional[AsyncHTTPTransport]:
        """
        Create an async transport with IPv4 only if llm.force_ipv4 is True.
...
def get_async_httpx_client(
    ...
    _cached_client = llm.in_memory_llm_clients_cache.get_cache(_cache_key_name)
    if _cached_client:
