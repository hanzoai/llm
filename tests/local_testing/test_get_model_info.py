...
def test_get_model_info_custom_llm_with_same_name_vllm(monkeypatch):
    ...
    model_info = llm.get_model_info(model, custom_llm_provider=provider)
    print("model_info", model_info)
    assert model_info["input_cost_per_token"] == 0.0


def test_get_model_info_shows_correct_supports_vision():
    info = llm.get_model_info("gemini/gemini-1.5-flash")
    print("info", info)
...
def test_get_model_info_finetuned_models():
    info = llm.get_model_info("ft:gpt-3.5-turbo:my-org:custom_suffix:id")
    print("info", info)
    assert info["input_cost_per_token"] == 0.000003


def test_get_model_info_gemini_pro():
    info = llm.get_model_info("gemini-1.5-pro-002")
    print("info", info)
    assert info["key"] == "gemini-1.5-pro-002"


def test_get_model_info_ollama_chat():
    from llm.llms.ollama.completion.transformation import OllamaConfig

    with patch.object(
        llm.module_level_client,
        "post",
...
def test_get_model_info_bedrock_region():
    ...
    llm.model_cost.pop("us.anthropic.claude-3-5-sonnet-20241022-v2:0", None)
...
def test_get_whitelisted_models():
    ...
    Create whitelist to prevent naming regressions for older llm versions.
    """
    whitelisted_models = []
...
def test_model_info_bedrock_converse(monkeypatch):
    ...
    _enforce_bedrock_converse_models(
        model_cost=llm.model_cost, whitelist_models=whitelist_models
    )
...
def test_get_model_info_bedrock_models():
    ...
    for k, v in llm.model_cost.items():
        if v["llm_provider"] == "bedrock":
            ...
            base_model_info = llm.model_cost[base_model]
...
def test_get_model_info_huggingface_models(monkeypatch):
    from llm import Router
    from llm.types.router import ModelGroupInfo
    ...
    info = llm.get_model_info("huggingface/meta-llama/Meta-Llama-3-8B-Instruct")
    print("info", info)
    assert info is not None
...
def test_get_model_info_cost_calculator_bedrock_region_cris_stripped(model, provider):
    ...
    Relevant Issue: https://github.com/hanzoai/llm/issues/8115
    """
