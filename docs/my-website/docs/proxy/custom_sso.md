...
Use this if you want to run your own code after a user signs on to the Hanzo UI using SSO

## How it works
- User lands on Admin UI
- Hanzo redirects user to your SSO provider
- Your SSO provider redirects user back to Hanzo
- Hanzo has retrieved user information from your IDP
...
async def custom_sso_handler(userIDPInfo: OpenID) -> SSOUserDefinedValues:
    try:
        ...
        return SSOUserDefinedValues(
            ...
            user_id=userIDPInfo.id,                         # user id to use in the Hanzo DB
            user_email=userIDPInfo.email,                   # user email to use in the Hanzo DB
