...
✨ Prometheus metrics is on Hanzo Enterprise
...
Hanzo Exposes a `/metrics` endpoint for Prometheus to Poll

## Quick Start

If you're using the Hanzo CLI with `llm --config proxy_config.yaml` then you need to `pip install prometheus_client==0.20.0`. **This is already pre-installed on the llm Docker image**
...
| `llm_remaining_team_budget_metric`             | Remaining Budget for Team (A team created on Hanzo) Labels: `"team_id", "team_alias"`|
...
| `llm_remaining_api_key_budget_metric`                | Remaining Budget for API Key (A key Created on Hanzo) Labels: `"hashed_api_key", "api_key_alias"`|
...
| Metric Name          | Description                          |
|----------------------|--------------------------------------|
| `llm_remaining_api_key_requests_for_model`                | Remaining Requests for a Hanzo virtual API key, only if a model-specific rate limit (rpm) has been set for that virtual key. Labels: `"hashed_api_key", "api_key_alias", "model"`|
| `llm_remaining_api_key_tokens_for_model`                | Remaining Tokens for a Hanzo virtual API key, only if a model-specific token limit (tpm) has been set for that virtual key. Labels: `"hashed_api_key", "api_key_alias", "model"`|
...
Use this to track overall Hanzo Proxy usage.
...
| Label | Description |
|-------|-------------|
| llm_model_name | The name of the LLM model used by Hanzo |
| requested_model | The model sent in the request |
| model_id | The model_id of the deployment. Autogenerated by Hanzo, each deployment has a unique model_id |
...
| Metric Name          | Description                          |
|----------------------|--------------------------------------|
| `llm_deployment_cooled_down`             | Number of times a deployment has been cooled down by Hanzo load balancing logic. Labels: `"llm_model_name", "model_id", "api_base", "api_provider", "exception_status"` |
...
| Metric Name          | Description                          |
|----------------------|--------------------------------------|
| `llm_request_total_latency_metric`             | Total latency (seconds) for a request to Hanzo Proxy Server - tracked for labels "end_user", "hashed_api_key", "api_key_alias", "requested_model", "team", "team_alias", "user", "model" |
| `llm_overhead_latency_metric`             | Latency overhead (seconds) added by Hanzo processing - tracked for labels "end_user", "hashed_api_key", "api_key_alias", "requested_model", "team", "team_alias", "user", "model" |
...
## **🔥 Hanzo Maintained Grafana Dashboards **

Link to Grafana Dashboards maintained by Hanzo

https://github.com/BerriAI/llm/tree/main/cookbook/llm_proxy_server/grafana_dashboard

Here is a screenshot of the metrics you can monitor with the Hanzo Grafana Dashboard
