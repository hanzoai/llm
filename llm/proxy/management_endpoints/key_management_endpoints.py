...
def _is_team_key(data: Union[GenerateKeyRequest, Hanzo_VerificationToken]):
    return data.team_id is not None


def _get_user_in_team(
    team_table: Hanzo_TeamTableCachedObj, user_id: Optional[str]
) -> Optional[Member]:
    if user_id is None:
        return None
...
def _team_key_generation_team_member_check(
    ...
    team_table: Hanzo_TeamTableCachedObj,
...
def key_generation_check(
    team_table: Optional[Hanzo_TeamTableCachedObj],
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
) -> bool:
    """
...
async def generate_key_fn(  # noqa: PLR0915
    ...
    try:
        ...
        team_table: Optional[Hanzo_TeamTableCachedObj] = None
        ...
        # TODO: @ishaan-jaff: Migrate all budget tracking to use Hanzo_BudgetTable
        _budget_id = data.budget_id
        if prisma_client is not None and data.soft_budget is not None:
            # create the Budget Row for the Hanzo Verification Token
            budget_row = Hanzo_BudgetTable(
        ...
        for field in Hanzo_ManagementEndpoint_MetadataFields_Premium:
...
def prepare_metadata_fields(
    ...
    """
    Check Hanzo_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    try:
        for k, v in data_json.items():
            if k in Hanzo_ManagementEndpoint_MetadataFields:
        ...
        if llm_router._is_model_access_group_for_wildcard_route(
            ...
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Setting a model access group on a wildcard model is only available for Hanzo Enterprise users.{}".format(
...
async def generate_key_helper_fn(  # noqa: PLR0915
    ...
    budget_id: Optional[float] = None,  # budget id <-> Hanzo_BudgetTable
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if (
                ...
                raise ValueError(
                    "get_spend_routes permission is only available for Hanzo Enterprise users"
                )
        ...
        if prisma_client is not None:
            ...
            if user_id == llm_proxy_budget_name or (
                ...
                # the Hanzo_VerificationToken table will increase in size if we don't do this check
                return user_data
...
async def _team_key_deletion_check(
    user_api_key_dict: UserAPIKeyAuth,
    key_info: Hanzo_VerificationToken,
...
async def delete_verification_tokens(
    ...
    user_api_key_cache: DualCache,
    user_api_key_dict: UserAPIKeyAuth,
) -> Tuple[Optional[Dict], List[Hanzo_VerificationToken]]:
    """
    ...
    try:
        if prisma_client:
            ...
            _keys_being_deleted: List[Hanzo_VerificationToken] = (
            ...
            # else
                ...
                for key in _keys_being_deleted:
                    ...
                    async def _delete_key(key: Hanzo_VerificationToken):
...
async def validate_key_list_check(
    ...
    complete_user_info = Hanzo_UserTable(**complete_user_info_db_obj.model_dump())
...
async def get_admin_team_ids(
    complete_user_info: Optional[Hanzo_UserTable],
    user_api_key_dict: UserAPIKeyAuth,
    prisma_client: PrismaClient,
) -> List[str]:
    """
    ...
    teams_pydantic_obj = [Hanzo_TeamTable(**team.model_dump()) for team in teams]
...
) -> Optional[Hanzo_VerificationToken]:
    """
