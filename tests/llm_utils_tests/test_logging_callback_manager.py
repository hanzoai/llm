...
def test_add_string_callback():
    ...
    # Add string callback
    manager.add_llm_callback(test_callback)
...
def test_duplicate_langfuse_logger_test():
    ...
    for _ in range(10):
        ...
        manager.add_llm_success_callback(langfuse_logger)
...
def test_duplicate_multiple_loggers_test():
    ...
    for _ in range(10):
        ...
        manager.add_llm_success_callback(langfuse_logger)
        manager.add_llm_success_callback(otel_logger)
    print("llm.success_callback: ", llm.success_callback)
...
def test_add_function_callback():
    ...
    manager.add_llm_callback(test_func)
...
def test_add_custom_logger(mock_custom_logger):
    ...
    # Add custom logger
    manager.add_llm_callback(mock_custom_logger)
...
def test_add_multiple_callback_types(mock_custom_logger):
    ...
    manager.add_llm_callback(string_callback)
    manager.add_llm_callback(test_func)
    manager.add_llm_callback(mock_custom_logger)
...
def test_success_failure_callbacks():
    ...
    # Add callbacks
    manager.add_llm_success_callback(success_callback)
    manager.add_llm_failure_callback(failure_callback)
...
def test_async_callbacks():
    ...
    # Add async callbacks
    manager.add_llm_async_success_callback(async_success)
    manager.add_llm_async_failure_callback(async_failure)
...
def test_remove_callback_from_list_by_object():
    ...
    def TestObject():
        def __init__(self):
            manager.add_llm_callback(self.callback)
            manager.add_llm_success_callback(self.callback)
            manager.add_llm_failure_callback(self.callback)
            manager.add_llm_async_success_callback(self.callback)
            manager.add_llm_async_failure_callback(self.callback)
...
def test_reset_callbacks(callback_manager):
    ...
    callback_manager.add_llm_callback("test")
    callback_manager.add_llm_success_callback("success")
    callback_manager.add_llm_failure_callback("failure")
    callback_manager.add_llm_async_success_callback("async_success")
    callback_manager.add_llm_async_failure_callback("async_failure")
