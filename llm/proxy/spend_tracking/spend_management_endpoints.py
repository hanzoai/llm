...
from llm.proxy._types import ProviderBudgetResponse, ProviderBudgetResponseObject
from llm.proxy.auth.user_api_key_auth import user_api_key_auth
from llm.proxy.spend_tracking.spend_tracking_utils import (
    get_spend_by_team_and_customer,
)
from llm.proxy.utils import handle_exception_on_proxy

if TYPE_CHECKING:
    from llm.proxy.proxy_server import PrismaClient
else:
    PrismaClient = Any
...
@router.get(
    ...
    dependencies=[Depends(user_api_key_auth)],
    responses={
        200: {"model": List[Hanzo_SpendLogs]},
    },
)
...
async def get_global_activity(
    ...
    try:
        ...
        if (
            user_api_key_dict.user_role == LLMUserRoles.INTERNAL_USER
            or user_api_key_dict.user_role == LLMUserRoles.INTERNAL_USER_VIEW_ONLY
        ):
...
async def get_global_spend_provider(
    ...
    from llm.proxy.proxy_server import llm_router, prisma_client
        ...
        for row in db_response:
            ...
            if llm_router is not None:
                ...
                if _deployment is not None:
                    try:
                        _, _provider, _, _ = llm.get_llm_provider(
                            model=_deployment.llm_params.model,
                            custom_llm_provider=_deployment.llm_params.custom_llm_provider,
                            api_base=_deployment.llm_params.api_base,
                            llm_params=_deployment.llm_params,
                        )
...
async def global_view_spend_tags(
    ...
    """
    Hanzo Enterprise - View Spend Per Request Tag. Used by Hanzo UI
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "Hanzo_SpendLogs" s
        LEFT JOIN
            "Hanzo_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    try:
        ...
        from llm.cost_calculator import CostPerToken
        ...
        if request.model is not None:
            ...
            1. user passes model, and model is defined on llm config.yaml or in DB. use info on config or in DB in this case
            2. user passes model, and model is not defined on llm config.yaml or in DB. Pass model as is to llm.completion_cost
            3. user passes completion_response
            
            """
            if _model_in_llm_router is not None:
                _llm_params = _model_in_llm_router.get("llm_params")
                _llm_model_name = _llm_params.get("model")
                input_cost_per_token = _llm_params.get("input_cost_per_token")
                output_cost_per_token = _llm_params.get("output_cost_per_token")
                if (
            ...
            _completion_response = llm.ModelResponse(**request.completion_response)
...
async def ui_view_spend_logs(  # noqa: PLR0915
    ...
    Returns:
        {
            "data": List[Hanzo_SpendLogs],  # Paginated spend logs
    ...
    try:
        ...
        total_records = await prisma_client.db.llm_spendlogs.count(
            where=where_conditions,
        )

        # Get paginated data
        data = await prisma_client.db.llm_spendlogs.find_many(
            where=where_conditions,
            order={
                "startTime": "desc",
            },
...
async def ui_view_request_response_for_request_id(
    ...
    """
    custom_loggers = (
        llm.logging_callback_manager.get_active_additional_logging_utils_from_custom_logger()
    )
...
-H "Authorization: Bearer sk-1234"
    ...
    try:
        ...
        if (
            ...
            # SQL query
            response = await prisma_client.db.llm_spendlogs.group_by(
...
async def global_spend_reset():
    ...
    1. Hanzo_SpendLogs will maintain the logs on spend, no data gets deleted from there
    ...
    await prisma_client.db.llm_verificationtoken.update_many(
        data={"spend": 0.0}, where={}
    )
    await prisma_client.db.llm_teamtable.update_many(data={"spend": 0.0}, where={})
...
async def global_spend_refresh():
    ...
    if view_exists:
        ...
        """
        try:
            from llm.proxy._types import CommonProxyErrors
            from llm.proxy.proxy_server import proxy_logging_obj
...
async def global_spend_logs(
    ...
    from llm.integrations.prometheus_helpers.prometheus_api import (
...
async def global_spend():
    ...
    try:
        ...
        return {"spend": total_spend, "max_budget": llm.max_budget}
    except Exception as e:
        error_trace = traceback.format_exc()
        error_str = str(e) + "\n" + error_trace
        if isinstance(e, HTTPException):
...
async def global_spend_key_internal_user(
    ...
    sql_query = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "Hanzo_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query = """
    SELECT DISTINCT end_user FROM "Hanzo_SpendLogs"
    """
...
async def provider_budgets() -> ProviderBudgetResponse:
    ...
    try:
        ...
        if provider_budget_config is None:
            raise ValueError(
                "No provider budget config found. Please set a provider budget config in the router settings. https://docs.llm.ai/docs/proxy/provider_budget_routing"
            )
