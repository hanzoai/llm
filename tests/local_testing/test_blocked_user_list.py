...
from llm.proxy.enterprise.enterprise_hooks.blocked_user_list import (
...
from llm.proxy.management_endpoints.customer_endpoints import block_user
from llm.proxy.spend_tracking.spend_management_endpoints import (
    spend_key_fn,
    spend_user_fn,
    view_spend_logs,
)
from llm.proxy.utils import PrismaClient, ProxyLogging, hash_token
...
async def test_block_user_check(prisma_client):
    """
    - Set a blocked user as a llm module value
    ...
    llm.blocked_user_list = ["user_id_1"]

    blocked_user_obj = _ENTERPRISE_BlockedUserList(
        prisma_client=llm.proxy.proxy_server.prisma_client
    )
