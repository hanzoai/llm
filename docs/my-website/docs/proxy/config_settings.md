...
| langfuse_default_tags | array of strings | Default tags for Langfuse Logging. Use this if you want to control which LLM-specific fields are logged as tags by the LLM proxy. By default LLM Proxy logs no LLM-specific fields as tags. [Further docs](./logging#llm-specific-tags-on-langfuse---cache_hit-cache_key) |
...
| allow_requests_on_db_unavailable | boolean | If true, allows requests to succeed even if DB is unreachable. **Only use this if running LLM in your VPC** This will allow requests to work even when LLM cannot connect to the DB to verify a Virtual Key |
...
| llm_key_header_name | str | If set, allows passing LLM keys as a custom header. [Doc on custom headers](./virtual_keys.md#custom-headers) |
...
| redis_host | string | The host address for the Redis server. **Only set this if you have multiple instances of LLM Proxy and want current tpm/rpm tracking to be shared across them** |
| redis_password | string | The password for the Redis server. **Only set this if you have multiple instances of LLM Proxy and want current tpm/rpm tracking to be shared across them** |
| redis_port | string | The port number for the Redis server. **Only set this if you have multiple instances of LLM Proxy and want current tpm/rpm tracking to be shared across them**|
...
| GCS_PUBSUB_TOPIC_ID | PubSub Topic ID to send LLM SpendLogs to.
| GCS_PUBSUB_PROJECT_ID | PubSub Project ID to send LLM SpendLogs to.
