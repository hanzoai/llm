...
from llm.types.llms.anthropic import ANTHROPIC_API_HEADERS
...
if TYPE_CHECKING:
    from llm.proxy.proxy_server import ProxyConfig as _ProxyConfig
...
def _get_metadata_variable_name(request: Request) -> str:
    ...
    For all /thread or /assistant endpoints we need to call this "llm_metadata"
...
def convert_key_logging_metadata_to_callback(
    ...
    ):  # assume 'success_and_failure' = llm.callbacks
        if team_callback_settings_obj.success_callback is None:
            team_callback_settings_obj.success_callback = []
        if team_callback_settings_obj.failure_callback is None:
            team_callback_settings_obj.failure_callback = []
    ...
    for var, value in data.callback_vars.items():
        ...
        team_callback_settings_obj.callback_vars[var] = str(
            llm.utils.get_secret(value, default_value=value) or value
        )
...
class HanzoProxyRequestSetup:
    ...
    def _get_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        ```
        """
        timeout_header = headers.get("x-llm-timeout", None)
    ...
    def add_headers_to_llm_call(
        ...
        returned_headers = HanzoProxyRequestSetup._get_forwardable_headers(headers)
    ...
    def add_llm_data_for_backend_llm_call(
        *,
        ...
        if (
            ...
            _headers = HanzoProxyRequestSetup.add_headers_to_llm_call(
                headers, user_api_key_dict
            )
            if _headers != {}:
                data["headers"] = _headers
        _organization = HanzoProxyRequestSetup.get_openai_org_id_from_headers(
    ...
    def add_key_level_controls(
        ...
        if "tags" in key_metadata and key_metadata["tags"] is not None:
            data[_metadata_variable_name]["tags"] = (
                HanzoProxyRequestSetup._merge_tags(
    ...
    def add_request_tag_to_metadata(
        ...
        if llm_router and llm_router.enable_tag_filtering is True:
            ...
            if "x-llm-tags" in headers:
                if isinstance(headers["x-llm-tags"], str):
                    _tags = headers["x-llm-tags"].split(",")
                    tags = [tag.strip() for tag in _tags]
                elif isinstance(headers["x-llm-tags"], list):
                    tags = headers["x-llm-tags"]
...
async def add_llm_data_to_request(  # noqa: PLR0915
    data: dict,
    request: Request,
    user_api_key_dict: UserAPIKeyAuth,
    proxy_config: ProxyConfig,
    ...
    """
    Adds Hanzo-specific data to the request.
    ...
    from llm.proxy.proxy_server import llm_router, premium_user
    ...
    data.update(
        HanzoProxyRequestSetup.add_llm_data_for_backend_llm_call(
            headers=_headers,
            user_api_key_dict=user_api_key_dict,
    ...
    if "tags" in team_metadata and team_metadata["tags"] is not None:
        data[_metadata_variable_name]["tags"] = HanzoProxyRequestSetup._merge_tags(
            request_tags=data[_metadata_variable_name].get("tags"),
    ...
    # Add the OTEL Parent Trace before sending it Hanzo
    data[_metadata_variable_name][
    ...
    tags = HanzoProxyRequestSetup.add_request_tag_to_metadata(
    ...
    verbose_proxy_logger.debug(
        "[PROXY] returned data from llm_pre_call_utils: %s", data
    )
    ...
    asyncio.create_task(
        service_logger_obj.async_service_success_hook(
            ...
            call_type="add_llm_data_to_request",
...
def _add_otel_traceparent_to_data(data: dict, request: Request):
    from llm.proxy.proxy_server import open_telemetry_logger
