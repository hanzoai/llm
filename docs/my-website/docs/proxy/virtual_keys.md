...
- [Deploy Hanzo Proxy with Key Management](https://docs.llm.ai/docs/proxy/deploy#deploy-with-database)
- [Dockerfile.database for Hanzo Proxy + Key Management](https://github.com/BerriAI/llm/blob/main/docker/Dockerfile.database)
...
Spend is automatically tracked for the key in the "Hanzo_VerificationTokenTable". If the key has an attached 'user_id' or 'team_id', the spend for that user is tracked in the "Hanzo_UserTable", and team in the "Hanzo_TeamTable".
...
### Pass Hanzo Key in custom header

Use this to make Hanzo proxy look for the virtual key in a custom header instead of the default `"Authorization"` header
...
client = openai.OpenAI(
    ...
    default_headers={
        ...
        "X-LLM-Key": f"Bearer sk-1234"              # For Hanzo Proxy
    }
)
```
</TabItem>
...
[Follow this doc to set budgets, rate limiters per virtual key with Hanzo](users)
