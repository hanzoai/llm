...
from llm.proxy._types import ProxyException, UserAPIKeyAuth
...
class ProxyBaseLLMRequestProcessing:
    ...
    def get_custom_headers(
        ...
        **kwargs,
        ...
        headers = {
            "x-llm-call-id": call_id,
            "x-llm-model-id": model_id,
            "x-llm-cache-key": cache_key,
            "x-llm-model-api-base": api_base,
            "x-llm-version": version,
            "x-llm-model-region": model_region,
            "x-llm-response-cost": str(response_cost),
            "x-llm-key-tpm-limit": str(user_api_key_dict.tpm_limit),
            "x-llm-key-rpm-limit": str(user_api_key_dict.rpm_limit),
            "x-llm-key-max-budget": str(user_api_key_dict.max_budget),
            "x-llm-key-spend": str(user_api_key_dict.spend),
            "x-llm-response-duration-ms": str(
                hidden_params.get("_response_ms", None)
            ),
            "x-llm-overhead-duration-ms": str(
                hidden_params.get("llm_overhead_time_ms", None)
            ),
            "x-llm-fastest_response_batch_completion": (
                str(fastest_response_batch_completion)
                if fastest_response_batch_completion is not None
                else None
            ),
            "x-llm-timeout": str(timeout) if timeout is not None else None,
    ...
    async def base_process_llm_request(
        ...
        self.data = await add_llm_data_to_request(
            data=self.data,
            request=request,
        ...
        if (
            ...
            and self.data["model"] in llm.model_alias_map
        ):
            self.data["model"] = llm.model_alias_map[self.data["model"]]
        ...
        self.data["llm_call_id"] = request.headers.get(
            "x-llm-call-id", str(uuid.uuid4())
        )
        logging_obj, self.data = llm.utils.function_setup(
        ...
        self.data["llm_logging_obj"] = logging_obj
        ...
        asyncio.create_task(
            proxy_logging_obj.update_request_status(
                llm_call_id=self.data.get("llm_call_id", ""), status="success"
            )
        )
        if (
            ...
            custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
                user_api_key_dict=user_api_key_dict,
                call_id=logging_obj.llm_call_id,
                model_id=model_id,
                cache_key=cache_key,
                api_base=api_base,
                version=version,
    ...
    async def _handle_llm_api_exception(
        ...
        verbose_proxy_logger.exception(
            f"llm.proxy.proxy_server._handle_llm_api_exception(): Exception occured - {str(e)}"
        )
        ...
        _llm_logging_obj: Optional[HanzoLoggingObj] = self.data.get(
            "llm_logging_obj", None
        )
        custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=(
                _llm_logging_obj.llm_call_id if _llm_logging_obj else None
            ),
            version=version,
            response_cost=0,
            model_region=getattr(user_api_key_dict, "allowed_model_region", ""),
