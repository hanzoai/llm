...
from llm.llm_core_utils.core_helpers import process_response_headers
from llm.llm_core_utils.llm_response_utils.get_api_base import get_api_base
from llm.llm_core_utils.logging_utils import HanzoLoggingObject
...
class ResponseMetadata:
    """
    Handles setting and managing `_hidden_params`, `response_time_ms`, and `llm_overhead_time_ms` for Hanzo responses
    """
    ...
    def set_hidden_params(
        self, logging_obj: HanzoLoggingObject, model: Optional[str], kwargs: dict
    ) -> None:
        """Set hidden parameters on the response"""
        new_params = {
            "llm_call_id": getattr(logging_obj, "llm_call_id", None),
