...
from llm.llms.azure.fine_tuning.handler import AzureOpenAIFineTuningAPI
from llm.llms.openai.fine_tuning.handler import OpenAIFineTuningAPI
from llm.llms.vertex_ai.fine_tuning.handler import VertexFineTuningAPI
...
from llm.utils import client, supports_httpx_timeout
    ...
    try:
        ...
        if custom_llm_provider == "openai":
            ...
            response = openai_fine_tuning_apis_instance.create_fine_tuning_job(
                ...
                client=kwargs.get(
                    "client", None
                ),  # note, when we add this to `GenericHanzoParams` it impacts a lot of other tests + linting
            )
        # Azure OpenAI
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            api_base = optional_params.api_base or llm.api_base or get_secret("AZURE_API_BASE")  # type: ignore
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            raise llm.exceptions.BadRequestError(
                message="Hanzo doesn't support {} for 'retrieve_fine_tuning_job'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="retrieve_fine_tuning_job", url="https://github.com/BerriAI/llm"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
