...
from llm.integrations.custom_logger import CustomLogger, Span
from llm.llm_core_utils.duration_parser import duration_in_seconds
from llm.router_strategy.tag_based_routing import _get_tags_from_request_kwargs
from llm.router_utils.cooldown_callbacks import (
    _get_prometheus_logger_from_callbacks,
)
from llm.types.llms.openai import AllMessageValues
from llm.types.router import DeploymentTypedDict, Hanzo_Params, RouterErrors
from llm.types.utils import BudgetConfig
from llm.types.utils import BudgetConfig as GenericBudgetInfo
from llm.types.utils import GenericBudgetConfigType, StandardLoggingPayload
...
class RouterBudgetLimiting(CustomLogger):
    def __init__(
        ...
        # Add self to llm callbacks if it's a list
        if isinstance(llm.callbacks, list):
            llm.logging_callback_manager.add_llm_callback(self)  # type: ignore
    ...
    def _filter_out_deployments_above_budget(
        ...
        for deployment in healthy_deployments:
            ...
            if self.deployment_budget_config and is_within_budget:
                ...
                _llm_params = deployment.get("llm_params") or {}
                ...
                if model_id in deployment_configs:
                    ...
                    if config.max_budget and current_spend >= config.max_budget:
                        debug_msg = f"Exceeded budget for deployment model_name: {_model_name}, llm_params.model: {_llm_model_name}, model_id: {model_id}: {current_spend} >= {config.budget_duration}"
    ...
    async def _increment_spend_in_current_window(
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of Hanzo)
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        custom_llm_provider: str = kwargs.get("llm_params", {}).get(
    ...
    def _get_llm_provider_for_deployment(self, deployment: Dict) -> Optional[str]:
        try:
            _llm_params: Hanzo_Params = Hanzo_Params(
                **deployment.get("llm_params", {"model": ""})
            )
            _, custom_llm_provider, _, _ = llm.get_llm_provider(
                model=_llm_params.model,
    ...
    def should_init_router_budget_limiter(
        ...
        if llm.tag_budget_config is not None:
            return True
        ...
        for _model in model_list:
            _llm_params = _model.get("llm_params", {})
            if (
                _llm_params.get("max_budget")
                or _llm_params.get("budget_duration") is not None
            ):
                return True
        return False
    ...
    def _init_deployment_budgets(
        ...
        for _model in model_list:
            ...
            _max_budget = _llm_params.get("max_budget")
            _budget_duration = _llm_params.get("budget_duration")
    ...
    def _init_tag_budgets(self):
        if llm.tag_budget_config is None:
            return
        ...
        for _tag, _tag_budget_config in llm.tag_budget_config.items():
