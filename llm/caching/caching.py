...
#### LLM.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: Optional[LLMCacheType] = LLMCacheType.LOCAL,
        mode: Optional[
            CacheMode
        ...
        **kwargs,
        ...
        elif type == LLMCacheType.REDIS_SEMANTIC:
        ...
        elif type == LLMCacheType.QDRANT_SEMANTIC:
        ...
        elif type == LLMCacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == LLMCacheType.S3:
            self.cache = S3Cache(
        ...
        if self.type == LLMCacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == LLMCacheType.REDIS
            or self.type == LLMCacheType.REDIS_SEMANTIC
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception(f"LLM Cache: Excepton add_cache: {str(e)}")
    ...
    def _supports_async(self) -> bool:
        ...
        """
        if self.type and self.type == LLMCacheType.S3:
            return False
        return True
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("LLM: Enabling Cache")
    if "cache" not in llm.input_callback:
        llm.input_callback.append("cache")
    if "cache" not in llm.success_callback:
        llm.logging_callback_manager.add_llm_success_callback("cache")
    ...
    print_verbose(f"LLM: Cache enabled, llm.cache={llm.cache}")
    print_verbose(f"LLM Cache: {vars(llm.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    print_verbose("LLM: Updating Cache")
    llm.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"LLM: Cache Updated, llm.cache={llm.cache}")
    print_verbose(f"LLM Cache: {vars(llm.cache)}")


def disable_cache():
    """
    Disable the cache used by LLM.

    This function disables the cache used by the LLM module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the llm.cache attribute to None.
    ...
    print_verbose("LLM: Disabling Cache")
    ...
    llm.cache = None
    print_verbose(f"LLM: Cache disabled, llm.cache={llm.cache}")
