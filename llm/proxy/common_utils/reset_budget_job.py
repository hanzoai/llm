...
class ResetBudgetJob:
    ...
    async def reset_budget(
        ...
        if self.prisma_client is not None:
            ### RESET KEY BUDGET ###
            await self.reset_budget_for_llm_keys()

            ### RESET USER BUDGET ###
            await self.reset_budget_for_llm_users()

            ## Reset Team Budget
            await self.reset_budget_for_llm_teams()

    async def reset_budget_for_llm_keys(self):
        """
        Resets the budget for all the llm keys

        Catches Exceptions and logs them
        """
        now = datetime.utcnow()
        start_time = time.time()
        keys_to_reset: Optional[List[Hanzo_VerificationToken]] = None
        try:
            ...
            updated_keys: List[Hanzo_VerificationToken] = []
            failed_keys = []
    ...
    async def reset_budget_for_llm_users(self):
        """
        Resets the budget for all Hanzo Internal Users if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        users_to_reset: Optional[List[Hanzo_UserTable]] = None
        try:
    ...
    async def reset_budget_for_llm_teams(self):
        """
        Resets the budget for all Hanzo Internal Teams if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        teams_to_reset: Optional[List[Hanzo_TeamTable]] = None
        try:
            teams_to_reset = await self.prisma_client.get_data(
                table_name="team", query_type="find_all", reset_at=now
            )
            updated_teams: List[Hanzo_TeamTable] = []
            failed_teams = []
    ...
    async def _reset_budget_common(
        item: Union[Hanzo_TeamTable, Hanzo_UserTable, Hanzo_VerificationToken],
        current_time: datetime,
        item_type: str,
    ) -> Union[Hanzo_TeamTable, Hanzo_UserTable, Hanzo_VerificationToken]:
        """
    ...
    async def _reset_budget_for_team(
        team: Hanzo_TeamTable, current_time: datetime
    ) -> Optional[Hanzo_TeamTable]:
        result = await ResetBudgetJob._reset_budget_common(team, current_time, "team")
        return result if isinstance(result, Hanzo_TeamTable) else None

    @staticmethod
    async def _reset_budget_for_user(
        user: Hanzo_UserTable, current_time: datetime
    ) -> Optional[Hanzo_UserTable]:
        result = await ResetBudgetJob._reset_budget_common(user, current_time, "user")
        return result if isinstance(result, Hanzo_UserTable) else None

    @staticmethod
    async def _reset_budget_for_key(
        key: Hanzo_VerificationToken, current_time: datetime
    ) -> Optional[Hanzo_VerificationToken]:
        result = await ResetBudgetJob._reset_budget_common(key, current_time, "key")
        return result if isinstance(result, Hanzo_VerificationToken) else None
