...
class HanzoDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    ...
    def _echo_llm_version():
        ...
        click.echo(f"\nHanzo: Current Version = {pkg_version}\n")

    @staticmethod
    def _run_health_check(host, port):
        print("\nHanzo: Health Testing models in config")  # noqa
    ...
    def _run_test_chat_completion(
        ...
        click.echo(
            f"\nHanzo: Making a test ChatCompletions request to your proxy. Model={request_model}"
        )
        import openai
        ...
        click.echo(f"\nHanzo: response from proxy {response}")

        print(  # noqa
            f"\n Hanzo: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}"
        )
        ...
        for chunk in stream_response:
            click.echo(f"Hanzo: streaming response from proxy {chunk}")
    ...
    def _get_default_unvicorn_init_args(
        ...
        uvicorn_args = {
            "app": "llm.proxy.proxy_server:app",
            "host": host,
            "port": port,
        }
    ...
    def _init_hypercorn_server(
        ...
        print(  # noqa
            f"\033[1;32mHanzo Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n"  # noqa
        )  # noqa
        config = Config()
        ...
        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(  # noqa
                f"\033[1;32mHanzo Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"  # noqa
            )
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()  # noqa
                print(  # noqa
                    '\033[1;34mHanzo: Test your local proxy with: "llm --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(  # noqa
                    f"\033[1;34mHanzo: Curl Command Test for your local proxy\n {curl_command} \033[0m\n"
                )
                print(  # noqa
            ...
            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with Hanzo Proxy config
        ...
        print(  # noqa
            f"\033[1;32mHanzo Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n"  # noqa
        )
        gunicorn_options = {
            "bind": f"{host}:{port}",
...
@click.option(
    "--model", "-m", default=None, help="The model name to pass to llm expects"
)
@click.option(
    "--alias",
    default=None,
    help='The alias for the model - use this to give a llm model name (e.g. "huggingface/codellama/CodeLlama-7b-Instruct-hf") a more user-friendly name ("codellama")',
)
@click.option(
    "--add_key", default=None, help="The model name to pass to llm expects"
)
...
@click.option(
    ...
    "-c",
    default=None,
    help="Path to the proxy configuration file (e.g. config.yaml). Usage `llm --config config.yaml`",
)
@click.option(
    "--max_budget",
    default=None,
...
def run_server(  # noqa: PLR0915
    ...
    if test is True:
        ...
        if iam_token_db_auth:
            from llm.proxy.auth.rds_iam_token import generate_iam_auth_token
        ...
        from llm.secret_managers.aws_secret_manager import decrypt_env_var
        ...
        if config is not None:
            ...
            if (
                ...
                and "json_logs" in llm_settings
                and llm_settings["json_logs"] is True
            ):
                import llm
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                HanzoDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            db_connection_timeout = general_settings.get(
                "database_connection_timeout",
                HanzoDatabaseConnectionPool.database_connection_pool_timeout.value,
            )
        ...
        if (
            ...
            if is_prisma_runnable:
                from llm.proxy.db.check_migration import check_prisma_schema_diff
        ...
        from llm.proxy.proxy_server import app  # noqa
