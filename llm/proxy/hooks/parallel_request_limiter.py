...
import llm
from llm import DualCache, ModelResponse
...
from llm.proxy._types import CommonProxyErrors, CurrentItemRateLimit, UserAPIKeyAuth
...
if TYPE_CHECKING:
    ...
    from llm.proxy.utils import InternalUsageCache as _InternalUsageCache
...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    async def check_key_in_limits(
        ...
        await self.internal_usage_cache.async_batch_set_cache(
            ...
            ttl=60,
            llm_parent_otel_span=user_api_key_dict.parent_otel_span,
            local_only=True,
        )
        return new_val
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        if (
            ...
            _remaining_limits_data = {
                f"llm-key-remaining-tokens-{_model}": _remaining_tokens,
                f"llm-key-remaining-requests-{_model}": _remaining_requests,
            }
    ...
    async def async_log_success_event(  # noqa: PLR0915
        ...
        llm_parent_otel_span: Union[Span, None] = _get_parent_otel_span_from_kwargs(
            kwargs=kwargs
        )
        try:
            self.print_verbose("INSIDE parallel request limiter ASYNC SUCCESS LOGGING")

            global_max_parallel_requests = kwargs["llm_params"]["metadata"].get(
                "global_max_parallel_requests", None
            )
            user_api_key = kwargs["llm_params"]["metadata"]["user_api_key"]
            user_api_key_user_id = kwargs["llm_params"]["metadata"].get(
                "user_api_key_user_id", None
            )
            user_api_key_team_id = kwargs["llm_params"]["metadata"].get(
                "user_api_key_team_id", None
            )
            user_api_key_model_max_budget = kwargs["llm_params"]["metadata"].get(
                "user_api_key_model_max_budget", None
            )
            user_api_key_end_user_id = kwargs.get("user")

            user_api_key_metadata = (
                kwargs["llm_params"]["metadata"].get("user_api_key_metadata", {})
                or {}
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            _metadata = kwargs["llm_params"].get("metadata", {}) or {}
    ...
    async def get_internal_user_object(
        ...
        It uses the `get_user_object` function from `llm.proxy.auth.auth_checks`
