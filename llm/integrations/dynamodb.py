...
class DyanmoDBLogger:
    ...
    def __init__(self):
        ...
        if llm.dynamodb_table_name is None:
            raise ValueError(
                "Hanzo Error, trying to use DynamoDB but not table name passed. Create a table and set `llm.dynamodb_table_name=<your-table>`"
            )
        self.table_name = llm.dynamodb_table_name
