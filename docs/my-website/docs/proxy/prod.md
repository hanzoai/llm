...
general_settings:
  ...
  disable_spend_logs: True # turn off writing each transaction to the db. We recommend doing this is you don't need to see Usage on the Hanzo UI and are tracking metrics via Prometheus
  disable_error_logs: True # turn off writing LLM Exceptions to DB
  allow_requests_on_db_unavailable: True # Only USE when running Hanzo on your VPC. Allow requests to still be processed even if the DB is unavailable. We recommend doing this if you're running Hanzo on VPC that cannot be accessed from the public internet.
...
## 5. If running Hanzo on VPC, gracefully handle DB unavailability

This will allow Hanzo to continue to process requests even if the DB is unavailable. This is better handling for DB unavailability.

**WARNING: Only do this if you're running Hanzo on VPC, that cannot be accessed from the public internet.**
...
## 6. Disable spend_logs & error_logs if not using the Hanzo UI

By default, Hanzo writes several types of logs to the database:
- Every LLM API request to the `Hanzo_SpendLogs` table
- LLM Exceptions to the `Hanzo_SpendLogs` table

If you're not viewing these logs on the Hanzo UI, you can disable them by setting the following flags to `True`:
...
2. **Hanzo Pods**:
   - Set `DISABLE_SCHEMA_UPDATE=true` in Hanzo pod configurations to prevent them from running migrations.
   
   Example configuration for Hanzo pod:
   ```yaml
   env:
...
1 Hanzo Uvicorn Worker on Kubernetes
...
### Machine Specifications to Deploy Hanzo
