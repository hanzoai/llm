...
from llm.types.utils import CallTypes, LoggedLLMParams
...
def print_verbose(print_statement):
    ...
    if llm.set_verbose:
        print(f"LLM Proxy: {print_statement}")  # noqa
...
def safe_deep_copy(data):
    ...
    The LLM Request has some object that can-not be pickled / deep copied

    Use this function to safely deep copy the LLM Request
    """
...
class ProxyLogging:
    ...
    async def post_call_failure_hook(
        ...
        if AlertType.llm_exceptions in self.alert_types and not isinstance(
            ...
            Related issue - https://github.com/hanzoai/llm/issues/3395
            """
...
class PrismaClient:
    ...
    async def check_view_exists(self):
        ...
        LLM_VerificationTokenView: This view is used for getting the token + team data in user_api_key_auth
        ...
        try:
            ...
            required_view = "LLM_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    await self.db.execute_raw(
                        """
                            CREATE VIEW "LLM_VerificationTokenView" AS
                            SELECT
                            v.*,
                            ...
                            FROM "LLM_VerificationToken" v
                            LEFT JOIN "LLM_TeamTable" t ON v.team_id = t.team_id;
                        """
                    )

                    verbose_proxy_logger.info(
                        "LLM_VerificationTokenView Created in DB!"
                    )
                else:
                    ...
                    if should_create_views:
                        ...
                        verbose_proxy_logger.warning(
                            "\n\n\033[93mNot all views exist in db, needed for UI 'Usage' tab. Missing={}.\nRun 'create_views.py' from https://github.com/hanzoai/llm/tree/main/db_scripts to create missing views.\033[0m\n".format(
                                missing_views
                            )
                        )
    ...
    async def get_generic_data(
        ...
        try:
            ...
            error_msg = f"LLM Prisma Client Exception get_generic_data: {str(e)}"
    ...
    async def get_data(  # noqa: PLR0915
        ...
        try:
            ...
            if (token is not None and table_name is None) or (
                ...
                if query_type == "find_unique":
                    ...
                    if expires is not None:
                        ...
                        FROM
                            "LLM_UserTable" u
                        LEFT JOIN "LLM_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = f"""
                        ...
                        FROM "LLM_VerificationToken" AS v
                        LEFT JOIN "LLM_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "LLM_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "LLM_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "LLM_BudgetTable" AS b ON v.budget_id = b.budget_id
                        WHERE v.token = '{token}'
                    """
                    ...
                    if response is not None:
                        ...
                        response = LLM_VerificationTokenView(
            ...
            prisma_query_info = f"LLM Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
        ...
        try:
            ...
            if table_name == "key":
                ...
                try:
                    ...
                    if (
                        "Foreign key constraint failed on the field: `LLM_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in LLM_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
            ...
            error_msg = f"LLM Prisma Client Exception in insert_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def update_data(  # noqa: PLR0915
        ...
        try:
            ...
            error_msg = f"LLM Prisma Client Exception - update_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            ...
            error_msg = f"LLM Prisma Client Exception - delete_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg = f"LLM Prisma Client Exception connect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg = f"LLM Prisma Client Exception disconnect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
...
def _raise_failed_update_spend_exception(
    ...
    error_msg = (
        f"[Non-Blocking]LLM Prisma Client Exception - update spend logs: {str(e)}"
    )
    error_traceback = error_msg + "\n" + traceback.format_exc()
    end_time = time.time()
    _duration = end_time - start_time
...
def _premium_user_check():
    ...
    if not premium_user:
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"This feature is only available for LLM Enterprise users. {CommonProxyErrors.not_premium_user.value}"
            },
        )
