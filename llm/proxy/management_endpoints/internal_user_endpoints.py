...
async def new_user(
    ...
    try:
        ...
        try:
            ...
            user_row_litellm_typed = LLM_UserTable(
...
async def _get_user_info_for_proxy_admin():
    ...
    sql_query = """
        SELECT 
            ...
            (SELECT json_agg(k.*) FROM "LLM_VerificationToken" k WHERE k.team_id != 'litellm-dashboard' OR k.team_id IS NULL) as keys
    """
    if prisma_client is None:
        raise Exception(
            "Database not connected. Connect a database to your proxy - https://docs.llm.ai/docs/simple_proxy#managing-auth---virtual-keys"
...
def _process_keys_for_user_info(
    ...
    from llm.proxy.proxy_server import general_settings, litellm_master_key_hash
