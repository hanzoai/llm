...
import {Hanzo_SpendLogs, Hanzo_IncrementSpend, Hanzo_IncrementObject} from './_types'

const app = new Hono()
const prisma = new PrismaClient()
// In-memory storage for logs
let spend_logs: Hanzo_SpendLogs[] = [];
const key_logs: Hanzo_IncrementObject[] = [];
const user_logs: Hanzo_IncrementObject[] = [];
const transaction_logs: Hanzo_IncrementObject[] = [];
...
app.post('/spend/update', async (c) => {
  const incomingLogs = await c.req.json<Hanzo_SpendLogs[]>();
