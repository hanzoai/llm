...
/sso/callback - returns JWT Redirect Response that redirects to LLM UI
"""
...
async def add_missing_team_member(
    user_info: Union[NewUserResponse, LLM_UserTable], sso_teams: List[str]
):
    """
    ...
    try:
        if prisma_client is not None:
            ...
            if user_info is not None:
                ...
                verbose_proxy_logger.info(
                    "user not in DB, inserting user into LLM DB"
                )
                # user not in DB, insert User into LLM DB
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in LLM DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): LLM SSOValues / fields that were read
