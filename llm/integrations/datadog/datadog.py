...
`async_log_success_event` - used by llm proxy to send logs to datadog
`log_success_event` - sync version of logging to DataDog, only used on llm Python SDK, if user opts in to using sync functions
...
from llm.types.integrations.base_health_check import IntegrationHealthCheckStatus
from llm.types.integrations.datadog import *
from llm.types.services import ServiceLoggerPayload, ServiceTypes
...
class DataDogLogger(
    ...
    def _create_v0_logging_payload(
        ...
        (Not Recommended) If you want this to get logged set `llm.datadog_use_v1 = True`
        """
        import json
    ...
    def _get_datadog_tags(
        ...
        base_tags = {
            ...
            "service": os.getenv("DD_SERVICE", "llm"),
    ...
    def _get_datadog_source():
        return os.getenv("DD_SOURCE", "llm")
