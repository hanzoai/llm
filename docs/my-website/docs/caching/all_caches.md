...
llm.cache = Cache(type="redis", host=<host>, port=<port>, password=<password>)
...
# pass s3-bucket name
llm.cache = Cache(type="s3", s3_bucket_name="cache-bucket-llm", s3_region_name="us-west-2")
...
llm.cache = Cache(
    ...
    redis_semantic_cache_embedding_model="text-embedding-ada-002", # this model is passed to llm.embedding(), any llm.embedding() model is supported here
)
response1 = completion(
    model="gpt-3.5-turbo",
    messages=[
...
llm.cache = Cache(
    ...
    qdrant_semantic_cache_embedding_model="text-embedding-ada-002", # this model is passed to llm.embedding(), any llm.embedding() model is supported here
)
...
## Switch Cache On / Off Per Hanzo Call 

Hanzo supports 4 cache-controls:
...
Use the context manager for easily enabling, disabling & updating the llm cache 
...
Set your function as llm.cache.get_cache_key
```python
from llm.caching.caching import Cache
...
llm.cache = cache # set llm.cache to your cache 
