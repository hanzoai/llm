...
class TestOpenAIResponsesAPIConfig:
    ...
    def test_validate_environment(self):
        ...
        with patch("llm.api_key", "llm_api_key"):
            result = self.config.validate_environment(headers=headers, model=self.model)

            assert "Authorization" in result
            assert result["Authorization"] == "Bearer llm_api_key"
    ...
    def test_get_complete_url(self):
        ...
        with patch("llm.api_base", "https://llm-api-base.example.com/v1"):
            result = self.config.get_complete_url(api_base=None, model=self.model)

            assert result == "https://llm-api-base.example.com/v1/responses"
