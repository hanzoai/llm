...
async def add_team_callbacks(
    ...
    try:
        ...
        new_team_row = await prisma_client.db.llm_teamtable.update(
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.add_team_callbacks(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def disable_team_logging(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            f"llm.proxy.proxy_server.disable_team_logging(): Exception occurred - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def get_team_callbacks(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.get_team_callbacks(): Exception occurred - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
