...
from llm.types.llms.openai import AllMessageValues, ChatCompletionImageObject
from llm.types.utils import ProviderSpecificModelInfo
...
class FireworksAIConfig(OpenAIGPTConfig):
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            if param == "tool_choice":
                if value == "required":
                    # relevant issue: https://github.com/hanzoai/llm/issues/4416
    ...
    def _transform_messages_helper(
        self, messages: List[AllMessageValues], model: str, llm_params: dict
        ...
        disable_add_transform_inline_image_block = cast(
            Optional[bool],
            llm_params.get("disable_add_transform_inline_image_block")
            or llm.disable_add_transform_inline_image_block,
        )
    ...
    def transform_request(
        ...
        messages = self._transform_messages_helper(
            messages=messages, model=model, llm_params=llm_params
        )
    ...
    def get_models(self, api_key: Optional[str] = None, api_base: Optional[str] = None):
        ...
        response = llm.module_level_client.get(
