...
s/o [Nicholas Cecere](https://www.linkedin.com/in/nicholas-cecere-24243549/) for his Hanzo User Management Terraform
...
Your Hanzo Proxy Server is now running on `http://127.0.0.1:4000`.
...
Hanzo is compatible with several SDKs - including OpenAI SDK, Anthropic SDK, Mistral SDK, LLamaIndex, Langchain (Js, Python)
...
| [Hanzo container + Redis](#llm-container--redis)                                            | + load balance across multiple llm containers                                                                                                     |
| [Hanzo Database container + PostgresDB + Redis](#llm-database-container--postgresdb--redis) | + use Virtual Keys + Track Spend + load balance across multiple llm containers                                                                    |
...
We maintain a [separate Dockerfile](https://github.com/BerriAI/llm/pkgs/container/llm-database) for reducing build time when running Hanzo proxy with a connected Postgres Database 
...
Your Hanzo Proxy Server is now running on `http://0.0.0.0:4000`.
...
Use this to deploy llm using a helm chart. Link to [the Hanzo Helm Chart](https://github.com/BerriAI/llm/tree/main/deploy/charts/llm-helm)
...
Hanzo Proxy supports sharing rpm/tpm shared across multiple llm instances, pass `redis_host`, `redis_password` and `redis_port` to enable this. (Hanzo will use Redis to track rpm/tpm usage )
...
💥 Use this when you want to serve Hanzo on a custom base url path like `https://localhost:4000/api/v1` 
...
That's it, now you can start using Hanzo Proxy
...
Hanzo AWS Cloudformation Stack - **Get the best Hanzo AutoScaling Policy and Provision the DB for Hanzo Proxy**

This will provision:
- HanzoServer - EC2 Instance
- HanzoServerAutoScalingGroup
- HanzoServerScalingPolicy (autoscaling policy)
- HanzoDB - RDS::DBInstance

#### Using AWS Cloud Formation Stack
**Hanzo Cloudformation stack is located [here - llm.yaml](https://github.com/BerriAI/llm/blob/main/enterprise/cloudformation_stack/llm.yaml)**
...
Create a `llm-config.yaml` file with your Hanzo config relative to your `docker-compose.yml` file.
...
Your Hanzo container should be running now on the defined port e.g. `4000`.
