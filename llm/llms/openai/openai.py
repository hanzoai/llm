...
from llm.llms.base_llm.base_model_iterator import BaseModelResponseIterator
...
from llm.llms.custom_httpx.http_handler import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
...
class OpenAIConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        elif llm.openAIGPTAudioConfig.is_model_gpt_audio_model(model=model):
            return llm.openAIGPTAudioConfig.get_supported_openai_params(model=model)
        else:
            return llm.openAIGPTConfig.get_supported_openai_params(model=model)
    ...
    def map_openai_params(
        ...
        if openaiOSeriesConfig.is_model_o_series_model(model=model):
            ...
            return llm.openAIGPTAudioConfig.map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=drop_params,
        ...
        return llm.openAIGPTConfig.map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params,
...
class OpenAIChatCompletion(BaseLLM):
    ...
    def _get_openai_client(
        ...
        if client is None:
            ...
            _cached_client = llm.in_memory_llm_clients_cache.get_cache(_cache_key)
            if _cached_client:
            ...
            if is_async:
                _new_client: Union[OpenAI, AsyncOpenAI] = AsyncOpenAI(
                    api_key=api_key,
                    base_url=api_base,
                    http_client=llm.aclient_session,
                    timeout=timeout,
                ...
                _new_client = OpenAI(
                    api_key=api_key,
                    base_url=api_base,
                    http_client=llm.client_session,
                    timeout=timeout,
                    max_retries=max_retries,
                    organization=organization,
                )
    ...
    async def make_openai_chat_completion_request(
        ...
        """
        Helper to:
        - call chat.completions.create.with_raw_response when llm.return_response_headers is True
        ...
        try:
            ...
            for _ in range(
                ...
                try:
                    ...
                    ## check if body contains unprocessable params - related issue https://github.com/BerriAI/llm/issues/4800
                    if llm.drop_params is True or drop_params is True:
                    ...
                    if (
                        ...
                        llm.remove_index_from_tool_calls(messages=messages)
                    else:
                        raise e
    ...
    def get_stream_options(
        ...
        if stream_options is not None:
            ...
            # by default llm will include usage for openai endpoints
    ...
    async def make_openai_embedding_request(
        ...
        """
        Helper to:
        - call embeddings.create.with_raw_response when llm.return_response_headers is True
    ...
    def embedding(  # type: ignore
        ...
        try:
            ...
            max_retries = max_retries or llm.DEFAULT_MAX_RETRIES
            if not isinstance(max_retries, int):
...
class OpenAIBatchesAPI(BaseLLM):
    ...
    async def acreate_batch(
        ...
        return HanzoBatch(**response.model_dump())
    ...
    def create_batch(
        ...
        organization: Optional[str],
        client: Optional[Union[OpenAI, AsyncOpenAI]] = None,
    ) -> Union[HanzoBatch, Coroutine[Any, Any, HanzoBatch]]:
        openai_client: Optional[Union[OpenAI, AsyncOpenAI]] = self.get_openai_client(
            api_key=api_key,
            api_base=api_base,
            timeout=timeout,
...
class OpenAIAssistantsAPI(BaseLLM):
    ...
    def create_thread(
        ...
        Here's an example:
        ```
        from llm.llms.openai.openai import OpenAIAssistantsAPI, MessageData
