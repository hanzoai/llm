...
Cache LLM Responses. LLM's caching system stores and reuses LLM responses to save costs and reduce latency. When you make the same request twice, the cached response is returned instead of calling the LLM API again.
...
[**See how it's read from the environment**](https://github.com/hanzoai/llm/blob/4d7ff1b33b9991dcf38d821266290631d9bcd2dd/llm/_redis.py#L40)
...
LLM Proxy exposes a `/cache/ping` endpoint to test if the cache is working as expected
...
[**SEE CODE**](https://github.com/hanzoai/llm/blob/main/llm/proxy/hooks/batch_redis_get.py)
