...
def setup_mocks():
    with patch(
        ...
        "llm.llms.azure.common_utils.llm"
    ) as mock_llm, patch(
...
def test_initialize_with_oidc_token(setup_mocks):
    ...
    result = BaseAzureLLM().initialize_azure_sdk_client(
        llm_params={"azure_ad_token": "oidc/test-token"},
        api_key=None,
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version=None,
...
def test_initialize_with_enable_token_refresh(setup_mocks):
    ...
    setup_mocks["llm"].enable_azure_ad_token_refresh = True
...
async def test_ensure_initialize_azure_sdk_client_always_used(call_type):
    ...
    patch_target = "llm.main.azure_chat_completions.initialize_azure_sdk_client"
    if call_type == CallTypes.atranscription:
        patch_target = (
            "llm.main.azure_audio_transcriptions.initialize_azure_sdk_client"
        )
    ...
    with patch(patch_target) as mock_init_azure:
        ...
        llm_params = azure_calls[0].kwargs["llm_params"]
        print("llm_params", llm_params)

        assert (
            "azure_ad_token" in llm_params
