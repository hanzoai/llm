...
def function_setup(  # noqa: PLR0915
    ...
    ### NOTICES ###
    from llm import Logging as LLMLogging
...
def openai_token_counter(  # noqa: PLR0915
    ...
    """
    print_verbose(f"LLM: Utils - Counting tokens for OpenAI model={model}")
    try:
        if "gpt-4o" in model:
    ...
    if (
        ...
        config_class = (
            ...
            if llm.AmazonStability3Config._is_stability_3_model(model=model)
                ...
                llm.AmazonNovaCanvasConfig
                if llm.AmazonNovaCanvasConfig._is_nova_model(model=model)
...
def get_optional_params(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: List[str]):
        verbose_logger.info(
            f"\nLLM completion() model= {model}; provider = {custom_llm_provider}"
        )
        verbose_logger.debug(
            f"\nLLM: Params passed to completion() {passed_params}"
        )
        verbose_logger.debug(
            f"\nLLM: Non-Default params passed to completion() {non_default_params}"
        )
        unsupported_params = {}
        for k in non_default_params.keys():
            if k not in supported_params:
    ...
    if custom_llm_provider == "anthropic":
        ...
        for param in passed_params.keys():
            if llm.IBMWatsonXAIConfig().is_watsonx_text_param(param):
                raise ValueError(
                    f"LLM now defaults to Watsonx's `/text/chat` endpoint. Please use the `watsonx_text` provider instead, to call the `/text/generation` endpoint. Param: {param}"
                )
...
def _get_model_region(
    custom_llm_provider: str, llm_params: LLM_Params
) -> Optional[str]:
    """
...
def _infer_model_region(llm_params: LLM_Params) -> Optional[AllowedModelRegion]:
    """
...
def _is_region_eu(llm_params: LLM_Params) -> bool:
    """
...
def _is_region_us(llm_params: LLM_Params) -> bool:
    """
...
def is_region_allowed(
    llm_params: LLM_Params, allowed_model_region: str
) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
...
def get_model_region(
    llm_params: LLM_Params, mode: Optional[str]
) -> Optional[str]:
    """
...
def trim_messages(
    ...
    Args:
        ...
        model: The LLM model being used (determines the token limit).
