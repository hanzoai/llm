...
from llm.types.services import ServiceLoggerPayload
...
if TYPE_CHECKING:
    ...
    from llm.proxy.proxy_server import UserAPIKeyAuth as _UserAPIKeyAuth
...
class OpenTelemetry(CustomLogger):
    def __init__(
        ...
        **kwargs,
        ...
        self._init_otel_logger_on_llm_proxy()

    def _init_otel_logger_on_llm_proxy(self):
        """
        Initializes OpenTelemetry for llm proxy server
        ...
        if "otel" not in llm.service_callback:
            llm.service_callback.append("otel")
                ...
                for key, value in event_metadata.items():
                    ...
                    if isinstance(value, dict):
                        try:
                            ...
                            value = "llm logging error - could_not_json_serialize"
                    self.safe_set_attribute(
                        span=service_logging_span,
                        key=key,
                        value=value,
    ...
    def set_attributes(  # noqa: PLR0915
        ...
        try:
            if self.callback_name == "arize":
                ...
                from llm.integrations.arize.arize_phoenix import ArizePhoenixLogger

                ArizePhoenixLogger.set_arize_phoenix_attributes(span, kwargs, response_obj)
                return
            elif self.callback_name == "langtrace":
                from llm.integrations.langtrace import LangtraceAttributes
            ...
            self.safe_set_attribute(
                ...
                value=llm_params.get("custom_llm_provider", "Unknown"),
            )
    ...
    def set_raw_request_attributes(self, span: Span, kwargs, response_obj):
        ...
        llm_params = kwargs.get("llm_params", {}) or {}
        custom_llm_provider = llm_params.get("custom_llm_provider", "Unknown")
        ...
        if _raw_response and isinstance(_raw_response, str):
            ...
            try:
                ...
                verbose_logger.debug(
                    "llm.integrations.opentelemetry.py::set_raw_request_attributes() - raw_response not json string - {}".format(
                        _raw_response
                    )
                )
