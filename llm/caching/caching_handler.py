...
from llm.caching.caching import S3Cache
...
class LLMCachingHandler:
    ...
    async def _async_get_cache(
        ...
        cached_result: Optional[Any] = None
        if (
            (kwargs.get("caching", None) is None and llm.cache is not None)
            or kwargs.get("caching", False) is True
        ) and (
            kwargs.get("cache", {}).get("no-cache", False) is not True
        ):  # allow users to control returning cached responses from the completion function
            if llm.cache is not None and self._is_call_type_supported_by_cache(
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    model, _, _, _ = llm.get_llm_provider(
                        model=model,
                        custom_llm_provider=kwargs.get("custom_llm_provider", None),
                        api_base=kwargs.get("api_base", None),
                        api_key=kwargs.get("api_key", None),
                    )
                    self._update_llm_logging_obj_environment(
                    ...
                    cache_key = llm.cache._get_preset_cache_key_from_kwargs(
                        **kwargs
                    )
                    if (
    ...
    def _sync_get_cache(
        ...
        if llm.cache is not None and self._is_call_type_supported_by_cache(
            ...
            cached_result = llm.cache.get_cache(**new_kwargs)
    ...
    def _async_log_cache_hit_on_callbacks(
        ...
        Args:
            logging_obj (HanzoLoggingObj): The logging object.
    ...
    async def _retrieve_from_cache(
        ...
        if call_type == CallTypes.aembedding.value and isinstance(
            ...
            for idx, i in enumerate(new_kwargs["input"]):
                preset_cache_key = llm.cache.get_cache_key(
                    **{**new_kwargs, "input": i}
                )
                tasks.append(llm.cache.async_get_cache(cache_key=preset_cache_key))
            ...
            if llm.cache._supports_async() is True:
                cached_result = await llm.cache.async_get_cache(**new_kwargs)
    ...
    def _convert_cached_result_to_model_response(
        ...
        Returns:
            Optional[Any]:
        """
        from llm.utils import convert_to_model_response_object
    ...
    async def async_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            if (
                isinstance(result, llm.ModelResponse)
                or isinstance(result, llm.EmbeddingResponse)
                ...
                if (
                    ...
                    asyncio.create_task(
                        llm.cache.async_add_cache_pipeline(result, **new_kwargs)
                    )
                elif isinstance(llm.cache.cache, S3Cache):
                    threading.Thread(
                        target=llm.cache.add_cache,
                        args=(result,),
                        kwargs=new_kwargs,
                    ).start()
                else:
                    asyncio.create_task(
                        llm.cache.async_add_cache(
                            result.model_dump_json(), **new_kwargs
                        )
                    )
            else:
                asyncio.create_task(llm.cache.async_add_cache(result, **new_kwargs))
    ...
    def sync_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            llm.cache.add_cache(result, **new_kwargs)
    ...
    def _should_store_result_in_cache(
        ...
        """
        return (
            (llm.cache is not None)
            and llm.cache.supported_call_types is not None
            and (str(original_function.__name__) in llm.cache.supported_call_types)
    ...
    def _is_call_type_supported_by_cache(
        ...
        Defined on `llm.types.utils.CallTypes`
    ...
    async def _add_streaming_response_to_cache(self, processed_chunk: ModelResponse):
        ...
        - If 'streaming_chunk' has a 'finish_reason' then assemble a llm.ModelResponse object
    ...
    def _update_llm_logging_obj_environment(
        self,
        logging_obj: HanzoLoggingObj,
        model: str,
        kwargs: Dict[str, Any],
        ...
        """
        Helper function to update the HanzoLoggingObj environment variables.

        Args:
            logging_obj (HanzoLoggingObj): The logging object to update.
        ...
        if llm.cache is not None:
            llm_params["preset_cache_key"] = (
                llm.cache._get_preset_cache_key_from_kwargs(**kwargs)
            )
        else:
            llm_params["preset_cache_key"] = None
