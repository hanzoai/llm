    ...
    async with aiohttp.ClientSession() as session:
        try:
            response, headers = await chat_completion(
                ...
                model="fake-openai-endpoint",
                messages=[
                    {"role": "user", "content": f"Hello my name is z@hanzo.ai"}
                ],
                guardrails=[
                    "aporia-post-guard",
                    "aporia-pre-guard",
...
async def test_guardrails_with_api_key_controls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        response, headers = await chat_completion(
            ...
            key_with_guardrails,
            model="fake-openai-endpoint",
            messages=[{"role": "user", "content": f"Hello my name is z@hanzo.ai"}],
        )
...
async def test_guardrails_with_team_controls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        response, headers = await chat_completion(
            ...
            key_without_team,
            model="fake-openai-endpoint",
            messages=[{"role": "user", "content": "Hello my name is z@hanzo.ai"}],
        )
        await asyncio.sleep(3)
