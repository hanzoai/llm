...
from llm.types.router import DeploymentTypedDict, LLM_Params, RouterErrors
...
class RouterBudgetLimiting(CustomLogger):
    ...
    async def _increment_spend_in_current_window(
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of LLM)
        """
    ...
    def _get_llm_provider_for_deployment(self, deployment: Dict) -> Optional[str]:
        try:
            _llm_params: LLM_Params = LLM_Params(
