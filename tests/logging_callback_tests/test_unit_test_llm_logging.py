...
def test_is_internal_llm_proxy_callback():
    """
    ...
    assert logging._is_internal_llm_proxy_callback(_PROXY_MaxBudgetLimiter) == True

    # Test non-internal callbacks
    def regular_callback():
        pass

    assert logging._is_internal_llm_proxy_callback(regular_callback) == False

    # Test string callback
    assert logging._is_internal_llm_proxy_callback("callback_string") == False
...
def test_remove_internal_llm_callbacks():
    logging = setup_logging()
    ...
    filtered = logging._remove_internal_llm_callbacks(callbacks)
