...
from llm import EmbeddingResponse, ImageResponse, ModelResponse, Router
from llm._logging import verbose_proxy_logger
from llm._service_logger import ServiceLogging, ServiceTypes
...
from llm.llms.custom_httpx.httpx_handler import HTTPHandler
from llm.proxy._types import (
    AlertType,
    CallInfo,
...
from llm.proxy.db.create_views import (
    create_missing_views,
    should_create_missing_views,
)
from llm.proxy.db.log_db_metrics import log_db_metrics
from llm.proxy.db.prisma_client import PrismaWrapper
...
from llm.proxy.llm_pre_call_utils import HanzoProxyRequestSetup
from llm.secret_managers.main import str_to_bool
from llm.types.integrations.slack_alerting import DEFAULT_ALERT_TYPES
from llm.types.utils import CallTypes, LoggedHanzoParams
...
def print_verbose(print_statement):
    """
    Prints the given `print_statement` to the console if `llm.set_verbose` is True.
    ...
    if llm.set_verbose:
        print(f"Hanzo Proxy: {print_statement}")  # noqa
...
def safe_deep_copy(data):
    ...
    The Hanzo Request has some object that can-not be pickled / deep copied

    Use this function to safely deep copy the Hanzo Request
    """
...
class ProxyLogging:
    ...
    async def update_request_status(
        self, llm_call_id: str, status: Literal["success", "fail"]
    ):
        ...
        await self.internal_usage_cache.async_set_cache(
            key="request_status:{}".format(llm_call_id),
            value=status,
            local_only=True,
    ...
    async def alerting_handler(
        ...
        for client in self.alerting:
            if client == "slack":
                ...
                if llm.utils.sentry_sdk_instance is not None:
                    llm.utils.sentry_sdk_instance.capture_message(formatted_message)
                else:
    ...
    async def failure_handler(
        ...
        if llm.utils.capture_exception:
            llm.utils.capture_exception(error=original_exception)
    ...
    async def post_call_failure_hook(
        ...
        await self.update_request_status(
            llm_call_id=request_data.get("llm_call_id", ""), status="fail"
        )
    ...
    async def _handle_logging_proxy_only_error(
        ...
        """
        Handle logging for proxy only errors by calling `llm_logging_obj.async_failure_handler`

        Is triggered when self._is_proxy_only_error() returns True
        """
        llm_logging_obj: Optional[Logging] = request_data.get(
            "llm_logging_obj", None
        )
        if llm_logging_obj is None:
            import uuid

            request_data["llm_call_id"] = str(uuid.uuid4())
            user_api_key_logged_metadata = (
                HanzoProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )

            llm_logging_obj, data = llm.utils.function_setup(
                original_function=route or "IGNORE_THIS",
                rules_obj=llm.utils.Rules(),
        ...
        if llm_logging_obj is not None:
            ## UPDATE LOGGING INPUT
            _optional_params = {}
            _llm_params = {}

            llm_param_keys = LoggedHanzoParams.__annotations__.keys()
            ...
            if "messages" in request_data and isinstance(
                ...
                llm_logging_obj.model_call_details["messages"] = input
                llm_logging_obj.call_type = CallTypes.acompletion.value
            elif "prompt" in request_data and isinstance(request_data["prompt"], str):
                input = request_data["prompt"]
                llm_logging_obj.model_call_details["prompt"] = input
                llm_logging_obj.call_type = CallTypes.atext_completion.value
            elif "input" in request_data and isinstance(request_data["input"], list):
                input = request_data["input"]
                llm_logging_obj.model_call_details["input"] = input
                llm_logging_obj.call_type = CallTypes.aembedding.value
            ...
            await llm_logging_obj.async_failure_handler(
                exception=original_exception,
                traceback_exception=traceback.format_exc(),
            )

            threading.Thread(
                target=llm_logging_obj.failure_handler,
                args=(
                    original_exception,
    ...
    async def async_post_call_streaming_hook(
        ...
        if isinstance(response, ModelResponse):
            response_str = llm.get_response_string(response_obj=response)
...
class PrismaClient:
    ...
    async def check_view_exists(self):
        """
        Checks if the Hanzo_VerificationTokenView and MonthlyGlobalSpend exists in the user's db.

        Hanzo_VerificationTokenView: This view is used for getting the token + team data in user_api_key_auth
        ...
        try:
            ...
            required_view = "Hanzo_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    await self.db.execute_raw(
                        """
                            CREATE VIEW "Hanzo_VerificationTokenView" AS
                            SELECT
                            v.*,
                            ...
                            FROM "Hanzo_VerificationToken" v
                            LEFT JOIN "Hanzo_TeamTable" t ON v.team_id = t.team_id;
                        """
                    )

                    verbose_proxy_logger.info(
                        "Hanzo_VerificationTokenView Created in DB!"
                    )
                else:
    ...
    async def get_generic_data(
        ...
        try:
            if table_name == "users":
                ...
                response = await self.db.llm_verificationtoken.find_first(  # type: ignore
                    where={key: value}  # type: ignore
                )
            ...
            error_msg = f"Hanzo Prisma Client Exception get_generic_data: {str(e)}"
        ...
        try:
            ...
            if (token is not None and table_name is None) or (
                ...
                if query_type == "find_unique" and hashed_token is not None:
                    ...
                    response = await self.db.llm_verificationtoken.find_unique(
                    ...
                    response = await self.db.llm_verificationtoken.find_many(
                ...
                if query_type == "find_unique":
                    ...
                    if expires is not None:
                        ...
                        FROM
                            "Hanzo_UserTable" u
                        LEFT JOIN "Hanzo_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if key_val is not None:
                    if query_type == "find_unique":
                        response = await self.db.llm_spendlogs.find_unique(  # type: ignore
                            where={  # type: ignore
                                key_val["key"]: key_val["value"],  # type: ignore
                            }
                        )
                    elif query_type == "find_all":
                        response = await self.db.llm_spendlogs.find_many(  # type: ignore
                            where={
                                key_val["key"]: key_val["value"],  # type: ignore
                            }
                        )
                ...
                if query_type == "find_unique":
                    response = await self.db.llm_teamtable.find_unique(
                        where={"team_id": team_id},  # type: ignore
                        include={"llm_model_table": True},  # type: ignore
                    )
                elif query_type == "find_all" and reset_at is not None:
                    response = await self.db.llm_teamtable.find_many(
                        where={  # type:ignore
                            "budget_reset_at": {"lt": reset_at},
                        }
                    )
                    ...
                    response = await self.db.llm_teamtable.find_many(take=20)
                return response
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = f"""
                        SELECT 
                            ...
                            b.max_budget AS llm_budget_table_max_budget,
                            b.tpm_limit AS llm_budget_table_tpm_limit,
                            b.rpm_limit AS llm_budget_table_rpm_limit,
                            b.model_max_budget as llm_budget_table_model_max_budget,
                            b.soft_budget as llm_budget_table_soft_budget
                        FROM "Hanzo_VerificationToken" AS v
                        LEFT JOIN "Hanzo_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "Hanzo_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "Hanzo_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "Hanzo_BudgetTable" AS b ON v.budget_id = b.budget_id
                        WHERE v.token = '{token}'
                    """
                    ...
                    if response is not None:
                        ...
                        response = Hanzo_VerificationTokenView(
            ...
            prisma_query_info = f"Hanzo Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
    ...
    async def insert_data(  # noqa: PLR0915
        ...
        try:
            ...
            if table_name == "key":
                ...
                print_verbose(
                    "PrismaClient: Before upsert into llm_verificationtoken"
                )
                new_verification_token = await self.db.llm_verificationtoken.upsert(  # type: ignore
                    where={
                        "token": hashed_token,
                    },
                    data={
                ...
                try:
                    ...
                    if (
                        "Foreign key constraint failed on the field: `Hanzo_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in Hanzo_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
                ...
                new_team_row = await self.db.llm_teamtable.upsert(
                ...
                db_data = self.jsonify_object(data=data)
                new_spend_row = await self.db.llm_spendlogs.upsert(
            ...
            error_msg = f"Hanzo Prisma Client Exception in insert_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def update_data(  # noqa: PLR0915
        ...
        try:
            ...
            if token is not None:
                ...
                token = _hash_token_if_needed(token=token)
                db_data["token"] = token
                response = await self.db.llm_verificationtoken.update(
                ...
                update_team_row = await self.db.llm_teamtable.upsert(
                    where={"team_id": team_id},  # type: ignore
                    data={
                        "create": {**db_data},  # type: ignore
                        "update": {
                ...
                for idx, t in enumerate(data_list):
                    ...
                    batcher.llm_verificationtoken.update(
            ...
            error_msg = f"Hanzo Prisma Client Exception - update_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            if tokens is not None and isinstance(tokens, List):
                ...
                deleted_tokens = await self.db.llm_verificationtoken.delete_many(
                ...
                await self.db.llm_teamtable.delete_many(
                    where={"team_id": {"in": team_id_list}}
                )
                ...
                # admin only endpoint -> `/team/delete`
                await self.db.llm_verificationtoken.delete_many(
                    where={"team_id": {"in": team_id_list}}
                )
        except Exception as e:
            import traceback

            error_msg = f"Hanzo Prisma Client Exception - delete_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg = f"Hanzo Prisma Client Exception connect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg = f"Hanzo Prisma Client Exception disconnect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
                ...
                try:
                    ...
                    if (
                        ...
                        for j in range(0, len(logs_to_process), BATCH_SIZE):
                            ...
                            await prisma_client.db.llm_spendlogs.create_many(
                ...
                async with prisma_client.db.tx(
                    ...
                    async with transaction.batch_() as batcher:
                        for (
                            ...
                            batcher.llm_verificationtoken.update_many(  # 'update_many' prevents error from being raised if no row exists
                                where={"token": token},
                                data={"spend": {"increment": response_cost}},
                            )
                ...
                async with prisma_client.db.tx(
                    ...
                    async with transaction.batch_() as batcher:
                        for (
                            ...
                            batcher.llm_teamtable.update_many(  # 'update_many' prevents error from being raised if no row exists
                                where={"team_id": team_id},
                                data={"spend": {"increment": response_cost}},
                            )
                ...
                async with prisma_client.db.tx(
                    ...
                    async with transaction.batch_() as batcher:
                        for (
                            ...
                            batcher.llm_teammembership.update_many(  # 'update_many' prevents error from being raised if no row exists
...
def _raise_failed_update_spend_exception(
    ...
    error_msg = (
        f"[Non-Blocking]Hanzo Prisma Client Exception - update spend logs: {str(e)}"
    )
    error_traceback = error_msg + "\n" + traceback.format_exc()
    end_time = time.time()
    _duration = end_time - start_time
...
def _premium_user_check():
    ...
    if not premium_user:
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"This feature is only available for Hanzo Enterprise users. {CommonProxyErrors.not_premium_user.value}"
            },
        )
