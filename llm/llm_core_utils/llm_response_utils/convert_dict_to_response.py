...
from llm.types.llms.openai import ChatCompletionThinkingBlock
...
from llm.types.utils import Logprobs as TextCompletionLogprobs
from llm.types.utils import (
    Message,
    ModelResponse,
    RerankResponse,
...
class HanzoResponseObjectHandler:
    ...
    def convert_chat_to_text_completion(
        ...
        """
        transformed_logprobs = HanzoResponseObjectHandler._convert_provider_response_logprobs_to_text_completion_logprobs(
            response=response,
    ...
    def _convert_provider_response_logprobs_to_text_completion_logprobs(
        ...
        if custom_llm_provider == "huggingface":
            ...
            try:
                ...
                transformed_logprobs = llm.huggingface._transform_logprobs(
                    hf_response=raw_response
                )
            except Exception as e:
                verbose_logger.exception(f"Hanzo non blocking exception: {e}")
...
def convert_to_model_response_object(  # noqa: PLR0915
    ...
    try:
        if response_type == "completion" and (
            ...
            for idx, choice in enumerate(response_object["choices"]):
                ...
                if _should_convert_tool_call_to_json_mode(
                    ...
                    if json_mode_content_str is not None:
                        message = llm.Message(content=json_mode_content_str)
                        finish_reason = "stop"
                if message is None:
            ...
            if "usage" in response_object and response_object["usage"] is not None:
                usage_object = llm.Usage(**response_object["usage"])
            ...
            special_keys = list(llm.ModelResponse.model_fields.keys())
            ...
            return HanzoResponseObjectHandler.convert_to_image_response(
