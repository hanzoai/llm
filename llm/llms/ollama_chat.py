...
from llm.types.llms.ollama import OllamaToolCall, OllamaToolCallFunction
from llm.types.llms.openai import ChatCompletionAssistantToolCall
from llm.types.utils import ModelResponse, StreamingChoices
...
def get_ollama_response(  # noqa: PLR0915
    ...
    ## Load Config
    config = llm.OllamaChatConfig.get_config()
    ...
    for m in messages:
        if isinstance(
            m, BaseModel
        ):  # avoid message serialization issues - https://github.com/BerriAI/llm/issues/5319
    ...
    sync_client = llm.module_level_client
    ...
    if data.get("format", "") == "json" and function_name is not None:
        ...
        _message = llm.Message(**response_json["message"])
        model_response.choices[0].message = _message  # type: ignore
    model_response.created = int(time.time())
    model_response.model = "ollama_chat/" + model
    prompt_tokens = response_json.get("prompt_eval_count", llm.token_counter(messages=messages))  # type: ignore
    completion_tokens = response_json.get(
        "eval_count", llm.token_counter(text=response_json["message"]["content"])
    )
    setattr(
        model_response,
        "usage",
...
def ollama_completion_stream(url, api_key, data, logging_obj):
    ...
    with httpx.stream(**_request) as response:
        try:
            ...
            streamwrapper = llm.CustomStreamWrapper(
...
async def ollama_async_streaming(
    ...
    try:
        ...
        verbose_logger.exception(
            "Hanzo.ollama(): Exception occured - {}".format(str(e))
        )
        raise e
...
async def ollama_acompletion(
    ...
    data["stream"] = False
    try:
        timeout = aiohttp.ClientTimeout(total=llm.request_timeout)  # 10 minutes
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            prompt_tokens = response_json.get("prompt_eval_count", llm.token_counter(messages=data["messages"]))  # type: ignore
            completion_tokens = response_json.get(
                "eval_count",
                llm.token_counter(
