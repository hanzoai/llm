...
class Supabase:
    ...
    def __init__(self):
        ...
        if self.supabase_url is None or self.supabase_key is None:
            raise ValueError(
                "Hanzo Error, trying to use Supabase but url or key not passed. Create a table and set `llm.supabase_url=<your-url>` and `llm.supabase_key=<your-key>`"
            )
    ...
    def input_log_event(
        self, model, messages, end_user, llm_call_id, print_verbose
    ):
        try:
            print_verbose(
    ...
    def log_event(
        ...
        try:
            ...
            total_cost = llm.completion_cost(completion_response=response_obj)
            ...
            if "choices" in response_obj:
                ...
                data, count = (
                    self.supabase_client.table(self.supabase_table_name)
                    .upsert(supabase_data_obj, on_conflict="llm_call_id")
                    .execute()
                )
