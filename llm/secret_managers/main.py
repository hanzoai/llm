...
def get_secret(  # noqa: PLR0915
    ...
    key_management_system = llm._key_management_system
    key_management_settings = llm._key_management_settings
    secret = None
    ...
    try:
        if (
            ...
            and llm.secret_manager_client is not None
        ):
            try:
                client = llm.secret_manager_client
                key_manager = "local"
                ...
                if (
                    ...
                    response = client.decrypt(
                        request={
                            "name": llm._google_kms_resource_name,
...
def _should_read_secret_from_secret_manager() -> bool:
    ...
    """
    if llm.secret_manager_client is not None:
        if llm._key_management_settings is not None:
            if (
                llm._key_management_settings.access_mode == "read_only"
                or llm._key_management_settings.access_mode == "read_and_write"
            ):
                return True
    return False
