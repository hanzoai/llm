...
class LLMDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    ...
    def _run_health_check(host, port):
        print("\nLLM: Health Testing models in config")  # noqa
    ...
    def _run_test_chat_completion(
        ...
        click.echo(
            f"\nLLM: Making a test ChatCompletions request to your proxy. Model={request_model}"
        )
        import openai
        ...
        click.echo(f"\nLLM: response from proxy {response}")

        print(  # noqa
            f"\n LLM: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}"
        )
        ...
        for chunk in stream_response:
            click.echo(f"LLM: streaming response from proxy {chunk}")
    ...
    def _init_hypercorn_server(
        ...
        print(  # noqa
            f"\033[1;32mLLM Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n"  # noqa
        )  # noqa
        config = Config()
        ...
        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(  # noqa
                f"\033[1;32mLLM Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"  # noqa
            )
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()  # noqa
                print(  # noqa
                    '\033[1;34mLLM: Test your local proxy with: "llm --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(  # noqa
            ...
            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with LLM Proxy config
        ...
        print(  # noqa
            f"\033[1;32mLLM Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n"  # noqa
        )
        gunicorn_options = {
            "bind": f"{host}:{port}",
    ...
    if test is True:
        ...
        if config is not None:
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                LLMDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            db_connection_timeout = general_settings.get(
                "database_connection_timeout",
                LLMDatabaseConnectionPool.database_connection_pool_timeout.value,
            )
        ...
        if (
            ...
            if is_prisma_runnable:
                ...
                from llm.proxy.db.prisma_client import (
                    PrismaManager,
