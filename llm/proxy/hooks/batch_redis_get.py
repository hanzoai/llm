...
class _PROXY_BatchRedisRequests(CustomLogger):
    ...
    def __init__(self):
        if llm.cache is not None:
            llm.cache.async_get_cache = (
                self.async_get_cache
            )  # map the llm 'get_cache' function to our custom function
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            if in_memory_cache_exists is False and llm.cache is not None:
                """
                - Check if `llm.Cache` is redis
                - Get the relevant values
                """
                if llm.cache.type is not None and isinstance(
                    ...
                    keys = await llm.cache.cache.async_scan_iter(
                    ...
                    if len(keys) > 0:
                        key_value_dict = (
                            await llm.cache.cache.async_batch_get_cache(
                                key_list=keys
                            )
                        )
            ...
            verbose_proxy_logger.error(
                "llm.proxy.hooks.batch_redis_get.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
        ...
        try:  # never block execution
            ...
            if (
                ...
                if cached_result is None:
                    cached_result = await llm.cache.cache.async_get_cache(
                        cache_key, *args, **kwargs
                    )
                ...
                return llm.cache._get_cache_logic(
