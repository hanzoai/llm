...
Not all vertex models support passing the json_schema to them (e.g. `gemini-1.5-flash`). To solve this, Hanzo supports client-side validation of the json schema. 

```
llm.enable_json_schema_validation=True
```
If `llm.enable_json_schema_validation=True` is set, Hanzo will validate the json response using `jsonvalidator`. 
