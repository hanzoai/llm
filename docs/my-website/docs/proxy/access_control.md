...
- `Internal Users` - users that can create keys, make LLM API calls, view usage on Hanzo 
- `Roles` define the permissions of an `Internal User`
- `Virtual Keys` - Keys are used for authentication to the Hanzo API. Keys are tied to a `Internal User` and `Team` 
