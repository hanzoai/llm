...
import llm
from llm._logging import print_verbose
from llm.utils import get_optional_params
...
def batch_completion(
    ...
    **kwargs,
    ...
    """
    Batch llm.completion function for a given model.
    ...
    if model.split("/", 1)[0] in llm.provider_list:
            ...
            for sub_batch in chunks(batch_messages, 100):
                for message_list in sub_batch:
                    ...
                    future = executor.submit(
                        llm.completion, **kwargs_modified, **original_kwargs
                    )
        ...
        with ThreadPoolExecutor(max_workers=len(models)) as executor:
            for model in models:
                futures[model] = executor.submit(
                    llm.completion, *args, model=model, **kwargs
                )
...
def batch_completion_models_all_responses(*args, **kwargs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(models)) as executor:
        for idx, model in enumerate(models):
            future = executor.submit(llm.completion, *args, model=model, **kwargs)
            if future.result() is not None:
