...
async def test_write_and_read_simple_secret():
    ...
    test_secret_name = f"llm_test_{uuid.uuid4().hex[:8]}"
...
async def test_write_and_read_json_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    test_secret_name = f"llm_test_{uuid.uuid4().hex[:8]}_json"
    test_secret_value = {
        "api_key": "test_key",
        "model": "gpt-4",
        "temperature": 0.7,
    ...
    try:
        ...
        write_response = await secret_manager.async_write_secret(
            ...
            description="Hanzo JSON Test Secret",
        )
...
async def test_read_nonexistent_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    nonexistent_secret = f"llm_nonexistent_{uuid.uuid4().hex}"
...
async def test_primary_secret_functionality():
    ...
    secret_manager = AWSSecretsManagerV2()
    primary_secret_name = f"llm_test_primary_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        write_response = await secret_manager.async_write_secret(
            ...
            description="Hanzo Test Primary Secret",
        )
