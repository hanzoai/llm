# used for /metrics endpoint on LLM Proxy
#### What this does ####
...
from llm.proxy._types import LLM_TeamTable, UserAPIKeyAuth
...
class PrometheusLogger(CustomLogger):
        ...
        **kwargs,
        ...
        try:
            ...
            self.llm_request_total_latency_metric = Histogram(
                ...
                "Total latency (seconds) for a request to LLM",
                labelnames=PrometheusMetricLabels.get_labels(
            ...
            self.llm_api_latency_metric = Histogram(
                "llm_api_latency_metric",
                "Total latency (seconds) for a models LLM API call",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_api_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            )

            self.llm_api_time_to_first_token_metric = Histogram(
                "llm_api_time_to_first_token_metric",
            ...
            self.llm_deployment_cooled_down = Counter(
                ...
                "LLM Deployment Analytics - Number of times a deployment has been cooled down by LLM load balancing logic. exception_status is the status of the exception that caused the deployment to be cooled down",
            ...
            self.llm_api_failed_requests_metric = Counter(
                name="llm_api_failed_requests_metric",
    ...
    def _set_latency_metrics(
        ...
        if (
            ...
            self.llm_api_time_to_first_token_metric.labels(
                model,
                user_api_key,
                user_api_key_alias,
                user_api_team,
        ...
        if api_call_total_time_seconds is not None:
            ...
            self.llm_api_latency_metric.labels(**_labels).observe(
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            self.llm_api_failed_requests_metric.labels(
                end_user_id,
                user_api_key,
                user_api_key_alias,
                model,
    ...
    async def _initialize_team_budget_metrics(self):
        ...
        ) -> Tuple[List[LLM_TeamTable], Optional[int]]:
    ...
    async def _set_team_list_budget_metrics(self, teams: List[LLM_TeamTable]):
    ...
    async def _set_team_budget_metrics_after_api_request(
        ...
        - Assemble a LLM_TeamTable object
