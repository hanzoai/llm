...
def test_get_llm_provider():
    _, response, _, _ = llm.get_llm_provider(model="anthropic.claude-v2:1")
...
def test_get_llm_provider_fireworks():  # tests finetuned fireworks models - https://github.com/hanzoai/llm/issues/4923
...
def test_get_llm_provider_catch_all():
    _, response, _, _ = llm.get_llm_provider(model="*")
    assert response == "openai"


def test_get_llm_provider_gpt_instruct():
    _, response, _, _ = llm.get_llm_provider(model="gpt-3.5-turbo-instruct-0914")
...
def test_default_api_base():
    ...
    with patch.dict(os.environ, {}, clear=True):
        for provider in llm.openai_compatible_providers:
            ...
            for other_provider in llm.provider_list:
