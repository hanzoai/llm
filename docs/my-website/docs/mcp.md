...
Use Model Context Protocol with LLM


<Image 
  img={require('../img/llm_mcp.png')}
  style={{width: '100%', display: 'block', margin: '2rem auto'}}
/>
<p style={{textAlign: 'left', color: '#666'}}>
  LLM MCP Architecture: Use MCP tools with all LLM supported models
</p>


## Overview

LLM acts as a MCP bridge to utilize MCP tools with all LLM supported models. LLM offers the following features for using MCP

- **List** Available MCP Tools: OpenAI clients can view all available MCP tools
  - `llm.experimental_mcp_client.load_mcp_tools` to list all available MCP tools
- **Call** MCP Tools: OpenAI clients can call MCP tools
  - `llm.experimental_mcp_client.call_openai_tool` to call an OpenAI tool on an MCP server
...
In this example we'll use `llm.experimental_mcp_client.load_mcp_tools` to list all available MCP tools on any MCP server. This method can be used in two ways:
...
<Tabs>
<TabItem value="sdk" label="LLM Python SDK">
...
<TabItem value="openai" label="OpenAI SDK + LLM Proxy">

In this example we'll walk through how you can use the OpenAI SDK pointed to the LLM proxy to call MCP tools. The key difference here is we use the OpenAI SDK to make the LLM API request
...
async with stdio_client(server_params) as (read, write):
    async with ClientSession(read, write) as session:
        ...
        # Get tools using llm mcp client
        tools = await experimental_mcp_client.load_mcp_tools(session=session, format="openai")
        print("MCP TOOLS: ", tools)

        # Use OpenAI SDK pointed to LLM proxy
        client = OpenAI(
            api_key="your-api-key",  # Your LLM proxy API key
            base_url="http://localhost:4000"  # Your LLM proxy URL
        )
...
- `llm.experimental_mcp_client.load_mcp_tools` to list all available MCP tools on any MCP server
- `llm.experimental_mcp_client.call_openai_tool` to call an OpenAI tool on an MCP server

The first llm response returns a list of OpenAI tools. We take the first tool call from the LLM response and pass it to `llm.experimental_mcp_client.call_openai_tool` to call the tool on the MCP server.

#### How `llm.experimental_mcp_client.call_openai_tool` works
...
<TabItem value="proxy" label="OpenAI SDK + LLM Proxy">
...
async with stdio_client(server_params) as (read, write):
    async with ClientSession(read, write) as session:
        ...
        client = OpenAI(
            api_key="your-api-key",  # Your LLM proxy API key
            base_url="http://localhost:8000"  # Your LLM proxy URL
        )
...
### Expose MCP tools on LLM Proxy Server

This allows you to define tools that can be called by any MCP compatible client. Define your mcp_tools with LLM and all your clients can list and call available tools.

#### How it works

LLM exposes the following MCP endpoints:

- `/mcp/list_tools` - List all available tools
- `/mcp/call_tool` - Call a specific tool with the provided arguments

When MCP clients connect to LLM they can follow this workflow:

1. Connect to the LLM MCP server
2. List all available tools on LLM
3. Client makes LLM API request with tool call(s)
4. LLM API returns which tools to call and with what arguments
5. MCP client makes tool calls to LLM
6. LLM makes the tool calls to the appropriate handlers
7. LLM returns the tool call results to the MCP client

#### Usage

#### 1. Define your tools on mcp_tools

LLM allows you to define your tools on the `mcp_tools` section in your config.yaml file. All tools listed here will be available to MCP clients (when they connect to LLM and call `list_tools`).
...
Mount your `mcp_tools.py` on the LLM Docker container.
...
The `mcp_tools` section in your LLM config defines tools that can be called by MCP-compatible clients.
