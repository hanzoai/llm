    ...
    try:
        ...
        if custom_llm_provider == "openai":
            ...
            response = openai_fine_tuning_apis_instance.create_fine_tuning_job(
                ...
                client=kwargs.get(
                    "client", None
                ),  # note, when we add this to `GenericLLMParams` it impacts a lot of other tests + linting
            )
        # Azure OpenAI
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            raise llm.exceptions.BadRequestError(
                message="LLM doesn't support {} for 'retrieve_fine_tuning_job'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
