...
class ResetBudgetJob:
    ...
    async def reset_budget_for_llm_keys(self):
        ...
        keys_to_reset: Optional[List[LLM_VerificationToken]] = None
        try:
            ...
            updated_keys: List[LLM_VerificationToken] = []
            failed_keys = []
    ...
    async def reset_budget_for_llm_users(self):
        """
        Resets the budget for all LLM Internal Users if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        users_to_reset: Optional[List[LLM_UserTable]] = None
        try:
    ...
    async def reset_budget_for_llm_teams(self):
        """
        Resets the budget for all LLM Internal Teams if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        teams_to_reset: Optional[List[LLM_TeamTable]] = None
        try:
            teams_to_reset = await self.prisma_client.get_data(
                table_name="team", query_type="find_all", reset_at=now
            )
            updated_teams: List[LLM_TeamTable] = []
            failed_teams = []
    ...
    async def _reset_budget_common(
        item: Union[LLM_TeamTable, LLM_UserTable, LLM_VerificationToken],
    ...
    async def _reset_budget_for_team(
        team: LLM_TeamTable, current_time: datetime
    ) -> Optional[LLM_TeamTable]:
    ...
    async def _reset_budget_for_user(
        user: LLM_UserTable, current_time: datetime
    ...
    async def _reset_budget_for_key(
        key: LLM_VerificationToken, current_time: datetime
