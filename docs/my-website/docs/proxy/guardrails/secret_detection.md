...
You can also see the raw request sent from llm to the API Provider with (`--detailed_debug`).
```json
...
Hanzo uses the [`detect-secrets`](https://github.com/Yelp/detect-secrets) library for secret detection. See [all plugins run by default](#default-config-used)
