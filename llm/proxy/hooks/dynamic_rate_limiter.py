...
import llm
from llm import ModelResponse, Router
    ...
    async def async_set_cache_sadd(self, model: str, value: List):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "llm.proxy.hooks.dynamic_rate_limiter.py::async_set_cache_sadd(): Exception occured - {}".format(
                    str(e)
                )
            )
            raise e
...
class _PROXY_DynamicRateLimitHandler(CustomLogger):
    ...
    async def check_available_usage(
        ...
        try:
            weight: float = 1
            if (
                llm.priority_reservation is None
                or priority not in llm.priority_reservation
            ):
                verbose_proxy_logger.error(
                    "Priority Reservation not set. priority={}, but llm.priority_reservation is {}.".format(
                        priority, llm.priority_reservation
                    )
                )
            ...
            verbose_proxy_logger.exception(
                "llm.proxy.hooks.dynamic_rate_limiter.py::check_available_usage: Exception occurred - {}".format(
                    str(e)
                )
            )
    ...
    async def async_post_call_success_hook(
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "llm.proxy.hooks.dynamic_rate_limiter.py::async_post_call_success_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            return response
