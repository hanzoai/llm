...
async def unblock_user(data: BlockUsers):
    ...
    if (
        not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in llm.callbacks)
        or llm.blocked_user_list is None
    ):
        raise HTTPException(
            status_code=400,
            detail={
    ...
    if isinstance(llm.blocked_user_list, list):
        for id in data.user_ids:
            llm.blocked_user_list.remove(id)
    else:
        raise HTTPException(
            status_code=500,
            detail={
    ...
    return {"blocked_users": llm.blocked_user_list}
...
async def new_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=Hanzo_EndUserTable,
)
@router.get(
    "/end_user/info",
    tags=["Customer Management"],
...
async def update_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.update_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def delete_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "llm.proxy.proxy_server.delete_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[Hanzo_EndUserTable],
)
@router.get(
    "/end_user/list",
    tags=["Customer Management"],
...
async def list_end_user(
    ...
    returned_response: List[Hanzo_EndUserTable] = []
    for item in response:
        returned_response.append(Hanzo_EndUserTable(**item.model_dump()))
