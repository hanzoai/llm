...
Use this if you want LLM to validate your JWT against multiple OIDC providers (e.g. Google Cloud, GitHub Auth)
...
- Validate if user id is in the DB (LLM_UserTable)
- Validate if any of the groups are in the DB (LLM_TeamTable)
...
Validate a JWT Token using custom logic, if you need an extra way to verify if tokens are valid for LLM Proxy.
...
[**See Code**](https://github.com/hanzoai/llm/blob/b204f0c01c703317d812a1553363ab0cb989d5b6/llm/proxy/_types.py#L95)
...
environment_variables:
  JWT_AUDIENCE: "api://LLM_Proxy" # ensures audience is validated
```
...
- `role_mappings`: A list of role mappings. Map the received role in the JWT token to an internal role on LLM.
...
- `internal_role`: The internal role on LLM that will be used to control access. 
