...
Hanzo supports **reading secrets (eg. `OPENAI_API_KEY`)** and **writing secrets (eg. Virtual Keys)** from Azure Key Vault, Google Secret Manager, Hashicorp Vault, and AWS Secret Manager.
...
Hanzo supports two methods of authentication:
...
**Reading Secrets**
Hanzo reads secrets from Hashicorp Vault's KV v2 engine using the following URL format:
```
...
Hanzo expects all secrets to be stored as a JSON object with a `key` field containing the secret value.
...
When a Virtual Key is Created / Deleted on Hanzo, Hanzo will automatically create / delete the secret in Hashicorp Vault.

- Create Virtual Key on Hanzo either through the Hanzo Admin UI or API
...
Hanzo stores secret under the `prefix_for_stored_virtual_keys` path (default: `llm/`)
