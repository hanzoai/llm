...
class HanzoTeamRoles(enum.Enum):
    # team admin
    TEAM_ADMIN = "admin"
    # team member
    TEAM_MEMBER = "user"
...
class LLMTableNames(str, enum.Enum):
    """
    Enum for Table Names used by Hanzo
    """

    TEAM_TABLE_NAME = "Hanzo_TeamTable"
    USER_TABLE_NAME = "Hanzo_UserTable"
    KEY_TABLE_NAME = "Hanzo_VerificationToken"
    PROXY_MODEL_TABLE_NAME = "Hanzo_ProxyModelTable"
...
class Hanzo_UpperboundKeyGenerateParams(HanzoPydanticObjectBase):
    """
...
class HanzoPromptInjectionParams(HanzoPydanticObjectBase):
...
class ProxyChatCompletionRequest(HanzoPydanticObjectBase):
    model: str
...
class ModelInfoDelete(HanzoPydanticObjectBase):
    id: str


class ModelInfo(HanzoPydanticObjectBase):
    id: Optional[str]
...
class ProviderInfo(HanzoPydanticObjectBase):
    name: str
    fields: List[ProviderField]


class BlockUsers(HanzoPydanticObjectBase):
    user_ids: List[str]  # required


class ModelParams(HanzoPydanticObjectBase):
    model_name: str
    llm_params: dict
...
class GenerateRequestBase(HanzoPydanticObjectBase):
    """
...
class KeyRequest(HanzoPydanticObjectBase):
...
class Hanzo_ModelTable(HanzoPydanticObjectBase):
...
class Hanzo_ProxyModelTable(HanzoPydanticObjectBase):
    model_id: str
    model_name: str
    llm_params: dict
    model_info: dict
...
class DeleteUserRequest(HanzoPydanticObjectBase):
    user_ids: List[str]  # required
...
class BudgetNewRequest(HanzoPydanticObjectBase):
...
class BudgetRequest(HanzoPydanticObjectBase):
    budgets: List[str]


class BudgetDeleteRequest(HanzoPydanticObjectBase):
    id: str


class CustomerBase(HanzoPydanticObjectBase):
    user_id: str
...
class UpdateCustomerRequest(HanzoPydanticObjectBase):
    """
...
class DeleteCustomerRequest(HanzoPydanticObjectBase):
    """
...
class MemberBase(HanzoPydanticObjectBase):
    user_id: Optional[str] = None
    user_email: Optional[str] = None
...
class TeamBase(HanzoPydanticObjectBase):
...
class GlobalEndUsersSpend(HanzoPydanticObjectBase):
    api_key: Optional[str] = None
    startTime: Optional[datetime] = None
    endTime: Optional[datetime] = None


class UpdateTeamRequest(HanzoPydanticObjectBase):
    """
...
class ResetTeamBudgetRequest(HanzoPydanticObjectBase):
    """
...
class DeleteTeamRequest(HanzoPydanticObjectBase):
    team_ids: List[str]  # required


class BlockTeamRequest(HanzoPydanticObjectBase):
    team_id: str  # required


class BlockKeyRequest(HanzoPydanticObjectBase):
    key: str  # required


class AddTeamCallback(HanzoPydanticObjectBase):
    callback_name: str
...
class TeamCallbackMetadata(HanzoPydanticObjectBase):
...
class Hanzo_TeamTable(TeamBase):
    ...
    llm_model_table: Optional[Hanzo_ModelTable] = None
...
class Hanzo_TeamTableCachedObj(Hanzo_TeamTable):
    last_refreshed_at: Optional[float] = None


class TeamRequest(HanzoPydanticObjectBase):
    teams: List[str]


class Hanzo_BudgetTable(HanzoPydanticObjectBase):
...
class Hanzo_TeamMemberTable(Hanzo_BudgetTable):
    """
...
class NewOrganizationRequest(Hanzo_BudgetTable):
    organization_id: Optional[str] = None
...
class OrganizationRequest(HanzoPydanticObjectBase):
    organizations: List[str]


class DeleteOrganizationRequest(HanzoPydanticObjectBase):
...
class KeyManagementSettings(HanzoPydanticObjectBase):
...
class TeamDefaultSettings(HanzoPydanticObjectBase):
    team_id: str
...
class DynamoDBArgs(HanzoPydanticObjectBase):
    billing_mode: Literal["PROVISIONED_THROUGHPUT", "PAY_PER_REQUEST"]
    read_capacity_units: Optional[int] = None
    write_capacity_units: Optional[int] = None
    ssl_verify: Optional[bool] = None
    region_name: str
    user_table_name: str = "Hanzo_UserTable"
    key_table_name: str = "Hanzo_VerificationToken"
    config_table_name: str = "Hanzo_Config"
    spend_table_name: str = "Hanzo_SpendLogs"
...
class PassThroughGenericEndpoint(HanzoPydanticObjectBase):
    path: str = Field(description="The route to be added to the Hanzo Proxy Server.")
    target: str = Field(
...
class PassThroughEndpointResponse(HanzoPydanticObjectBase):
    endpoints: List[PassThroughGenericEndpoint]


class ConfigFieldUpdate(HanzoPydanticObjectBase):
    field_name: str
    field_value: Any
    config_type: Literal["general_settings"]


class ConfigFieldDelete(HanzoPydanticObjectBase):
    config_type: Literal["general_settings"]
    field_name: str
...
class ConfigList(HanzoPydanticObjectBase):
    field_name: str
    field_type: str
    field_description: str
    field_value: Any
...
class ConfigGeneralSettings(HanzoPydanticObjectBase):
    """
...
class ConfigYAML(HanzoPydanticObjectBase):
    """
...
class Hanzo_VerificationToken(HanzoPydanticObjectBase):
    token: Optional[str] = None
    key_name: Optional[str] = None
    key_alias: Optional[str] = None
    spend: float = 0.0
...
class Hanzo_VerificationTokenView(Hanzo_VerificationToken):
    """
...
class UserInfoResponse(HanzoPydanticObjectBase):
...
class Hanzo_Config(HanzoPydanticObjectBase):
    param_name: str
    param_value: Dict


class Hanzo_OrganizationMembershipTable(HanzoPydanticObjectBase):
    """
    ...
    llm_budget_table: Optional[Hanzo_BudgetTable] = None

    model_config = ConfigDict(protected_namespaces=())


class Hanzo_OrganizationTable(HanzoPydanticObjectBase):
    """Represents user-controllable params for a Hanzo_OrganizationTable record"""
...
class Hanzo_OrganizationTableUpdate(HanzoPydanticObjectBase):
    """Represents user-controllable params for a Hanzo_OrganizationTable record"""
...
class Hanzo_OrganizationTableWithMembers(Hanzo_OrganizationTable):
    """Returned by the /organization/info endpoint and /organization/list endpoint"""

    members: List[Hanzo_OrganizationMembershipTable] = []
    teams: List[Hanzo_TeamTable] = []
    llm_budget_table: Optional[Hanzo_BudgetTable] = None
    created_at: datetime
    updated_at: datetime


class NewOrganizationResponse(Hanzo_OrganizationTable):
    organization_id: str  # type: ignore
    created_at: datetime
    updated_at: datetime


class Hanzo_UserTable(HanzoPydanticObjectBase):
    user_id: str
    max_budget: Optional[float] = None
    spend: float = 0.0
    model_max_budget: Optional[Dict] = {}
    ...
    rpm_limit: Optional[int] = None
    user_role: Optional[str] = None
    organization_memberships: Optional[List[Hanzo_OrganizationMembershipTable]] = None
    teams: List[str] = []
...
class Hanzo_UserTableFiltered(BaseModel):  # done to avoid exposing sensitive data
    user_id: str
    user_email: str


class Hanzo_UserTableWithKeyCount(Hanzo_UserTable):
    key_count: int = 0


class Hanzo_EndUserTable(HanzoPydanticObjectBase):
    user_id: str
    blocked: bool
    alias: Optional[str] = None
    spend: float = 0.0
...
class Hanzo_SpendLogs(HanzoPydanticObjectBase):
    request_id: str
    api_key: str
    model: Optional[str] = ""
...
class Hanzo_ErrorLogs(HanzoPydanticObjectBase):
...
class Hanzo_AuditLogs(HanzoPydanticObjectBase):
    id: str
    updated_at: datetime
...
class Hanzo_SpendLogs_ResponseObject(HanzoPydanticObjectBase):
    response: Optional[List[Union[Hanzo_SpendLogs, Any]]] = None


class TokenCountRequest(HanzoPydanticObjectBase):
    model: str
    prompt: Optional[str] = None
    messages: Optional[List[dict]] = None


class TokenCountResponse(HanzoPydanticObjectBase):
    total_tokens: int
    request_model: str
    model_used: str
    tokenizer_type: str
...
class CallInfo(HanzoPydanticObjectBase):
...
class InvitationNew(HanzoPydanticObjectBase):
    user_id: str


class InvitationUpdate(HanzoPydanticObjectBase):
    invitation_id: str
    is_accepted: bool


class InvitationDelete(HanzoPydanticObjectBase):
    invitation_id: str


class InvitationModel(HanzoPydanticObjectBase):
    id: str
    user_id: str
    is_accepted: bool
...
class InvitationClaim(HanzoPydanticObjectBase):
    invitation_link: str
    user_id: str
    password: str


class ConfigFieldInfo(HanzoPydanticObjectBase):
    field_name: str
    field_value: Any


class CallbackOnUI(HanzoPydanticObjectBase):
    llm_callback_name: str
    llm_callback_params: Optional[list]
    ui_callback_name: str


class AllCallbacks(HanzoPydanticObjectBase):
...
class ManagementEndpointLoggingPayload(HanzoPydanticObjectBase):
    route: str
    request_data: dict
...
class ProxyException(Exception):
    ...
    def __init__(
        ...
        # Related Hanzo issue: https://github.com/BerriAI/llm/discussions/4834
        self.code = str(code)
...
class CommonProxyErrors(str, enum.Enum):
    ...
    not_premium_user = "You must be a Hanzo Enterprise user to use this feature. If you have a license please set `LLM_LICENSE` in your env. Get a 7 day trial key here: https://www.llm.ai/#trial. \nPricing: https://www.llm.ai/#pricing"
    max_parallel_request_limit_reached = (
        "Crossed TPM / RPM / Max Parallel Request Limit"
    )


class SpendCalculateRequest(HanzoPydanticObjectBase):
...
class VirtualKeyEvent(HanzoPydanticObjectBase):
...
class CreatePassThroughEndpoint(HanzoPydanticObjectBase):
    path: str
    target: str
    headers: dict


class Hanzo_TeamMembership(HanzoPydanticObjectBase):
    user_id: str
    team_id: str
    budget_id: str
    llm_budget_table: Optional[Hanzo_BudgetTable]


#### Organization / Team Member Requests ####


class MemberAddRequest(HanzoPydanticObjectBase):
...
class OrgMemberAddRequest(HanzoPydanticObjectBase):
...
class TeamAddMemberResponse(Hanzo_TeamTable):
    updated_users: List[Hanzo_UserTable]
    updated_team_memberships: List[Hanzo_TeamMembership]


class OrganizationAddMemberResponse(HanzoPydanticObjectBase):
    organization_id: str
    updated_users: List[Hanzo_UserTable]
    updated_organization_memberships: List[Hanzo_OrganizationMembershipTable]


class MemberDeleteRequest(HanzoPydanticObjectBase):
    user_id: Optional[str] = None
    user_email: Optional[str] = None
...
class MemberUpdateResponse(HanzoPydanticObjectBase):
    user_id: str
    user_email: Optional[str] = None
...
class TeamInfoResponseObject(TypedDict):
    team_id: str
    team_info: Hanzo_TeamTable
    keys: List
    team_memberships: List[Hanzo_TeamMembership]


class TeamListResponseObject(Hanzo_TeamTable):
    team_memberships: List[Hanzo_TeamMembership]
...
Hanzo_ManagementEndpoint_MetadataFields = [
    "model_rpm_limit",
    "model_tpm_limit",
    "guardrails",
    "tags",
...
Hanzo_ManagementEndpoint_MetadataFields_Premium = [
    "guardrails",
    "tags",
]


class ProviderBudgetResponseObject(HanzoPydanticObjectBase):
    """
...
class ProviderBudgetResponse(HanzoPydanticObjectBase):
    """
...
class JWTAuthBuilderResult(TypedDict):
    is_proxy_admin: bool
    team_object: Optional[Hanzo_TeamTable]
    user_object: Optional[Hanzo_UserTable]
    end_user_object: Optional[Hanzo_EndUserTable]
    org_object: Optional[Hanzo_OrganizationTable]
    token: str
    team_id: Optional[str]
    user_id: Optional[str]
...
class OIDCPermissions(HanzoPydanticObjectBase):
    models: Optional[List[str]] = None
...
class Hanzo_JWTAuth(HanzoPydanticObjectBase):
    """
    A class to define the roles and permissions for a Hanzo Proxy w/ JWT Auth.
    ...
    - user_id_jwt_field: The field in the JWT token that stores the user id (maps to `HanzoUserTable`). Use this for internal employees.
    - user_email_jwt_field: The field in the JWT token that stores the user email (maps to `HanzoUserTable`). Use this for internal employees.
    - user_allowed_email_subdomain: If specified, only emails from specified subdomain will be allowed to access proxy.
    - end_user_id_jwt_field: The field in the JWT token that stores the end-user ID (maps to `HanzoEndUserTable`). Turn this off by setting to `None`. Enables end-user cost tracking. Use this for external customers.
