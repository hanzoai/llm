...
import llm
from llm import ModelResponse, token_counter, verbose_logger
...
class LowestCostLoggingHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["llm_params"].get("metadata") is None:
                ...
                model_group = kwargs["llm_params"]["metadata"].get(
                    "model_group", None
                )

                id = kwargs["llm_params"].get("model_info", {}).get("id", None)
                ...
                if isinstance(response_obj, ModelResponse):
                    ...
                    if _usage is not None and isinstance(_usage, llm.Usage):
            ...
            verbose_logger.exception(
                "llm.router_strategy.lowest_cost.py::log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_logger.exception(
                "llm.proxy.hooks.prompt_injection_detection.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_get_available_deployments(  # noqa: PLR0915
        ...
        for item, item_map in all_deployments.items():
            ...
            _deployment_tpm = (
                ...
                or _deployment.get("llm_params", {}).get("tpm", None)
            ...
            _deployment_rpm = (
                ...
                or _deployment.get("llm_params", {}).get("rpm", None)
            ...
            # check if user provided input_cost_per_token and output_cost_per_token in llm_params
            item_input_cost = None
            item_output_cost = None
            if _deployment.get("llm_params", {}).get("input_cost_per_token", None):
                item_input_cost = _deployment.get("llm_params", {}).get(
                    "input_cost_per_token"
                )

            if _deployment.get("llm_params", {}).get("output_cost_per_token", None):
                item_output_cost = _deployment.get("llm_params", {}).get(
            ...
            _deployment_api_base = _deployment.get("llm_params", {}).get(
                "api_base", ""
            )
