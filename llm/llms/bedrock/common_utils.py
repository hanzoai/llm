...
import llm
from llm.llms.base_llm.base_utils import BaseLLMModelInfo
from llm.llms.base_llm.chat.transformation import BaseLLMException
...
def init_bedrock_client(
    ...
    ssl_verify = os.getenv("SSL_VERIFY", llm.ssl_verify)
    ...
    elif llm_aws_region_name:
        region_name = llm_aws_region_name
    ...
    if (
        ...
        # aws_access_key_id is not None, assume user is trying to auth using llm.completion
...
def get_bedrock_tool_name(response_tool_name: str) -> str:
    """
    If llm formatted the input tool name, we need to convert it back to the original name.
    ...
    if response_tool_name in llm.bedrock_tool_name_mappings.cache_dict:
        response_tool_name = llm.bedrock_tool_name_mappings.cache_dict[
            response_tool_name
        ]
...
class BedrockModelInfo(BaseLLMModelInfo):
    ...
    def get_bedrock_route(model: str) -> Literal["converse", "invoke", "converse_like"]:
        ...
        if "invoke/" in model:
            ...
            base_model in llm.bedrock_converse_models
            or alt_model in llm.bedrock_converse_models
        ):
            return "converse"
        return "invoke"
