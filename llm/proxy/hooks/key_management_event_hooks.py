...
class KeyManagementEventHooks:
    ...
    async def async_key_generated_hook(
        ...
        """
        from llm.proxy.management_helpers.audit_logs import (
            create_audit_log_for_update,
        )
        from llm.proxy.proxy_server import llm_proxy_admin_name
    ...
    async def async_key_deleted_hook(
        data: KeyRequest,
        keys_being_deleted: List[Hanzo_VerificationToken],
        response: dict,
        user_api_key_dict: UserAPIKeyAuth,
        llm_changed_by: Optional[str] = None,
    ):
        ...
        if llm.store_audit_logs is True and data.keys is not None:
    ...
    async def _store_virtual_key_in_secret_manager(secret_name: str, secret_token: str):
        ...
        """
        if llm._key_management_settings is not None:
            if llm._key_management_settings.store_virtual_keys is True:
                from llm.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )

                # store the key in the secret manager
                if isinstance(llm.secret_manager_client, BaseSecretManager):
                    await llm.secret_manager_client.async_write_secret(
    ...
    async def _rotate_virtual_key_in_secret_manager(
        ...
        if llm._key_management_settings is not None:
            if llm._key_management_settings.store_virtual_keys is True:
                ...
                # store the key in the secret manager
                if isinstance(llm.secret_manager_client, BaseSecretManager):
                    await llm.secret_manager_client.async_rotate_secret(
    ...
    def _get_secret_name(secret_name: str) -> str:
        if llm._key_management_settings.prefix_for_stored_virtual_keys.endswith(
            "/"
        ):
            return f"{llm._key_management_settings.prefix_for_stored_virtual_keys}{secret_name}"
        else:
            return f"{llm._key_management_settings.prefix_for_stored_virtual_keys}/{secret_name}"
                ...
                if isinstance(llm.secret_manager_client, BaseSecretManager):
                    for key in keys_being_deleted:
                        if key.key_alias is not None:
                            await llm.secret_manager_client.async_delete_secret(
                                secret_name=KeyManagementEventHooks._get_secret_name(
                                    key.key_alias
                                )
                            )
