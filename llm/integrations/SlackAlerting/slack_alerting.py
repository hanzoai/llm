...
from llm.integrations.custom_batch_logger import CustomBatchLogger
...
from llm.proxy._types import AlertType, CallInfo, VirtualKeyEvent, WebhookEvent
from llm.types.integrations.slack_alerting import *
...
class SlackAlerting(CustomBatchLogger):
    ...
    def _response_taking_too_long_callback_helper(
        ...
        try:
            ...
            api_base = llm.get_api_base(model=model, optional_params=llm_params)
    ...
    async def response_taking_too_long_callback(
        ...
        if llm.turn_off_message_logging or llm.redact_messages_in_exceptions:
            messages = "Message not logged. llm.redact_messages_in_exceptions=True"
        request_info = f"\nRequest Model: `{model}`\nAPI Base: `{api_base}`\nMessages: `{messages}`"
        ...
        if time_difference_float > self.alerting_threshold:
            ...
            if (
                kwargs is not None
                and "llm_params" in kwargs
                and "metadata" in kwargs["llm_params"]
            ):
                _metadata: dict = kwargs["llm_params"]["metadata"]
    ...
    async def send_daily_reports(self, router) -> bool:  # noqa: PLR0915
        ...
        # format alert -> return the llm model name + api base
        ...
        for i in range(len(top_5_failed)):
            ...
            if isinstance(_deployment, dict):
                deployment_name = _deployment["llm_params"].get("model", "")
            else:
                return False

            api_base = llm.get_api_base(
                model=deployment_name,
                optional_params=(
                    _deployment["llm_params"] if _deployment is not None else {}
                ),
            )
            if api_base is None:
                api_base = ""
    ...
    async def response_taking_too_long(
        ...
        if request_data is not None:
            ...
            if (
                llm.turn_off_message_logging
                or llm.redact_messages_in_exceptions
            ):
                messages = (
                    "Message not logged. llm.redact_messages_in_exceptions=True"
                )
            request_info = f"\nRequest Model: `{model}`\nMessages: `{messages}`"
        else:
            request_info = ""
...
"""
    ...
    async def region_outage_alerts(
        ...
        ### GET PROVIDER ###
        provider = deployment.llm_params.custom_llm_provider
        if provider is None:
            model, provider, _, _ = llm.get_llm_provider(model=model)

        ### GET REGION ###
        region_name = deployment.llm_params.region_name
        if region_name is None:
            region_name = llm.utils._get_model_region(
                custom_llm_provider=provider, llm_params=deployment.llm_params
            )
        ...
        try:
            ...
            if provider is None:
                try:
                    ...
                    provider = ""
            api_base = llm.get_api_base(
                model=model, optional_params=deployment.llm_params
            )
    ...
    async def model_added_alert(
        self, model_name: str, llm_model_name: str, passed_model_info: Any
    ):
        base_model_from_user = getattr(passed_model_info, "base_model", None)
        model_info = {}
        base_model = ""
        if base_model_from_user is not None:
            model_info = llm.model_cost.get(base_model_from_user, {})
            base_model = f"Base Model: `{base_model_from_user}`\n"
        else:
            model_info = llm.model_cost.get(llm_model_name, {})
        model_info_str = ""
...
"""
    ...
    async def send_key_created_or_user_invited_email(
        ...
        try:
            from llm.proxy.utils import send_email
            ...
            if email_logo_url is None:
                email_logo_url = LLM_LOGO_URL
            if email_support_contact is None:
                email_support_contact = LLM_SUPPORT_CONTACT
            ...
            email_html_content = "Alert from Hanzo Server"
    ...
    async def send_email_alert_using_smtp(
        ...
        if webhook_event.event == "budget_crossed":
            ...
            <img src="{email_logo_url}" alt="Hanzo Logo" width="150" height="50" />
        ...
        if webhook_event.event_group == "team":
            from llm.integrations.email_alerting import send_team_budget_alert
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            if "daily_reports" in self.alert_types:
                ...
                model_info = llm_params.get("model_info", {}) or {}
                model_id = model_info.get("id", "") or ""
                response_s: timedelta = end_time - start_time

                final_value = response_s

                if isinstance(response_obj, llm.ModelResponse) and (
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _llm_params = kwargs.get("llm_params", {})
        _model_info = _llm_params.get("model_info", {}) or {}
    ...
    async def _request_is_completed(self, request_data: Optional[dict]) -> bool:
        ...
        if (
            ...
            status: Optional[str] = await self.internal_usage_cache.async_get_cache(
                key="request_status:{}".format(llm_call_id), local_only=True
            )
