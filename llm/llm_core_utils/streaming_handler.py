...
from llm.llm_core_utils.redact_messages import LLMLoggingObject
from llm.llm_core_utils.thread_pool_executor import executor
from llm.types.llms.openai import ChatCompletionChunk
...
class CustomStreamWrapper:
    def __init__(
        ...
        self.logging_obj: LLMLoggingObject = logging_obj
        self.completion_stream = completion_stream
        self.sent_first_chunk = False
        self.sent_last_chunk = False

        llm_params: GenericLLMParams = GenericLLMParams(
            **self.logging_obj.model_call_details.get("llm_params", {})
        )
        self.merge_reasoning_content_in_choices: bool = (
            llm_params.merge_reasoning_content_in_choices or False
        )
        ...
        _model_info: Dict = llm_params.model_info or {}
        ...
        self._hidden_params = {
            ...
            "api_base": _api_base,
        }  # returned as x-llm-model-id response header in proxy
    ...
    def safety_checker(self) -> None:
        """
        Fixes - https://github.com/hanzoai/llm/issues/5158
        ...
        """
        if len(self.chunks) >= llm.REPEATED_STREAMING_CHUNK_LIMIT:
            # Get the last n chunks
            last_chunks = self.chunks[-llm.REPEATED_STREAMING_CHUNK_LIMIT :]
            ...
            if all(content == last_contents[0] for content in last_contents):
                ...
                ):  # ignore empty content - https://github.com/hanzoai/llm/issues/5158#issuecomment-2287156946
    ...
    def handle_baseten_chunk(self, chunk):
        try:
            ...
            verbose_logger.exception(
                "llm.CustomStreamWrapper.handle_baseten_chunk(): Exception occured - {}".format(
                    str(e)
                )
            )
            return ""
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore  # noqa: PLR0915
        ...
        try:
            ...
            if (
                ...
                and self.custom_llm_provider in llm._custom_providers
            ):
                ...
                if anthropic_response_obj["usage"] is not None:
                    model_response.usage = llm.Usage(
                ...
                response_obj = cast(
                    Dict[str, Any],
                    llm.CodestralTextCompletionConfig()._chunk_parser(chunk),
                )
                completion_obj["content"] = response_obj["text"]
                print_verbose(f"completion obj content: {completion_obj['content']}")
                if response_obj["is_finished"]:
                ...
                if response_obj["usage"] is not None:
                    if isinstance(response_obj["usage"], dict):
                        ...
                        setattr(
                            model_response,
                            "usage",
                            llm.Usage(**response_obj["usage"].model_dump()),
                        )
                ...
                if original_chunk.choices and len(original_chunk.choices) > 0:
                    ...
                    if delta is not None and (
                        ...
                        try:
                            ...
                            verbose_logger.exception(
                                "llm.CustomStreamWrapper.chunk_creator(): Exception occured - {}".format(
                                    str(e)
                                )
                            )
                            model_response.choices[0].delta = Delta()
    ...
    def set_logging_event_loop(self, loop):
        ...
        response = llm.completion(.., stream=True)
    ...
    def run_success_logging_and_cache_storage(self, processed_chunk, cache_hit: bool):
        ...
        """
        if llm.disable_streaming_logging is True:
            """
            [NOT RECOMMENDED]
            Set this via `llm.disable_streaming_logging = True`.
    ...
    def fetch_sync_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = self.make_call(client=llm.module_level_client)
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = await self.make_call(
                client=llm.module_level_aclient
            )
