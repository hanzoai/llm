...
def get_api_base(
    model: str, optional_params: Union[dict, Hanzo_Params]
) -> Optional[str]:
    """
    ...
    - optional_params - the 'llm_params' in router.completion *OR* additional params passed to llm.completion - eg. api_base, api_key, etc. See `Hanzo_Params` - https://github.com/BerriAI/llm/blob/f09e6ba98d65e035a79f73bc069145002ceafd36/llm/router.py#L67
    ...
    try:
        if isinstance(optional_params, Hanzo_Params):
            _optional_params = optional_params
        elif "model" in optional_params:
            _optional_params = Hanzo_Params(**optional_params)
        else:  # prevent needing to copy and pop the dict
            _optional_params = Hanzo_Params(
