...
from llm.llms.bedrock.chat.invoke_handler import make_call, make_sync_call
from llm.llms.bedrock.common_utils import BedrockError
...
from llm.types.utils import ModelResponse, Usage
...
class AmazonInvokeConfig(BaseConfig, BaseAWSLLM):
    ...
    def transform_request(
        ...
        custom_prompt_dict: dict = llm_params.pop("custom_prompt_dict", None) or {}
        hf_model_name = llm_params.get("hf_model_name", None)
        ...
        if provider == "cohere":
            ...
            return llm.AmazonAnthropicClaude3Config().transform_request(
                model=model,
                messages=messages,
                optional_params=optional_params,
            ...
            return llm.AmazonInvokeNovaConfig().transform_request(
                model=model,
                messages=messages,
                optional_params=optional_params,
                llm_params=llm_params,
    ...
    def transform_response(  # noqa: PLR0915
        ...
        try:
            if provider == "cohere":
                ...
                return llm.AmazonAnthropicClaude3Config().transform_response(
                    model=model,
                ...
                return llm.AmazonInvokeNovaConfig().transform_response(
                    model=model,
                    raw_response=raw_response,
                    model_response=model_response,
                    logging_obj=logging_obj,
    ...
    def get_bedrock_invoke_provider(
        ...
        if _split_model in get_args(llm.BEDROCK_INVOKE_PROVIDERS_LITERAL):
            return cast(llm.BEDROCK_INVOKE_PROVIDERS_LITERAL, _split_model)
        ...
        for provider in get_args(llm.BEDROCK_INVOKE_PROVIDERS_LITERAL):
