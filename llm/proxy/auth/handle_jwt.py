...
JWT token must have 'llm_proxy_admin' in scope. 
"""
...
from llm.llm_core_utils.dot_notation_indexing import get_nested_value
...
from llm.proxy.auth.auth_checks import can_team_access_model
...
class JWTHandler:
    ...
    - if role="llm_proxy_user" -> allow making calls + info. Can not edit budgets
    """
    ...
    def get_rbac_role(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        Resolves: https://github.com/hanzoai/llm/issues/6793
...
class JWTAuthManager:
    ...
    async def find_and_validate_specific_team_id(
    ...
    ) -> Tuple[Optional[str], Optional[LLM_TeamTable]]:
        ...
        team_object: Optional[LLM_TeamTable] = None
    ...
    async def get_objects(
        ...
        org_object: Optional[LLM_OrganizationTable] = None
        if org_id:
            org_object = (
        ...
        end_user_object: Optional[LLM_EndUserTable] = None
        if end_user_id:
            end_user_object = (
    ...
    async def auth_builder(
        ...
        if rbac_role and object_id:
            ...
            if rbac_role == LLMUserRoles.TEAM:
                team_id = object_id
            elif rbac_role == LLMUserRoles.INTERNAL_USER:
                user_id = object_id
