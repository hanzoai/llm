...
from llm.types.caching import *
from llm.types.utils import all_llm_params
...
#### Hanzo.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: Optional[HanzoCacheType] = HanzoCacheType.LOCAL,
        mode: Optional[
            CacheMode
        ...
        **kwargs,
        ...
        Returns:
            None. Cache is set as a llm param
        """
        if type == HanzoCacheType.REDIS:
        ...
        elif type == HanzoCacheType.REDIS_SEMANTIC:
        ...
        elif type == HanzoCacheType.QDRANT_SEMANTIC:
        ...
        elif type == HanzoCacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == HanzoCacheType.S3:
            self.cache = S3Cache(
        ...
        if self.type == HanzoCacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == HanzoCacheType.REDIS
            or self.type == HanzoCacheType.REDIS_SEMANTIC
    ...
    def get_cache_key(self, **kwargs) -> str:
        ...
        Args:
            **kwargs: kwargs to llm.completion() or embedding()
    ...
    def _get_model_param_value(self, kwargs: dict) -> str:
        ...
        llm_params: Dict = kwargs.get("llm_params", {}) or {}
        metadata_in_llm_params: Dict = llm_params.get("metadata", {}) or {}
        model_group: Optional[str] = metadata.get(
            "model_group"
        ) or metadata_in_llm_params.get("model_group")
    ...
    def _get_file_param_value(self, kwargs: dict) -> str:
        ...
        return (
            ...
            or llm_params.get("file_name")
        )

    def _get_preset_cache_key_from_kwargs(self, **kwargs) -> Optional[str]:
        """
        Get the preset cache key from kwargs["llm_params"]
        ...
        if kwargs:
            if "llm_params" in kwargs:
                return kwargs["llm_params"].get("preset_cache_key", None)
        return None
    ...
    def _set_preset_cache_key_in_kwargs(self, preset_cache_key: str, **kwargs) -> None:
        ...
        Placed in kwargs["llm_params"]
        """
        if kwargs:
            if "llm_params" in kwargs:
                kwargs["llm_params"]["preset_cache_key"] = preset_cache_key
    ...
    def get_cache(self, **kwargs):
        ...
        Args:
            *args: args to llm.completion() or embedding()
            **kwargs: kwargs to llm.completion() or embedding()
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception(f"Hanzo Cache: Excepton add_cache: {str(e)}")
    ...
    def _supports_async(self) -> bool:
        ...
        """
        if self.type and self.type == HanzoCacheType.S3:
            return False
        return True
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("Hanzo: Enabling Cache")
    if "cache" not in llm.input_callback:
        llm.input_callback.append("cache")
    if "cache" not in llm.success_callback:
        llm.logging_callback_manager.add_llm_success_callback("cache")
    ...
    print_verbose(f"Hanzo: Cache enabled, llm.cache={llm.cache}")
    print_verbose(f"Hanzo Cache: {vars(llm.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    print_verbose("Hanzo: Updating Cache")
    llm.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"Hanzo: Cache Updated, llm.cache={llm.cache}")
    print_verbose(f"Hanzo Cache: {vars(llm.cache)}")


def disable_cache():
    """
    Disable the cache used by Hanzo.

    This function disables the cache used by the Hanzo module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the llm.cache attribute to None.
    ...
    print_verbose("Hanzo: Disabling Cache")
    with suppress(ValueError):
        llm.input_callback.remove("cache")
        llm.success_callback.remove("cache")
        llm._async_success_callback.remove("cache")

    llm.cache = None
    print_verbose(f"Hanzo: Cache disabled, llm.cache={llm.cache}")
