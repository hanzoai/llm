...
from llm.llm_core_utils.redact_messages import redact_user_api_key_info
from llm.llms.custom_httpx.http_handler import _get_httpx_client
from llm.secret_managers.main import str_to_bool
from llm.types.integrations.langfuse import *
...
class LangFuseLogger:
    ...
    def __init__(
        ...
        if Version(self.langfuse_sdk_version) >= Version("2.6.0"):
            parameters["sdk_integration"] = "llm"
    ...
    @staticmethod
    def add_metadata_from_header(llm_params: dict, metadata: dict) -> dict:
        """
        Adds metadata from proxy request headers to Langfuse logging if keys start with "langfuse_"
        and overwrites llm_params.metadata if already included.
        ...
        if llm_params.get("proxy_server_request") is None:
            return metadata
        ...
        proxy_headers = (
            llm_params.get("proxy_server_request", {}).get("headers", {}) or {}
        )
    ...
    def log_event_on_langfuse(
        ...
        try:
            ...
            metadata = self.add_metadata_from_header(llm_params, metadata)
    ...
    def _get_langfuse_input_output_content(
        ...
        if (
            ...
            or isinstance(response_obj, llm.EmbeddingResponse)
        ):
            input = prompt
            output = None
            ...
            response_obj, llm.HttpxBinaryResponseContent
        ):
            input = prompt
            output = "speech-output"
        elif response_obj is not None and isinstance(
            response_obj, llm.TextCompletionResponse
        ):
            input = prompt
            output = self._get_text_completion_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(
            response_obj, llm.ImageResponse
        ):
            input = prompt
            output = response_obj.get("data", None)
        elif response_obj is not None and isinstance(
            response_obj, llm.TranscriptionResponse
        ):
            input = prompt
            output = response_obj.get("text", None)
        elif response_obj is not None and isinstance(
            response_obj, llm.RerankResponse
        ):
            input = prompt
        ...
        try:
            ...
            if isinstance(metadata, dict):
                for key, value in metadata.items():
                    # generate langfuse tags - Default Tags sent to Langfuse from Hanzo Proxy
                    if (
                        llm.langfuse_default_tags is not None
                        and isinstance(llm.langfuse_default_tags, list)
                        and key in llm.langfuse_default_tags
                    ):
            ...
            trace_id = clean_metadata.pop("trace_id", llm_call_id)
            ...
            if (
                llm.langfuse_default_tags is not None
                and isinstance(llm.langfuse_default_tags, list)
                and "proxy_base_url" in llm.langfuse_default_tags
            ):
            ...
            proxy_server_request = llm_params.get("proxy_server_request", None)
            ...
            if response_obj is not None:
                if (
                    ...
                    generation_id = llm.utils.get_logging_id(
    ...
    def add_default_langfuse_tags(self, tags, kwargs, metadata):
        """
        Helper function to add llm default langfuse tags
        ...
        """
        if llm.langfuse_default_tags is not None and isinstance(
            llm.langfuse_default_tags, list
        ):
            if "cache_hit" in llm.langfuse_default_tags:
                _cache_hit_value = kwargs.get("cache_hit", False)
                tags.append(f"cache_hit:{_cache_hit_value}")
            if "cache_key" in llm.langfuse_default_tags:
                _hidden_params = metadata.get("hidden_params", {}) or {}
                _cache_key = _hidden_params.get("cache_key", None)
                if _cache_key is None and llm.cache is not None:
                    # fallback to using "preset_cache_key"
                    _preset_cache_key = llm.cache._get_preset_cache_key_from_kwargs(
                        **kwargs
                    )
