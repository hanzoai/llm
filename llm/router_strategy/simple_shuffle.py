...
def simple_shuffle(
    ...
    weight = healthy_deployments[0].get("llm_params").get("weight", None)
    if weight is not None:
        # use weight-random pick if rpms provided
        weights = [m["llm_params"].get("weight", 0) for m in healthy_deployments]
    ...
    rpm = healthy_deployments[0].get("llm_params").get("rpm", None)
    if rpm is not None:
        # use weight-random pick if rpms provided
        rpms = [m["llm_params"].get("rpm", 0) for m in healthy_deployments]
        verbose_router_logger.debug(f"\nrpms {rpms}")
        total_rpm = sum(rpms)
        ...
        return deployment or deployment[0]
    ############## Check if we can do a RPM/TPM based weighted pick #################
    tpm = healthy_deployments[0].get("llm_params").get("tpm", None)
    if tpm is not None:
        # use weight-random pick if rpms provided
        tpms = [m["llm_params"].get("tpm", 0) for m in healthy_deployments]
        verbose_router_logger.debug(f"\ntpms {tpms}")
        total_tpm = sum(tpms)
