                ...
                if isinstance(value, list):
                    for item in value:
                        if "file_id" in item:
                            ...
                            if llm.drop_params is True:
                                pass
                            else:
                                raise llm.utils.UnsupportedParamsError(
                                    message="Azure doesn't support {}. To drop it from the call, set `llm.drop_params = True.".format(
                                        value
                                    ),
                                    status_code=400,
                                )
...
class AzureChatCompletion(BaseAzureLLM, BaseLLM):
    ...
    def completion(  # noqa: PLR0915
        ...
        try:
            ...
            if "gateway.ai.cloudflare.com" in api_base:
                ...
                data = llm.AzureOpenAIConfig().transform_request(
                    model=model,
                    messages=messages,
    ...
    def make_sync_azure_httpx_request(
        ...
        if client is None:
            ...
            sync_handler = HTTPHandler(**_params, client=llm.client_session)  # type: ignore
        else:
    ...
    def get_headers(
        ...
        client_session = llm.client_session or httpx.Client()
        if "gateway.ai.cloudflare.com" in api_base:
