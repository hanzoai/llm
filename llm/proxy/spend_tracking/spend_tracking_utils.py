...
from llm.llm_core_utils.core_helpers import get_llm_metadata_from_kwargs
from llm.proxy._types import SpendLogsMetadata, SpendLogsPayload
from llm.proxy.utils import PrismaClient, hash_token
...
def get_spend_logs_id(
    ...
    if call_type == "aretrieve_batch":
        ...
        id = cast(Optional[str], response_obj.get("id")) or cast(
            Optional[str], kwargs.get("llm_call_id")
        )
    return id
...
def get_logging_payload(  # noqa: PLR0915
    ...
    from llm.proxy.proxy_server import general_settings, master_key
    ...
    metadata = get_llm_metadata_from_kwargs(kwargs)
    metadata = _add_proxy_server_request_to_metadata(
        metadata=metadata, llm_params=llm_params
    )
    ...
    end_user_id = get_end_user_id_for_cost_tracking(llm_params)
    ...
    if api_key is not None and isinstance(api_key, str):
        ...
        if (
            ...
            api_key = "llm_proxy_master_key"  # use a known alias, if the user disabled storing master key in db
    ...
    if llm.cache is not None:
        cache_key = llm.cache.get_cache_key(**kwargs)
    else:
        cache_key = "Cache OFF"
...
def _add_proxy_server_request_to_metadata(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        _proxy_server_request = cast(
            Optional[dict], llm_params.get("proxy_server_request", {})
        )
