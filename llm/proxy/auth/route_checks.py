...
class RouteChecks:
    ...
    def non_proxy_admin_allowed_routes_check(
        ...
        _user_role: Optional[LLMUserRoles],
        route: str,
        request: Request,
        ...
        if RouteChecks.is_llm_api_route(route=route):
            ...
            route in HanzoRoutes.info_routes.value
            ...
            route in HanzoRoutes.global_spend_tracking_routes.value
            and getattr(valid_token, "permissions", None) is not None
            and "get_spend_routes" in getattr(valid_token, "permissions", [])
        ):

            pass
        elif _user_role == LLMUserRoles.PROXY_ADMIN_VIEW_ONLY.value:
            if RouteChecks.is_llm_api_route(route=route):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
            ...
            if RouteChecks.check_route_access(
                route=route, allowed_routes=HanzoRoutes.management_routes.value
            ):
            ...
            and RouteChecks.check_route_access(
                route=route, allowed_routes=HanzoRoutes.internal_user_routes.value
            )
        ):
            pass
            ...
            route=route, allowed_routes=HanzoRoutes.org_admin_allowed_routes.value
        ):
            pass
        elif (
            _user_role == LLMUserRoles.INTERNAL_USER_VIEW_ONLY.value
            and RouteChecks.check_route_access(
                route=route,
                allowed_routes=HanzoRoutes.internal_user_view_only_routes.value,
            )
        ):
            pass
        elif RouteChecks.check_route_access(
            route=route, allowed_routes=HanzoRoutes.self_managed_routes.value
    ...
    def is_llm_api_route(route: str) -> bool:
        ...
        if route in HanzoRoutes.openai_routes.value:
            return True

        if route in HanzoRoutes.anthropic_routes.value:
            return True

        # fuzzy match routes like "/v1/threads/thread_49EIN5QF32s4mH20M7GFKdlZ"
        # Check for routes with placeholders
        for openai_route in HanzoRoutes.openai_routes.value:
        ...
        for _llm_passthrough_route in HanzoRoutes.mapped_pass_through_routes.value:
