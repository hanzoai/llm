...
# In a real-world scenario, these would be instances of Hanzo_VerificationToken, Hanzo_UserTable, etc.
...
async def test_reset_budget_keys_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_llm_keys()
...
async def test_reset_budget_users_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_llm_users()
        await asyncio.sleep(0.1)
...
async def test_reset_budget_teams_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_llm_teams()
        await asyncio.sleep(0.1)
