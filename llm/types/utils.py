...
from ..llm_core_utils.core_helpers import map_finish_reason
...
class LLMPydanticObjectBase(BaseModel):
    """
...
class ModelInfo(ModelInfoBase, total=False):
    """
    Model info for a given model, this is information found in llm.model_prices_and_context_window.json
    """
...
class StreamingChoices(OpenAIObject):
    def __init__(
        ...
        **params,
        ...
        # https://github.com/hanzoai/llm/issues/8455
...
class LoggedLLMParams(TypedDict, total=False):
...
class StandardLoggingUserAPIKeyMetadata(TypedDict):
    user_api_key_hash: Optional[str]  # hash of the llm virtual key used
...
class StandardLoggingHiddenParams(TypedDict):
    ...
    llm_overhead_time_ms: Optional[float]
    additional_headers: Optional[StandardLoggingAdditionalHeaders]
    batch_models: Optional[List[str]]
    llm_model_name: Optional[str]  # the model name sent to the provider by llm
...
class LLM_IMAGE_VARIATION_PROVIDERS(Enum):
    """
