...
from llm import Router, provider_list
from llm._logging import verbose_proxy_logger
from llm.proxy._types import *
from llm.types.router import CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS
...
async def pre_db_read_auth_checks(
    ...
    """
    from llm.proxy.proxy_server import general_settings, llm_router, premium_user
...
def route_in_additonal_public_routes(current_route: str):
    ...
    In order to use this the llm config.yaml should have the following in general_settings:

    ```yaml
    general_settings:
        master_key: sk-1234
        public_routes: ["HanzoRoutes.public_routes", "/spend/calculate"]
    ```
    """
...
def should_run_auth_on_pass_through_provider_route(route: str) -> bool:
    """
    Use this to decide if the rest of the Hanzo Virtual Key auth checks should run on /vertex-ai/{endpoint} routes
    Use this to decide if the rest of the Hanzo Virtual Key auth checks should run on provider pass through routes
    ...
    # only enabled for Hanzo Enterprise
    return True
...
def get_end_user_id_from_request_body(request_body: dict) -> Optional[str]:
    ...
    # anthropic - check 'llm_metadata'
    end_user_id = request_body.get("llm_metadata", {}).get("user", None)
    if end_user_id:
