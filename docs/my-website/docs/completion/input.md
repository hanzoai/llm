...
## Common Params 
Hanzo accepts and translates the [OpenAI Chat Completion params](https://platform.openai.com/docs/api-reference/chat/create) across all providers. 
...
By default, Hanzo raises an exception if the openai param being passed in isn't supported. 
...
Hanzo assumes any non-openai param is provider specific and passes it in as a kwarg in the request body
