...
from llm.llms.cohere.rerank.transformation import CohereRerankConfig
...
class AzureAIRerankConfig(CohereRerankConfig):
    ...
    def validate_environment(
        ...
        if api_key is None:
            api_key = get_secret_str("AZURE_AI_API_KEY") or llm.azure_key

        if api_key is None:
            raise ValueError(
                "Azure AI API key is required. Please set 'AZURE_AI_API_KEY' or 'llm.azure_key'"
            )
