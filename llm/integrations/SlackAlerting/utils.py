...
from llm.proxy._types import AlertType
from llm.secret_managers.main import get_secret

if TYPE_CHECKING:
    from llm.llm_core_utils.llm_logging import Logging as _Logging
...
async def _add_langfuse_trace_id_to_alert(
    ...
    if (
        ...
        and request_data.get("llm_logging_obj", None) is not None
    ):
        trace_id: Optional[str] = None
        llm_logging_obj: Logging = request_data["llm_logging_obj"]

        for _ in range(3):
            trace_id = llm_logging_obj._get_trace_id(service_name="langfuse")
            if trace_id is not None:
                break
            await asyncio.sleep(3)  # wait 3s before retrying for trace id

        _langfuse_object = llm_logging_obj._get_callback_object(
