...
from llm.secret_managers.get_azure_ad_token_provider import (
...
def get_azure_ad_token_from_oidc(azure_ad_token: str):
    ...
    client = llm.module_level_client
...
class BaseAzureLLM:
    ...
    def initialize_azure_sdk_client(
        ...
        azure_ad_token = llm_params.get("azure_ad_token")
        tenant_id = llm_params.get("tenant_id")
        client_id = llm_params.get("client_id")
        client_secret = llm_params.get("client_secret")
        azure_username = llm_params.get("azure_username")
        azure_password = llm_params.get("azure_password")
        ...
        if azure_ad_token is not None and azure_ad_token.startswith("oidc/"):
            ...
            and llm.enable_azure_ad_token_refresh is True
        ):
            try:
        ...
        if api_version is None:
            api_version = os.getenv(
                "AZURE_API_VERSION", llm.AZURE_DEFAULT_API_VERSION
            )
