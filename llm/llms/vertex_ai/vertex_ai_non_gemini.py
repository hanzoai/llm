...
from llm.llms.bedrock.common_utils import ModelResponseIterator
from llm.llms.custom_httpx.http_handler import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
from llm.types.llms.vertex_ai import *
from llm.utils import CustomStreamWrapper, ModelResponse, Usage
...
def _get_client_from_cache(client_cache_key: str):
    return llm.in_memory_llm_clients_cache.get_cache(client_cache_key)
...
def completion(  # noqa: PLR0915
    ...
    try:
        ...
        raise VertexAIError(
            status_code=400,
            message="vertexai import failed please run `pip install google-cloud-aiplatform`. This is required for the 'vertex_ai/' route on Hanzo",
        )
    ...
    try:
        ...
        ## Load Config
        config = llm.VertexAIConfig.get_config()
        ...
        if (
        ...
        elif model in llm.vertex_text_models:
        ...
        elif model in llm.vertex_code_text_models:
            ...
            fake_stream = True
        elif model in llm.vertex_code_chat_models:  # vertex_code_llm_models
        ...
        ## RESPONSE OBJECT
        if isinstance(completion_response, llm.Message):
        ...
        ## CALCULATING USAGE
        if model in llm.vertex_language_models and response_obj is not None:
            ...
            # init prompt tokens
            # this block attempts to get usage from response_obj if it exists, if not it uses the llm token counter
