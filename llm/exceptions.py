...
## Hanzo versions of the OpenAI Exception Types
...
class AuthenticationError(openai.AuthenticationError):  # type: ignore
    ...
    def __str__(self):
        ...
        if self.num_retries:
            _message += f" Hanzo Retried: {self.num_retries} times"
        if self.max_retries:
            _message += f", Hanzo Max Retries: {self.max_retries}"
        return _message
...
class HanzoUnknownProvider(BadRequestError):
    def __init__(self, model: str, custom_llm_provider: Optional[str] = None):
        self.message = HanzoCommonStrings.llm_provider_not_provided.value.format(
