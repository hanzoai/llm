...
from llm.llm_core_utils.default_encoding import encoding
...
from llm.llm_core_utils.llm_request_utils import _ensure_extra_body_is_safe
from llm.llm_core_utils.llm_response_utils.convert_dict_to_response import (
    HanzoResponseObjectHandler,
...
from llm.llm_core_utils.llm_response_utils.get_api_base import get_api_base
from llm.llm_core_utils.llm_response_utils.get_formatted_prompt import (
    get_formatted_prompt,
)
from llm.llm_core_utils.llm_response_utils.get_headers import (
    get_response_headers,
)
from llm.llm_core_utils.llm_response_utils.response_metadata import (
    ResponseMetadata,
)
...
from llm.llm_core_utils.rules import Rules
from llm.llm_core_utils.streaming_handler import CustomStreamWrapper
from llm.llm_core_utils.token_counter import (
...
from llm.router_utils.get_retry_from_policy import (
...
from llm.types.utils import FileTypes  # type: ignore
...
from llm.llm_core_utils.thread_pool_executor import executor
...
from llm.llms.base_llm.base_utils import (
    BaseLLMModelInfo,
    type_to_response_format_param,
)
from llm.llms.base_llm.chat.transformation import BaseConfig
from llm.llms.base_llm.completion.transformation import BaseTextCompletionConfig
from llm.llms.base_llm.embedding.transformation import BaseEmbeddingConfig
from llm.llms.base_llm.image_variations.transformation import (
    BaseImageVariationConfig,
)
from llm.llms.base_llm.rerank.transformation import BaseRerankConfig
...
from .types.router import Hanzo_Params
...
def print_verbose(
    ...
    try:
        ...
        if llm.set_verbose is True and logger_only is False:
...
def custom_llm_setup():
    ...
    """
    for custom_llm in llm.custom_provider_map:
        if custom_llm["provider"] not in llm.provider_list:
            llm.provider_list.append(custom_llm["provider"])

        if custom_llm["provider"] not in llm._custom_providers:
            llm._custom_providers.append(custom_llm["provider"])
...
def _add_custom_logger_callback_to_specific_event(
    ...
    """
    from llm import _custom_logger_compatible_callbacks_literal
    from llm.llm_core_utils.llm_logging import (
        _init_custom_logger_compatible_class,
    )

    if callback not in llm._known_custom_logger_compatible_callbacks:
        verbose_logger.debug(
            f"Callback {callback} is not a valid custom logger compatible callback. Known list - {llm._known_custom_logger_compatible_callbacks}"
        )
        return
    ...
    if callback_class:
        if (
            ...
            if callback in llm.success_callback:
                llm.success_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in llm._async_success_callback:
                llm._async_success_callback.remove(
                    callback
                )  # remove the string from the callback list
        elif (
            ...
            if callback in llm.failure_callback:
                llm.failure_callback.remove(
                    callback
                )  # remove the string from the callback list
            if callback in llm._async_failure_callback:
                llm._async_failure_callback.remove(
                    callback
                )  # remove the string from the callback list
...
def _custom_logger_class_exists_in_success_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in llm.success_callback or llm._async_success_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in llm.success_callback or llm._async_success_callback

    Prevents double adding a custom logger callback to the llm callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in llm.success_callback + llm._async_success_callback
    )
...
def _custom_logger_class_exists_in_failure_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in llm.failure_callback or llm._async_failure_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in llm.failure_callback or llm._async_failure_callback

    Prevents double adding a custom logger callback to the llm callbacks
    """
    return any(
        isinstance(cb, type(callback_class))
        for cb in llm.failure_callback + llm._async_failure_callback
    )
...
def load_credentials_from_list(kwargs: dict):
    ...
    """
    credential_name = kwargs.get("llm_credential_name")
    if credential_name and llm.credential_list:
...
def get_dynamic_callbacks(
    ...
    returned_callbacks = llm.callbacks.copy()
...
def function_setup(  # noqa: PLR0915
    ...
    ### NOTICES ###
    from llm import Logging as HanzoLogging
    from llm.llm_core_utils.llm_logging import set_callbacks

    if llm.set_verbose is True:
        verbose_logger.warning(
            "`llm.set_verbose` is deprecated. Please set `os.environ['LLM_LOG'] = 'DEBUG'` for debug logs."
        )
    try:
            ...
            for callback in all_callbacks:
                ...
                if isinstance(callback, str):
                    ...
                    if callback is None or any(
                        ...
                        for cb in llm._async_success_callback
        ...
        if (
            len(llm.input_callback) > 0
            or len(llm.success_callback) > 0
            or len(llm.failure_callback) > 0
        ) and len(
            callback_list  # type: ignore
        ) == 0:  # type: ignore
            callback_list = list(
                set(
                    llm.input_callback  # type: ignore
        ...
        ## ASYNC CALLBACKS
        if len(llm.input_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(llm.input_callback):  # type: ignore
                if inspect.iscoroutinefunction(callback):
                    llm._async_input_callback.append(callback)
                    removed_async_items.append(index)

            # Pop the async items from input_callback in reverse order to avoid index issues
            for index in reversed(removed_async_items):
                llm.input_callback.pop(index)
        if len(llm.success_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(llm.success_callback):  # type: ignore
                if inspect.iscoroutinefunction(callback):
                    ...
                    callback in llm._known_custom_logger_compatible_callbacks
            ...
            for index in reversed(removed_async_items):
                llm.success_callback.pop(index)

        if len(llm.failure_callback) > 0:
            removed_async_items = []
            ...
            for index in reversed(removed_async_items):
                llm.failure_callback.pop(index)
        ### DYNAMIC CALLBACKS ###
        ...
        if add_breadcrumb:
            ...
            if llm.turn_off_message_logging:
                # make a copy of the _model_Call_details and log it
                details_to_log.pop("messages", None)
                details_to_log.pop("input", None)
                details_to_log.pop("prompt", None)
            add_breadcrumb(
                category="llm.llm_call",
        ...
        logging_obj = HanzoLogging(
            model=model,
            messages=messages,
            stream=stream,
            llm_call_id=kwargs["llm_call_id"],
            llm_trace_id=kwargs.get("llm_trace_id"),
        ...
        llm_params: Dict[str, Any] = {"api_base": ""}
        if "metadata" in kwargs:
            llm_params["metadata"] = kwargs["metadata"]
        ...
        verbose_logger.exception(
            "llm.utils.py::function_setup() - [Non-Blocking] Error in function_setup"
        )
        raise e


async def _client_async_logging_helper(
    logging_obj: HanzoLoggingObject,
    result,
    start_time,
    end_time,
...
def client(original_function):  # noqa: PLR0915
                    ...
                    if is_coroutine is True:
                        ...
                        if (
                            ...
                            if model_response is not None:
                                ...
                                if llm.enable_json_schema_validation is True:
                                    try:
                                        if (
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0915
        ...
        if _is_async_request(kwargs):
            ...
            if llm.num_retries_per_request is not None:
                # check if previous_models passed in as ['llm_params']['metadata]['previous_models']
                previous_models = kwargs.get("metadata", {}).get(
                    "previous_models", None
                )
                if previous_models is not None:
                    if llm.num_retries_per_request <= len(previous_models):
            ...
            if "stream" in kwargs and kwargs["stream"] is True:
                if (
                    ...
                    for idx, chunk in enumerate(result):
                        chunks.append(chunk)
                    return llm.stream_chunk_builder(
        ...
        result = None
        logging_obj: Optional[HanzoLoggingObject] = kwargs.get(
            "llm_logging_obj", None
        )

        # only set llm_call_id if its not in kwargs
        if "llm_call_id" not in kwargs:
            kwargs["llm_call_id"] = str(uuid.uuid4())
        ...
        try:
            ...
            kwargs["llm_logging_obj"] = logging_obj
            ...
            for k, v in kwargs.items():
                if v is not None and isinstance(v, str) and v.startswith("os.environ/"):
                    kwargs[k] = llm.get_secret(v)
            # [OPTIONAL] CHECK BUDGET
            if llm.max_budget:
                if llm._current_cost > llm.max_budget:
                    raise BudgetExceededError(
                        current_cost=llm._current_cost,
            ...
            print_verbose(
                f"SYNC kwargs[caching]: {kwargs.get('caching', False)}; llm.cache: {llm.cache}; kwargs.get('cache')['no-cache']: {kwargs.get('cache', {}).get('no-cache', False)}"
            )
            ...
            if call_type == CallTypes.completion.value:
                num_retries = (
                    kwargs.get("num_retries", None) or llm.num_retries or None
                )
                if kwargs.get("retry_policy", None):
                ...
                if (
                    ...
                    if (
                        ...
                        return llm.completion_with_retries(*args, **kwargs)
                elif (
    ...
    async def wrapper_async(*args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            # [OPTIONAL] CHECK CACHE
            print_verbose(
                f"ASYNC kwargs[caching]: {kwargs.get('caching', False)}; llm.cache: {llm.cache}; kwargs.get('cache'): {kwargs.get('cache', None)}"
            )
            ...
            if call_type == CallTypes.acompletion.value:
                ...
                if (
                    ...
                    try:
                        ...
                        return await llm.acompletion_with_retries(*args, **kwargs)
                    except Exception:
                        pass
                elif (
                    isinstance(e, llm.exceptions.ContextWindowExceededError)
...
def update_response_metadata(
    ...
    Updates response metadata, adds the following:
        ...
        - response._hidden_params["llm_overhead_time_ms"]
...
def _select_tokenizer_helper(model: str) -> SelectTokenizerResponse:
    ...
    if llm.disable_hf_tokenizer_download is True:
...
def _return_huggingface_tokenizer(model: str) -> Optional[SelectTokenizerResponse]:
    if model in llm.cohere_models and "command-r" in model:
        # cohere
    ...
    # anthropic
    elif model in llm.anthropic_models and "claude-3" not in model:
...
def openai_token_counter(  # noqa: PLR0915
    ...
    count_response_tokens: Optional[
        bool
    ] = False,  # Flag passed from llm.stream_chunk_builder, to indicate counting tokens for LLM Response. We need this because for LLM input we add +3 tokens per message - based on OpenAI's token counter
    ...
    """
    print_verbose(f"Hanzo: Utils - Counting tokens for OpenAI model={model}")
    try:
        if "gpt-4o" in model:
    ...
    elif model in llm.open_ai_chat_completion_models:
        tokens_per_message = 3
        tokens_per_name = 1
    elif model in llm.azure_llms:
        tokens_per_message = 3
        tokens_per_name = 1
    else:
...
def token_counter(
    ...
    if model is not None or custom_tokenizer is not None:
        ...
        if tokenizer_json["type"] == "huggingface_tokenizer":
            ...
            if (
                model in llm.open_ai_chat_completion_models
                or model in llm.azure_llms
            ):
                if model in llm.azure_llms:
...
def register_model(model_cost: Union[str, dict]):  # noqa: PLR0915
    ...
    if isinstance(model_cost, dict):
        ...
        loaded_model_cost = llm.get_model_cost_map(url=model_cost)
    ...
    for key, value in loaded_model_cost.items():
        ...
        llm.model_cost.setdefault(model_cost_key, {}).update(updated_dictionary)
        verbose_logger.debug(
            f"added/updated model={model_cost_key} in llm.model_cost: {model_cost_key}"
        )
        # add new model names to provider lists
        if value.get("llm_provider") == "openai":
            if key not in llm.open_ai_chat_completion_models:
                llm.open_ai_chat_completion_models.append(key)
        elif value.get("llm_provider") == "text-completion-openai":
            if key not in llm.open_ai_text_completion_models:
                llm.open_ai_text_completion_models.append(key)
        elif value.get("llm_provider") == "cohere":
            if key not in llm.cohere_models:
                llm.cohere_models.append(key)
        elif value.get("llm_provider") == "anthropic":
            if key not in llm.anthropic_models:
                llm.anthropic_models.append(key)
        elif value.get("llm_provider") == "openrouter":
            split_string = key.split("/", 1)
            if key not in llm.openrouter_models:
                llm.openrouter_models.append(split_string[1])
        elif value.get("llm_provider") == "vertex_ai-text-models":
            if key not in llm.vertex_text_models:
                llm.vertex_text_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-code-text-models":
            if key not in llm.vertex_code_text_models:
                llm.vertex_code_text_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-chat-models":
            if key not in llm.vertex_chat_models:
                llm.vertex_chat_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-code-chat-models":
        ...
        elif value.get("llm_provider") == "aleph_alpha":
            if key not in llm.aleph_alpha_models:
                llm.aleph_alpha_models.append(key)
        elif value.get("llm_provider") == "bedrock":
            if key not in llm.bedrock_models:
                llm.bedrock_models.append(key)
    return model_cost
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    drop_params is True or llm.drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting user/encoding format is not supported by {custom_llm_provider}. To drop it from the call, set `llm.drop_params = True`.",
                    )
            return non_default_params
    ...
    if custom_llm_provider == "openai" or custom_llm_provider == "azure":
        ...
        supported_params = llm.GroqSTTConfig().get_supported_openai_params_stt()
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.GroqSTTConfig().map_openai_params_stt(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    llm.drop_params is True and k not in supported_params
                ):  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting `{k}` is not supported by {custom_llm_provider}. To drop it from the call, set `llm.drop_params = True`.",
                    )
            return non_default_params
    ...
    if (
        ...
        or custom_llm_provider in llm.openai_compatible_providers
    ):
        optional_params = non_default_params
    elif custom_llm_provider == "bedrock":
        # use stability3 config class if model is a stability3 model
        config_class = (
            llm.AmazonStability3Config
            if llm.AmazonStability3Config._is_stability_3_model(model=model)
            else llm.AmazonNovaCanvasConfig if llm.AmazonNovaCanvasConfig._is_nova_model(model=model)
            else llm.AmazonStabilityConfig
        )
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: Optional[list]):
        ...
        if unsupported_params:
            if llm.drop_params is True or (
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {unsupported_params}, for model={model}. To drop these, set `llm.drop_params=True` or for proxy:\n\n`llm_settings:\n drop_params: true`\n",
                )
    ...
    if custom_llm_provider == "openai":
        ...
        if (
            ...
            raise UnsupportedParamsError(
                status_code=500,
                message="Setting dimensions is not supported for OpenAI `text-embedding-3` and later models. To drop it from the call, set `llm.drop_params = True`.",
            )
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.TritonEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.DatabricksEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.nvidiaNimEmbeddingConfig.map_openai_params(
        ...
        (
            optional_params,
            kwargs,
        ) = llm.VertexAITextEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}, kwargs=kwargs
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        supported_params = (
            llm.LmStudioEmbeddingConfig().get_supported_openai_params()
        )
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.LmStudioEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        if "amazon.titan-embed-text-v1" in model:
            object: Any = llm.AmazonTitanG1Config()
        elif "amazon.titan-embed-image-v1" in model:
            object = llm.AmazonTitanMultimodalEmbeddingG1Config()
        elif "amazon.titan-embed-text-v2:0" in model:
            object = llm.AmazonTitanV2Config()
        elif "cohere.embed-multilingual-v3" in model:
            object = llm.BedrockCohereEmbeddingConfig()
        else:  # unmapped model
            supported_params = []
            _check_valid_arg(supported_params=supported_params)
            final_params = {**kwargs}
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.MistralEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.JinaAIEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        final_params = {**optional_params, **kwargs}
        return final_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.VoyageEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = llm.FireworksAIEmbeddingConfig().map_openai_params(
        ...
        and custom_llm_provider not in llm.openai_compatible_providers
    ):
        if len(non_default_params.keys()) > 0:
            if (
                llm.drop_params is True or drop_params is True
            ):  # drop the unsupported non-default values
                keys = list(non_default_params.keys())
                for k in keys:
                    non_default_params.pop(k, None)
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"Setting {non_default_params} is not supported by {custom_llm_provider}. To drop it from the call, set `llm.drop_params = True`.",
                )
...
def get_optional_params(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    common_auth_dict = llm.common_cloud_provider_auth_params
    if custom_llm_provider in common_auth_dict["providers"]:
        ...
        if custom_llm_provider == "azure":
            optional_params = llm.AzureOpenAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "bedrock":
            optional_params = (
                llm.AmazonBedrockGlobalConfig().map_special_auth_params(
                    non_default_params=passed_params, optional_params=optional_params
                )
            )
        elif (
            ...
            optional_params = llm.VertexAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "watsonx":
            optional_params = llm.IBMWatsonXAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
    ...
    def _check_valid_arg(supported_params: List[str]):
        verbose_logger.info(
            f"\nHanzo completion() model= {model}; provider = {custom_llm_provider}"
        )
        verbose_logger.debug(
            f"\nHanzo: Params passed to completion() {passed_params}"
        )
        verbose_logger.debug(
            f"\nHanzo: Non-Default params passed to completion() {non_default_params}"
        )
        unsupported_params = {}
        for k in non_default_params.keys():
            if k not in supported_params:
    ...
    if custom_llm_provider == "anthropic":
        ...
        optional_params = llm.AnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = llm.AnthropicTextConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = llm.CohereConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        # handle cohere params
        optional_params = llm.CohereChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.TritonConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        optional_params = llm.MaritalkConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.ReplicateConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.PredibaseConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.HuggingfaceConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.TogetherAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        model in llm.vertex_chat_models
        or model in llm.vertex_code_chat_models
        or model in llm.vertex_text_models
        or model in llm.vertex_code_text_models
        or model in llm.vertex_language_models
        or model in llm.vertex_vision_models
    ):
        optional_params = llm.VertexGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.GoogleAIStudioGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
    ...
    elif llm.VertexAIAnthropicConfig.is_supported_model(
        model=model, custom_llm_provider=custom_llm_provider
    ):
        optional_params = llm.VertexAIAnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        if model in llm.vertex_mistral_models:
            if "codestral" in model:
                optional_params = (
                    llm.CodestralTextCompletionConfig().map_openai_params(
                        model=model,
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        drop_params=(
                ...
                optional_params = llm.MistralConfig().map_openai_params(
                    model=model,
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    drop_params=(
        ...
        elif model in llm.vertex_ai_ai21_models:
            optional_params = llm.VertexAIAi21Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
            ...
            optional_params = llm.VertexAILlama3Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
        ...
        optional_params = llm.SagemakerConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if bedrock_route == "converse" or bedrock_route == "converse_like":
            optional_params = llm.AmazonConverseConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=(
            ...
            if bedrock_base_model.startswith("anthropic.claude-3"):
                ...
                optional_params = (
                    llm.AmazonAnthropicClaude3Config().map_openai_params(
                        non_default_params=non_default_params,
                        optional_params=optional_params,
                        model=model,
                        drop_params=(
                ...
                optional_params = llm.AmazonAnthropicConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    model=model,
                    drop_params=(
        ...
        optional_params = llm.CloudflareChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = llm.OllamaConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.OllamaChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = llm.NLPCloudConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.PetalsConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.DeepInfraConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.CodestralTextCompletionConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.DatabricksConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.NvidiaNimConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=(
        ...
        optional_params = llm.CerebrasConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.XAIChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
        )
    elif custom_llm_provider == "ai21_chat" or custom_llm_provider == "ai21":
        optional_params = llm.AI21ChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.FireworksAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.VolcEngineConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.HostedVLLMChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.VLLMConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.GroqChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.OpenAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.OpenrouterConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        optional_params = llm.IBMWatsonXChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        for param in passed_params.keys():
            if llm.IBMWatsonXAIConfig().is_watsonx_text_param(param):
                raise ValueError(
                    f"Hanzo now defaults to Watsonx's `/text/chat` endpoint. Please use the `watsonx_text` provider instead, to call the `/text/generation` endpoint. Param: {param}"
                )
    elif custom_llm_provider == "watsonx_text":
        optional_params = llm.IBMWatsonXAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
        ...
        if llm.AzureOpenAIO1Config().is_o_series_model(model=model):
            optional_params = llm.AzureOpenAIO1Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=(
            ...
            verbose_logger.debug(
                "Azure optional params - api_version: api_version={}, llm.api_version={}, os.environ['AZURE_API_VERSION']={}".format(
                    api_version, llm.api_version, get_secret("AZURE_API_VERSION")
                )
            )
            api_version = (
                api_version
                ...
                or llm.AZURE_DEFAULT_API_VERSION
            )
            optional_params = llm.AzureOpenAIConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
        ...
        optional_params = llm.OpenAILikeChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=(
...
def _get_order_filtered_deployments(healthy_deployments: List[Dict]) -> List:
    min_order = min(
        (
            deployment["llm_params"]["order"]
            for deployment in healthy_deployments
            if "order" in deployment["llm_params"]
        ),
        default=None,
    )
    ...
    if min_order is not None:
        filtered_deployments = [
            deployment
            for deployment in healthy_deployments
            if deployment["llm_params"].get("order") == min_order
        ]
...
def _get_model_region(
    custom_llm_provider: str, llm_params: Hanzo_Params
) -> Optional[str]:
    """
    ...
    if custom_llm_provider == "vertex_ai":
        ...
        aws_region_name = llm_params.aws_region_name
        if aws_region_name is not None:
            return aws_region_name
    elif custom_llm_provider == "watsonx":
        watsonx_region_name = llm_params.watsonx_region_name
        if watsonx_region_name is not None:
            return watsonx_region_name
    return llm_params.region_name


def _infer_model_region(llm_params: Hanzo_Params) -> Optional[AllowedModelRegion]:
    """
    ...
    """
    model, custom_llm_provider, _, _ = llm.get_llm_provider(
        model=llm_params.model, llm_params=llm_params
    )

    model_region = _get_model_region(
        custom_llm_provider=custom_llm_provider, llm_params=llm_params
    )

    if model_region is None:
        verbose_logger.debug(
            "Cannot infer model region for model: {}".format(llm_params.model)
        )
        return None

    if custom_llm_provider == "azure":
        eu_regions = llm.AzureOpenAIConfig().get_eu_regions()
        us_regions = llm.AzureOpenAIConfig().get_us_regions()
    elif custom_llm_provider == "vertex_ai":
        eu_regions = llm.VertexAIConfig().get_eu_regions()
        us_regions = llm.VertexAIConfig().get_us_regions()
    elif custom_llm_provider == "bedrock":
        eu_regions = llm.AmazonBedrockGlobalConfig().get_eu_regions()
        us_regions = llm.AmazonBedrockGlobalConfig().get_us_regions()
    elif custom_llm_provider == "watsonx":
        eu_regions = llm.IBMWatsonXAIConfig().get_eu_regions()
        us_regions = llm.IBMWatsonXAIConfig().get_us_regions()
    else:
        eu_regions = []
        us_regions = []
...
def _is_region_eu(llm_params: Hanzo_Params) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if llm_params.region_name == "eu":
        return True

    ## Else - try and infer from model region
    model_region = _infer_model_region(llm_params=llm_params)
    if model_region is not None and model_region == "eu":
        return True
    return False


def _is_region_us(llm_params: Hanzo_Params) -> bool:
    """
    Return true/false if a deployment is in the US
    """
    if llm_params.region_name == "us":
        return True
...
def is_region_allowed(
    llm_params: Hanzo_Params, allowed_model_region: str
) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if llm_params.region_name == allowed_model_region:
        return True
    return False


def get_model_region(
    llm_params: Hanzo_Params, mode: Optional[str]
) -> Optional[str]:
    """
    Pass the llm params for an azure model, and get back the region
    """
    if (
        "azure" in llm_params.model
        and isinstance(llm_params.api_key, str)
        and isinstance(llm_params.api_base, str)
    ):
        _model = llm_params.model.replace("azure/", "")
        response: dict = llm.AzureChatCompletion().get_headers(
            model=_model,
            api_key=llm_params.api_key,
            api_base=llm_params.api_base,
            api_version=llm_params.api_version or llm.AZURE_DEFAULT_API_VERSION,
            timeout=10,
            mode=mode or "chat",
        )
...
def get_api_key(llm_provider: str, dynamic_api_key: Optional[str]):
    api_key = dynamic_api_key or llm.api_key
    # openai
    if llm_provider == "openai" or llm_provider == "text-completion-openai":
        api_key = api_key or llm.openai_key or get_secret("OPENAI_API_KEY")
    # anthropic
    ...
    # aleph_alpha
        ...
        api_key = (
            api_key or llm.aleph_alpha_key or get_secret("ALEPH_ALPHA_API_KEY")
        )
    # baseten
    elif llm_provider == "baseten":
        api_key = api_key or llm.baseten_key or get_secret("BASETEN_API_KEY")
    # cohere
    elif llm_provider == "cohere" or llm_provider == "cohere_chat":
        api_key = api_key or llm.cohere_key or get_secret("COHERE_API_KEY")
    # huggingface
    elif llm_provider == "huggingface":
        api_key = (
            api_key or llm.huggingface_key or get_secret("HUGGINGFACE_API_KEY")
        )
    # nlp_cloud
    elif llm_provider == "nlp_cloud":
        api_key = api_key or llm.nlp_cloud_key or get_secret("NLP_CLOUD_API_KEY")
    # replicate
    elif llm_provider == "replicate":
        api_key = api_key or llm.replicate_key or get_secret("REPLICATE_API_KEY")
    # together_ai
    elif llm_provider == "together_ai":
        api_key = (
            api_key
            or llm.togetherai_api_key
...
def get_max_tokens(model: str) -> Optional[int]:
    ...
    def _get_max_position_embeddings(model_name):
        ...
        try:
            ...
            response = llm.module_level_client.get(config_url)
    ...
    try:
        if model in llm.model_cost:
            if "max_output_tokens" in llm.model_cost[model]:
                return llm.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in llm.model_cost[model]:
                return llm.model_cost[model]["max_tokens"]
        model, custom_llm_provider, _, _ = get_llm_provider(model=model)
        if custom_llm_provider == "huggingface":
            max_tokens = _get_max_position_embeddings(model_name=model)
            return max_tokens
        if model in llm.model_cost:  # check if extracted model is in model_list
            if "max_output_tokens" in llm.model_cost[model]:
                return llm.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in llm.model_cost[model]:
                return llm.model_cost[model]["max_tokens"]
...
def _check_provider_match(model_info: dict, custom_llm_provider: Optional[str]) -> bool:
    ...
    if custom_llm_provider and (
        "llm_provider" in model_info
        and model_info["llm_provider"] != custom_llm_provider
    ):
...
def _get_model_info_helper(  # noqa: PLR0915
    ...
    """
    try:
        azure_llms = {**llm.azure_llms, **llm.azure_embedding_models}
        if model in azure_llms:
            model = azure_llms[model]
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai_beta":
            custom_llm_provider = "vertex_ai"
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
            if "meta/" + model in llm.vertex_llama3_models:
                model = "meta/" + model
            elif model + "@latest" in llm.vertex_mistral_models:
                model = model + "@latest"
            elif model + "@latest" in llm.vertex_ai_ai21_models:
                model = model + "@latest"
        ##########################
        ...
        verbose_logger.debug(
            f"checking potential_model_names in llm.model_cost: {potential_model_names}"
        )
        ...
        if custom_llm_provider == "huggingface":
            ...
            return llm.OllamaConfig().get_model_info(model)
        else:
            """
            Check if: (in order of specificity)
            1. 'custom_llm_provider/model' in llm.model_cost. Checks "groq/llama3-8b-8192" if model="llama3-8b-8192" and custom_llm_provider="groq"
            2. 'model' in llm.model_cost. Checks "gemini-1.5-pro-002" in  llm.model_cost if model="gemini-1.5-pro-002" and custom_llm_provider=None
            3. 'combined_stripped_model_name' in llm.model_cost. Checks if 'gemini/gemini-1.5-flash' in model map, if 'gemini/gemini-1.5-flash-001' given.
            4. 'stripped_model_name' in llm.model_cost. Checks if 'ft:gpt-3.5-turbo' in model map, if 'ft:gpt-3.5-turbo:my-org:custom_suffix:id' given.
            5. 'split_model' in llm.model_cost. Checks "llama3-8b-8192" in llm.model_cost if model="groq/llama3-8b-8192"
            """

            _model_info: Optional[Dict[str, Any]] = None
            key: Optional[str] = None

            if combined_model_name in llm.model_cost:
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and model in llm.model_cost:
            ...
            if (
                _model_info is None
                and combined_stripped_model_name in llm.model_cost
            ):
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and stripped_model_name in llm.model_cost:
                ...
                if not _check_provider_match(
                    ...
                    _model_info = None
            if _model_info is None and split_model in llm.model_cost:
...
def get_model_info(model: str, custom_llm_provider: Optional[str] = None) -> ModelInfo:
    ...
    Parameters:
    - model (str): The name of the model.
    - custom_llm_provider (str | null): the provider used for the model. If provided, used to check if the llm model info is for that provider.

    Returns:
        dict: A dictionary containing the following information:
            key: Required[str] # the key in llm.model_cost which is returned
...
def function_to_dict(input_function):  # noqa: C901
    ...
    -------
    dictionnary
        A dictionnary to add to the list passed to `functions` parameter of `llm.completion`
    """
...
def get_provider_fields(custom_llm_provider: str) -> List[ProviderField]:
    ...
    if custom_llm_provider == "databricks":
        return llm.DatabricksConfig().get_required_params()

    elif custom_llm_provider == "ollama":
        return llm.OllamaConfig().get_required_params()

    elif custom_llm_provider == "azure_ai":
        return llm.AzureAIStudioConfig().get_required_params()
...
def validate_environment(  # noqa: PLR0915
    ...
    if custom_llm_provider:
        ...
        if (
            model in llm.open_ai_chat_completion_models
            or model in llm.open_ai_text_completion_models
            or model in llm.open_ai_embedding_models
            or model in llm.openai_image_generation_models
        ):
            if "OPENAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("OPENAI_API_KEY")
        ## anthropic
        elif model in llm.anthropic_models:
            if "ANTHROPIC_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("ANTHROPIC_API_KEY")
        ## cohere
        elif model in llm.cohere_models:
            if "COHERE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("COHERE_API_KEY")
        ## replicate
        elif model in llm.replicate_models:
            if "REPLICATE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("REPLICATE_API_KEY")
        ...
        ## vertex - text + chat models
            ...
            model in llm.vertex_chat_models
            or model in llm.vertex_text_models
            or model in llm.models_by_provider["vertex_ai"]
        ):
            if "VERTEXAI_PROJECT" in os.environ and "VERTEXAI_LOCATION" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.extend(["VERTEXAI_PROJECT", "VERTEXAI_LOCATION"])
        ## huggingface
        elif model in llm.huggingface_models:
            if "HUGGINGFACE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("HUGGINGFACE_API_KEY")
        ## ai21
            ...
            if "AI21_API_KEY" in os.environ:
                ...
                missing_keys.append("AI21_API_KEY")
        ## together_ai
        elif model in llm.together_ai_models:
            if "TOGETHERAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("TOGETHERAI_API_KEY")
        ## aleph_alpha
            ...
            if "ALEPH_ALPHA_API_KEY" in os.environ:
                ...
                missing_keys.append("ALEPH_ALPHA_API_KEY")
        ## baseten
        elif model in llm.baseten_models:
            if "BASETEN_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("BASETEN_API_KEY")
...
def acreate(*args, **kwargs):  ## Thin client to handle the acreate langchain call
    return llm.acompletion(*args, **kwargs)
...
def valid_model(model):
    try:
        ...
        if (
            ...
            llm.completion(model=model, messages=messages)
    except Exception:
...
def check_valid_key(model: str, api_key: str):
    """
    Checks if a given API key is valid for a specific model by making a llm.completion call with max_tokens=10
...
def register_prompt_template(
    ...
    if tokenizer_config:
        for m in potential_models:
            llm.known_tokenizer_config[m] = {
                "tokenizer": tokenizer_config,
                "status": "success",
            }
    else:
        for m in potential_models:
            llm.custom_prompt_dict[m] = {
                "roles": roles,
                "initial_prompt_value": initial_prompt_value,
                "final_prompt_value": final_prompt_value,
            }

    return llm.custom_prompt_dict
...
def mock_completion_streaming_obj(
    ...
    if isinstance(mock_response, llm.MockException):
        raise mock_response
    for i in range(0, len(mock_response), 3):
        ...
        if n is None:
            ...
            _all_choices = []
            for j in range(n):
                _streaming_choice = llm.utils.StreamingChoices(
                    index=j,
...
def trim_messages(
    ...
    Args:
        ...
        model: The Hanzo model being used (determines the token limit).
    ...
    try:
        if max_tokens is None:
            ...
            if model in llm.model_cost:
                max_tokens_for_model = llm.model_cost[model].get(
                    "max_input_tokens", llm.model_cost[model]["max_tokens"]
                )
                max_tokens = int(max_tokens_for_model * trim_ratio)
            else:
                # if user did not specify max (input) tokens
                # or passed an llm llm does not know
...
def get_valid_models(check_provider_endpoint: bool = False) -> List[str]:
    ...
    try:
        ...
        for provider in llm.provider_list:
            # edge case llm has together_ai as a provider, it should be togetherai
            env_provider_1 = provider.replace("_", "")
            env_provider_2 = provider

            # llm standardizes expected provider keys to
        ...
        for provider in valid_providers:
            ...
            if provider == "azure":
                ...
                models_for_provider = llm.models_by_provider.get(provider, [])
...
def print_args_passed_to_llm(original_function, args, kwargs):
    ...
    try:
        ...
        if args and kwargs:
            print_verbose(
                f"\033[92mllm.{original_function.__name__}({args_str}, {kwargs_str})\033[0m"
            )
        elif args:
            print_verbose(
                f"\033[92mllm.{original_function.__name__}({args_str})\033[0m"
            )
        elif kwargs:
            print_verbose(
                f"\033[92mllm.{original_function.__name__}({kwargs_str})\033[0m"
            )
        else:
            print_verbose(f"\033[92mllm.{original_function.__name__}()\033[0m")
        print_verbose("\n")  # new line after
    except Exception:
...
def _get_base_model_from_metadata(model_call_details=None):
    if model_call_details is None:
        return None
    llm_params = model_call_details.get("llm_params", {})
    if llm_params is not None:
        _base_model = llm_params.get("base_model", None)
...
class ProviderConfigManager:
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            ...
            return llm.HanzoProxyChatConfig()
...
def get_end_user_id_for_cost_tracking(
    ...
    end_user_id = cast(
        Optional[str],
        llm_params.get("user_api_key_end_user_id")
        or _metadata.get("user_api_key_end_user_id"),
    )
    if llm.disable_end_user_cost_tracking:
        return None
    if (
        service_type == "prometheus"
        and llm.disable_end_user_cost_tracking_prometheus_only
    ):
        return None
    return end_user_id
...
def get_non_default_completion_params(kwargs: dict) -> dict:
    openai_params = llm.OPENAI_CHAT_COMPLETION_PARAMS
    default_params = openai_params + all_llm_params
    non_default_params = {
...
def return_raw_request(endpoint: CallTypes, kwargs: dict) -> RawRequestTypedDict:
    ...
    This is currently in BETA, and tested for `/chat/completions` -> `llm.completion` calls.
    """
    from datetime import datetime
    ...
    raw_request_typed_dict = llm_logging_obj.model_call_details.get(
