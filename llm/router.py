...
from llm.llm_core_utils.asyncify import run_async_function
from llm.llm_core_utils.core_helpers import _get_parent_otel_span_from_kwargs
...
from llm.router_strategy.lowest_tpm_rpm import LowestTPMLoggingHandler
from llm.router_strategy.lowest_tpm_rpm_v2 import LowestTPMLoggingHandler_v2
from llm.router_strategy.simple_shuffle import simple_shuffle
from llm.router_strategy.tag_based_routing import get_deployments_for_tag
from llm.router_utils.add_retry_fallback_headers import (
...
from llm.router_utils.client_initalization_utils import InitalizeCachedClient
from llm.router_utils.clientside_credential_handler import (
    get_dynamic_llm_params,
    is_clientside_credential,
)
from llm.router_utils.cooldown_cache import CooldownCache
...
from llm.router_utils.handle_error import (
    async_raise_no_deployment_exception,
    send_llm_exception_alert,
)
from llm.router_utils.pre_call_checks.prompt_caching_deployment_check import (
...
from llm.types.llms.openai import AllMessageValues, Batch, FileObject, FileTypes
from llm.types.router import (
    CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS,
    VALID_LLM_ENVIRONMENTS,
    AlertingConfig,
    AllowedFailsPolicy,
    AssistantsTypedDict,
    CredentialLLMParams,
...
from llm.types.utils import GenericBudgetConfigType
...
class Router:
    ...
    def __init__(  # noqa: PLR0915
        ...
        Args:
            ...
            default_llm_params (dict): Default parameters for Router.chat.completion.create. Defaults to {}.
        ...
        Returns:
            Router: An instance of the llm.Router class.
        ...
        self.deployment_names: List = (
            []
        )  # names of models under llm_params. ex. azure/chatgpt-v-2
        ...
        if cache_responses:
            if llm.cache is None:
                ...
                llm.cache = llm.Cache(type=cache_type, **cache_config)  # type: ignore
        ...
        if model_list is not None:
            ...
            for m in model_list:
                if "model" in m["llm_params"]:
                    self.deployment_latency_map[m["llm_params"]["model"]] = 0
        else:
        ...
        if allowed_fails is not None:
            ...
            self.allowed_fails = llm.allowed_fails
        ...
        elif llm.num_retries is not None:
            self.num_retries = llm.num_retries
        else:
            self.num_retries = openai.DEFAULT_MAX_RETRIES

        if max_fallbacks is not None:
            self.max_fallbacks = max_fallbacks
        elif llm.max_fallbacks is not None:
            self.max_fallbacks = llm.max_fallbacks
        else:
            self.max_fallbacks = llm.ROUTER_MAX_FALLBACKS

        self.timeout = timeout or llm.request_timeout
        ...
        _fallbacks = fallbacks or llm.fallbacks

        self.validate_fallbacks(fallback_param=_fallbacks)
        ### set fallbacks
        self.fallbacks = _fallbacks

        if default_fallbacks is not None or llm.default_fallbacks is not None:
            _fallbacks = default_fallbacks or llm.default_fallbacks
        ...
        self.context_window_fallbacks = (
            context_window_fallbacks or llm.context_window_fallbacks
        )

        _content_policy_fallbacks = (
            content_policy_fallbacks or llm.content_policy_fallbacks
        )
        ...
        default_llm_params = default_llm_params or {}
        self.chat = llm.Chat(params=default_llm_params, router_obj=self)

        # default llm args
        self.default_llm_params = default_llm_params
        self.default_llm_params.setdefault("timeout", timeout)
        self.default_llm_params.setdefault("max_retries", 0)
        self.default_llm_params.setdefault("metadata", {}).update(
        ...
        if isinstance(llm.failure_callback, list):
            ...
            llm.failure_callback = [self.deployment_callback_on_failure]
    ...
    def discard(self):
        ...
        """
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm._async_success_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm.success_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm._async_failure_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm.failure_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm.input_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm.service_callback, self
        )
        llm.logging_callback_manager.remove_callback_from_list_by_object(
            llm.callbacks, self
        )
    ...
    def routing_strategy_init(
        ...
        if (
            ...
            ## add callback
            if isinstance(llm.input_callback, list):
                llm.input_callback.append(self.leastbusy_logger)  # type: ignore
            else:
                llm.input_callback = [self.leastbusy_logger]  # type: ignore
            if isinstance(llm.callbacks, list):
                llm.logging_callback_manager.add_llm_callback(self.leastbusy_logger)  # type: ignore
        elif (
            ...
            if isinstance(llm.callbacks, list):
                llm.logging_callback_manager.add_llm_callback(self.lowesttpm_logger)  # type: ignore
        elif (
            ...
            if isinstance(llm.callbacks, list):
                llm.logging_callback_manager.add_llm_callback(self.lowesttpm_logger_v2)  # type: ignore
        elif (
            ...
            if isinstance(llm.callbacks, list):
                llm.logging_callback_manager.add_llm_callback(self.lowestlatency_logger)  # type: ignore
        elif (
            ...
            if isinstance(llm.callbacks, list):
                llm.logging_callback_manager.add_llm_callback(self.lowestcost_logger)  # type: ignore
        else:
            pass

    def initialize_assistants_endpoint(self):
        ## INITIALIZE PASS THROUGH ASSISTANTS ENDPOINT ##
        self.acreate_assistants = self.factory_function(llm.acreate_assistants)
        self.adelete_assistant = self.factory_function(llm.adelete_assistant)
        self.aget_assistants = self.factory_function(llm.aget_assistants)
        self.acreate_thread = self.factory_function(llm.acreate_thread)
        self.aget_thread = self.factory_function(llm.aget_thread)
        self.a_add_message = self.factory_function(llm.a_add_message)
        self.aget_messages = self.factory_function(llm.aget_messages)
        self.arun_thread = self.factory_function(llm.arun_thread)

    def initialize_router_endpoints(self):
        self.amoderation = self.factory_function(
            llm.amoderation, call_type="moderation"
        )
        self.aanthropic_messages = self.factory_function(
            llm.anthropic_messages, call_type="anthropic_messages"
        )
        self.aresponses = self.factory_function(
            llm.aresponses, call_type="aresponses"
        )
        self.responses = self.factory_function(llm.responses, call_type="responses")
        ...
        if optional_pre_call_checks is not None:
            for pre_call_check in optional_pre_call_checks:
                ...
                if _callback is not None:
                    llm.logging_callback_manager.add_llm_callback(_callback)
    ...
    def print_deployment(self, deployment: dict):
        ...
        try:
            ...
            llm_params: dict = _deployment_copy["llm_params"]
            if "api_key" in llm_params:
                llm_params["api_key"] = llm_params["api_key"][:2] + "*" * 10
    ...
    def _completion(
        ...
        try:
            ...
            data = deployment["llm_params"].copy()
            ...
            verbose_router_logger.info(
                f"llm.completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            ...
            if isinstance(response, ModelResponse):
                ...
                if _should_raise:
                    raise llm.ContentPolicyViolationError(
            ...
            verbose_router_logger.info(
                f"llm.completion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            raise e
    ...
    async def _acompletion(
        ...
        try:
            ...
            logging_obj: Optional[LLMLogging] = kwargs.get(
            ...
            verbose_router_logger.info(
                f"llm.acompletion(model={model_name})\033[32m 200 OK\033[0m"
            )
            # debug how often this deployment picked
            self._track_deployment_metrics(
                deployment=deployment,
            ...
            verbose_router_logger.info(
                f"llm.acompletion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    def _update_kwargs_before_fallbacks(self, model: str, kwargs: dict) -> None:
        ...
        kwargs.setdefault("llm_trace_id", str(uuid.uuid4()))
        kwargs.setdefault("metadata", {}).update({"model_group": model})

    def _update_kwargs_with_default_llm_params(
        self, kwargs: dict, metadata_variable_name: Optional[str] = "metadata"
    ) -> None:
        """
        Adds default llm params to kwargs, if set.
        """
        self.default_llm_params[metadata_variable_name] = (
            self.default_llm_params.pop("metadata", {})
        )
    ...
    def _handle_clientside_credential(
        ...
        llm_params = deployment["llm_params"].copy()
        dynamic_llm_params = get_dynamic_llm_params(
            llm_params=llm_params, request_kwargs=kwargs
        )
        metadata = kwargs.get("metadata", {})
        model_group = cast(str, metadata.get("model_group"))
        _model_id = self._generate_model_id(
            model_group=model_group, llm_params=dynamic_llm_params
        )
        ...
        deployment_pydantic_obj = Deployment(
            ...
            llm_params=LLM_Params(**dynamic_llm_params),
    ...
    def _update_kwargs_with_deployment(
        ...
        """
        model_info = deployment.get("model_info", {}).copy()
        deployment_model_name = deployment["llm_params"]["model"]
        deployment_api_base = deployment["llm_params"].get("api_base")
        if is_clientside_credential(request_kwargs=kwargs):
            ...
            deployment_model_name = deployment_pydantic_obj.llm_params.model
            deployment_api_base = deployment_pydantic_obj.llm_params.api_base
        ...
        kwargs["timeout"] = self._get_timeout(
            kwargs=kwargs, data=deployment["llm_params"]
        )

        self._update_kwargs_with_default_llm_params(
    ...
    def _get_stream_timeout(
        ...
        return (
            ...
            or data.get(
                "stream_timeout", None
            )  # timeout set on llm_params for this deployment
            or self.stream_timeout  # timeout set on router
            or self.default_llm_params.get("stream_timeout", None)
        )
    ...
    def _get_non_stream_timeout(
        ...
        timeout = (
            ...
            or self.default_llm_params.get("timeout", None)
        )
        return timeout
    ...
    async def abatch_completion(
        ...
        **kwargs,
        ...
        1. Batch Process 1 request to N models on llm.Router. Pass messages as List[Dict[str, str]] to use this
        2. Batch Process N requests to M models on llm.Router. Pass messages as List[List[Dict[str, str]]] to use this
        ...
        ```
            response = await router.abatch_completion(
                ...
                messages=[
                    {"role": "user", "content": "is llm becoming a better product ?"}
                ],
                max_tokens=15,
            )
        ```
    ...
    async def abatch_completion_one_model_multiple_requests(
        ...
        """
        Async Batch Completion - Batch Process multiple Messages to one model_group on llm.Router
        ...
        if make_request:
            try:
                ...
                _response._hidden_params["additional_headers"].update(
                    {"x-llm-request-prioritization-used": True}
                )
                return _response
            except Exception as e:
    ...
    async def _prompt_management_factory(
        ...
        llm_logging_object = kwargs.get("llm_logging_obj", None)
        if llm_logging_object is None:
            llm_logging_object, kwargs = function_setup(
                **{
        ...
        llm_logging_object = cast(LLMLogging, llm_logging_object)
        ...
        prompt_variables = kwargs.get(
            "prompt_variables"
        ) or prompt_management_deployment["llm_params"].get(
        ...
        kwargs["model"] = model
        kwargs["messages"] = messages
        kwargs["llm_logging_obj"] = llm_logging_object
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.image_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.image_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aimage_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = hanzo.aimage_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"hanzo.aimage_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"hanzo.aimage_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _atranscription(self, file: FileTypes, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.atranscription(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.atranscription(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _arerank(self, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.arerank(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.arerank(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _arealtime(self, model: str, **kwargs):
        ...
        try:
            ...
            return await llm._arealtime(**{**data, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
    ...
    def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            # call via llm.completion()
            return llm.text_completion(**{**data, "prompt": prompt, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
            raise e
    ...
    async def _atext_completion(self, model: str, prompt: str, **kwargs):
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.atext_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.atext_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    async def _aadapter_completion(self, adapter_id: str, model: str, **kwargs):
        try:
            ...
            response = llm.aadapter_completion(
                **{
                    **data,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.aadapter_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.aadapter_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _ageneric_api_call_with_fallbacks(
        ...
        """
        Make a generic LLM API call through the router, this allows you to use retries/fallbacks with llm router

        Args:
            model: The model to use
            handler_function: The handler function to call (e.g., llm.anthropic_messages)
    ...
    def _generic_api_call_with_fallbacks(
        ...
        Args:
            model: The model to use
            original_function: The handler function to call (e.g., llm.completion)
            **kwargs: Additional arguments to pass to the handler function
        Returns:
            The response from the handler function
        """
    ...
    def _embedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.embedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.embedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aembedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.aembedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"llm.aembedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _acreate_file(
        ...
        **kwargs,
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"llm.acreate_file(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response  # type: ignore
        except Exception as e:
            verbose_router_logger.exception(
                f"llm.acreate_file(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            kwargs.setdefault(metadata_variable_name, {}).update(
                {
                    "deployment": deployment["llm_params"]["model"],
                    "model_info": deployment.get("model_info", {}),
                    "api_base": deployment.get("llm_params", {}).get("api_base"),
                }
            )
            ...
            verbose_router_logger.exception(
                f"llm._acreate_batch(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
            ...
            async def try_retrieve_batch(model_name):
                try:
                    ...
                    _, custom_llm_provider, _, _ = get_llm_provider(  # type: ignore
                        model=model_name["llm_params"]["model"]
                    )
                    new_kwargs = copy.deepcopy(kwargs)
                    new_kwargs.pop("custom_llm_provider", None)
                    return await llm.aretrieve_batch(
    ...
    async def alist_batches(
        ...
        **kwargs,
        ...
        async def try_retrieve_batch(model: DeploymentTypedDict):
            try:
                # Update kwargs with the current model name or any other model-specific adjustments
                return await llm.alist_batches(
                    **{**model["llm_params"], **kwargs}
                )
            except Exception:
                return None
    ...
    async def _pass_through_moderation_endpoint_factory(
        ...
        **kwargs,
        ...
        if kwargs.get("model") and self.get_model_list(model_name=kwargs["model"]):
            ...
            kwargs["model"] = deployment["llm_params"]["model"]
        ...
        **kwargs,
        ...
        if custom_llm_provider is None:
            if self.assistants_config is not None:
                ...
                kwargs.update(self.assistants_config["llm_params"])
            else:
                raise Exception(
    ...
    async def async_function_with_fallbacks(self, *args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            try:
                ...
                if isinstance(e, llm.ContextWindowExceededError):
                    if context_window_fallbacks is not None:
                        ...
                        error_message = "model={}. context_window_fallbacks={}. fallbacks={}.\n\nSet 'context_window_fallback' - https://docs.hanzo.ai/docs/routing#fallbacks".format(
                ...
                elif isinstance(e, llm.ContentPolicyViolationError):
                    if content_policy_fallbacks is not None:
                        ...
                        error_message = "model={}. content_policy_fallback={}. fallbacks={}.\n\nSet 'content_policy_fallback' - https://docs.hanzo.ai/docs/routing#fallbacks".format(
                ...
                verbose_router_logger.error(
                    "llm.router.py::async_function_with_fallbacks() - Error occurred while trying to do fallbacks - {}\n{}\n\nDebug Information:\nCooldown Deployments={}".format(
                        str(new_exception),
    ...
    def _handle_mock_testing_fallbacks(
        ...
        """
        Helper function to raise a llm Error for mock testing purposes.

        Raises:
            llm.InternalServerError: when `mock_testing_fallbacks=True` passed in request params
            llm.ContextWindowExceededError: when `mock_testing_context_fallbacks=True` passed in request params
            llm.ContentPolicyViolationError: when `mock_testing_content_policy_fallbacks=True` passed in request params
        """
        ...
        if mock_testing_fallbacks is not None and mock_testing_fallbacks is True:
            ...
            raise llm.ContextWindowExceededError(
                model=model_group,
                llm_provider="",
    ...
    async def async_function_with_retries(self, *args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            if type(original_exception) in llm.LLM_EXCEPTION_TYPES:
    ...
    def _handle_mock_testing_rate_limit_error(
        ...
        """
        Helper function to raise a mock llm.RateLimitError error for testing purposes.

        Raises:
            llm.RateLimitError error when `mock_testing_rate_limit_error=True` passed in request params
        """
        ...
        if available_models is not None and len(available_models) == 1:
            num_retries = cast(
                Optional[int], available_models[0]["llm_params"].get("num_retries")
            )
        ...
        if (
            ...
            verbose_router_logger.info(
                f"llm.router.py::_mock_rate_limit_error() - Raising mock RateLimitError for model={model_group}"
            )
    ...
    def should_retry_this_error(
        ...
        if (
            isinstance(error, llm.ContextWindowExceededError)
            and context_window_fallbacks is not None
        ):
            raise error

        if (
            isinstance(error, llm.ContentPolicyViolationError)
            and content_policy_fallbacks is not None
        ):
            raise error

        if isinstance(error, llm.NotFoundError):
            raise error
    ...
    async def deployment_callback_on_success(
        ...
        try:
            ...
            if kwargs["llm_params"].get("metadata") is None:
                pass
            else:
                deployment_name = kwargs["llm_params"]["metadata"].get(
                    "deployment", None
            ...
            verbose_router_logger.exception(
                "llm.router.Router::deployment_callback_on_success(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def sync_deployment_callback_on_success(
        ...
        if kwargs["llm_params"].get("metadata") is None:
            ...
            model_group = kwargs["llm_params"]["metadata"].get("model_group", None)
            model_info = kwargs["llm_params"].get("model_info", {}) or {}
    ...
    def deployment_callback_on_failure(
        ...
        try:
            ...
            _time_to_cooldown = kwargs.get("llm_params", {}).get(
    ...
    async def async_deployment_callback_on_failure(
        ...
        deployment_name = kwargs["llm_params"]["metadata"].get(
            "deployment", None
        )  # handles wildcard routes - by giving the original name sent to `llm.completion`
        model_group = kwargs["llm_params"]["metadata"].get("model_group", None)
        model_info = kwargs["llm_params"].get("model_info", {}) or {}
        id = model_info.get("id", None)
        if model_group is None or id is None:
    ...
    def routing_strategy_pre_call_checks(self, deployment: dict):
        ...
        """
        for _callback in llm.callbacks:
    ...
    async def async_routing_strategy_pre_call_checks(
        ...
        logging_obj: Optional[LLMLogging] = None,
    ):
        """
    ...
    def _generate_model_id(self, model_group: str, llm_params: dict):
        """
        Helper function to consistently generate the same id for a deployment

        - create a string from all the llm params
        - hash
        - use hash as id
        """
    ...
    def _create_deployment(
        ...
        - None: If the deployment is not active for the current environment (if 'supported_environments' is set in llm_params)
        """
        deployment = Deployment(
            **deployment_info,
            model_name=_model_name,
            llm_params=LLM_Params(**_llm_params),
        ...
        if deployment.llm_params.custom_llm_provider is not None:
            _model_name = (
                deployment.llm_params.custom_llm_provider + "/" + _model_name
            )
    ...
    def deployment_is_active_for_environment(self, deployment: Deployment) -> bool:
        ...
        for _env in deployment.model_info["supported_environments"]:
            if _env not in VALID_LLM_ENVIRONMENTS:
                raise ValueError(
    ...
    def _add_deployment(self, deployment: Deployment) -> Deployment:
        ...
        if (
            ...
            deployment.llm_params.rpm = getattr(deployment, "rpm")

        if (
            deployment.llm_params.tpm is None
            and getattr(deployment, "tpm", None) is not None
        ):
            deployment.llm_params.tpm = getattr(deployment, "tpm")
        ...
        (
            ...
            model=deployment.llm_params.model,
            custom_llm_provider=deployment.llm_params.get(
        ...
        #     if deployment.llm_params.model == "*":
        #         # user wants to pass through all requests to llm.acompletion for unknown deployments
        ...
        data_sources = deployment.llm_params.get("dataSources", []) or []
        ...
        # done reading model["llm_params"]
        if custom_llm_provider not in llm.provider_list:
    ...
    def _initialize_deployment_for_pass_through(
        ...
        """
        Optional: Initialize deployment for pass-through endpoints if `deployment.llm_params.use_in_pass_through` is True
        ...
        """
        if deployment.llm_params.use_in_pass_through is True:
            ...
            if custom_llm_provider == "vertex_ai":
                if (
                    deployment.llm_params.vertex_project is None
                    or deployment.llm_params.vertex_location is None
                    or deployment.llm_params.vertex_credentials is None
                ):
                    raise ValueError(
                        "vertex_project, vertex_location, and vertex_credentials must be set in llm_params for pass-through endpoints"
                    )
                passthrough_endpoint_router.add_vertex_credentials(
                    project_id=deployment.llm_params.vertex_project,
                    location=deployment.llm_params.vertex_location,
                    vertex_credentials=deployment.llm_params.vertex_credentials,
                )
            else:
                passthrough_endpoint_router.set_pass_through_credentials(
                    custom_llm_provider=custom_llm_provider,
                    api_base=deployment.llm_params.api_base,
                    api_key=deployment.llm_params.api_key,
                )
            pass
        pass
    ...
    def upsert_deployment(self, deployment: Deployment) -> Optional[Deployment]:
        ...
        if _deployment_on_router is not None:
            ...
            if deployment.llm_params == _deployment_on_router.llm_params:
                # No need to update
                return None

            # if there is a new llm param -> then update the deployment
    ...
    def get_deployment_credentials(self, model_id: str) -> Optional[dict]:
        ...
        if deployment is None:
            return None
        return CredentialLLMParams(
            **deployment.llm_params.model_dump(exclude_none=True)
    ...
    def get_router_model_info(
        ...
        Augment llm info with additional params set in `model_info`.
        ...
        if base_model is None:
            base_model = deployment.get("llm_params", {}).get("base_model", None)

        model = base_model

        ## GET PROVIDER
        _model, custom_llm_provider, _, _ = llm.get_llm_provider(
            model=deployment.get("llm_params", {}).get("model", ""),
            llm_params=LLM_Params(**deployment.get("llm_params", {})),
        )

        ## SET MODEL TO 'model=' - if base_model is None + not azure
        if custom_llm_provider == "azure" and base_model is None:
            verbose_router_logger.error(
                "Could not identify azure model. Set azure 'base_model' for accurate max tokens, cost tracking, etc.- https://docs.hanzo.ai/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models"
            )
                ...
                for potential_model in potential_models:
                    try:
                        if potential_model.get("model_info", {}).get(
                            ...
                            model = potential_model.get("llm_params", {}).get(
                                "model"
                            )
                            break
                    except Exception:
        ...
        model_info = llm.get_model_info(model=model_info_name)
    ...
    def get_model_info(self, id: str) -> Optional[dict]:
        ...
        Returns
        - dict: the model in list with 'model_name', 'llm_params', Optional['model_info']
    ...
    def _set_model_group_info(  # noqa: PLR0915
        ...
        for model in model_list:
            ...
            llm_params = LLM_Params(**model["llm_params"])  # type: ignore
            # get configurable clientside auth params
            configurable_clientside_auth_params = (
                llm_params.configurable_clientside_auth_params
            )
            # get model tpm
            ...
            if _deployment_tpm is None:
                _deployment_tpm = model.get("llm_params", {}).get("tpm", None)  # type: ignore
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = model.get("llm_params", {}).get("rpm", None)  # type: ignore
            if _deployment_rpm is None:
                _deployment_rpm = model.get("model_info", {}).get("rpm", None)  # type: ignore

            # get model info
            try:
                model_info = llm.get_model_info(model=llm_params.model)
            except Exception:
                model_info = None
            # get llm provider
            ...
            except llm.exceptions.BadRequestError as e:
                verbose_router_logger.error(
                    "llm.router.py::get_model_group_info() - {}".format(str(e))
                )
    ...
    def _pre_call_checks(  # noqa: PLR0915
        ...
        - model context window < message length. For azure openai models, requires 'base_model' is set. - https://docs.hanzo.ai/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models
        ...
        try:
            input_tokens = llm.token_counter(messages=messages)
        except Exception as e:
            verbose_router_logger.error(
                "llm.router.py::_pre_call_checks: failed to count tokens. Returning initial list of deployments. Got - {}".format(
                    str(e)
                )
            )
        ...
        for idx, deployment in enumerate(_returned_deployments):
            ...
            try:
                base_model = deployment.get("model_info", {}).get("base_model", None)
                if base_model is None:
                    base_model = deployment.get("llm_params", {}).get(
                        "base_model", None
                    )
                model_info = self.get_router_model_info(
                    deployment=deployment, received_model_name=model
                )
                model = base_model or deployment.get("llm_params", {}).get(
                    "model", None
                )
            ...
            _llm_params = deployment.get("llm_params", {})
            ...
            if (
                ...
                if (
                    isinstance(_llm_params, dict)
                    and _llm_params.get("rpm", None) is not None
                ):
                    if (
                        isinstance(_llm_params["rpm"], int)
                        and _llm_params["rpm"] <= current_request
                    ):
                        invalid_model_indices.append(idx)
            ...
            if request_kwargs is not None and llm.drop_params is False:
                # get supported params
                model, custom_llm_provider, _, _ = llm.get_llm_provider(
                    model=model, llm_params=LLM_Params(**_llm_params)
                )
                ...
                if supported_openai_params is None:
                    ...
                    non_default_params = llm.utils.get_non_default_params(
        ...
        if len(invalid_model_indices) == len(_returned_deployments):
            ...
            if _rate_limit_error is True:  # allow generic fallback logic to take place
                ...
                raise llm.ContextWindowExceededError(
                    message="llm._pre_call_checks: Context Window exceeded for given call. No models have context window large enough for this call.\n{}".format(
                        _potential_error_str
                    ),
                    model=model,
                    llm_provider="",
        ...
        if len(_returned_deployments) > 0:
            _returned_deployments = llm.utils._get_order_filtered_deployments(
    ...
    def _get_deployment_by_llm_model(self, model: str) -> List:
        ...
        """
        return [m for m in self.model_list if m["llm_params"]["model"] == model]
    ...
    def _common_checks_available_deployment(
        ...
        if specific_deployment is True:
            ...
            if deployment is not None:
                deployment_model = deployment.llm_params.model
                return deployment_model, deployment.model_dump(exclude_none=True)
            raise ValueError(
                f"LLM Router: Trying to call specific deployment, but Model ID :{model} does not exist in \
        ...
        if model not in self.model_names:
            ...
            if self.default_deployment is not None:
                ...
                updated_deployment["llm_params"]["model"] = model
    ...
    async def async_get_available_deployment(
        ...
        try:
            ...
            if request_kwargs is not None:
                logging_obj = request_kwargs.get("llm_logging_obj", None)
    ...
    def _initialize_alerting(self):
        ...
        verbose_router_logger.info(
            "\033[94m\nInitialized Alerting for llm.Router\033[0m\n"
        )
    ...
    def set_custom_routing_strategy(
        ...
        """
        Sets get_available_deployment and async_get_available_deployment on an instanced of llm.Router

        Use this to set your custom routing strategy

        Args:
            CustomRoutingStrategy: llm.router.CustomRoutingStrategyBase
        """
