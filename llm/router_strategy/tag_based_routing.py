...
async def get_deployments_for_tag(
    ...
    if "metadata" in request_kwargs:
        ...
        if request_tags:
            ...
            for deployment in healthy_deployments:
                deployment_llm_params = deployment.get("llm_params")
                deployment_tags = deployment_llm_params.get("tags")
    ...
    for deployment in healthy_deployments:
        if "default" in deployment.get("llm_params", {}).get("tags", []):
...
def _get_tags_from_request_kwargs(
    ...
    elif "llm_params" in request_kwargs:
        llm_params = request_kwargs["llm_params"]
