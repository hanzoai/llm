...
If configured correctly, since `z@hanzo.ai` would be detected by the Aim Guard as PII, you'll receive a response similar to the following with a `400 Bad Request` status code:

```json
{
  "error": {
    "message": "\"z@hanzo.ai\" detected as email",
    "type": "None",
    "param": "None",
    "code": "400"
  }
