...
from llm.fine_tuning.main import vertex_fine_tuning_apis_instance
...
from llm.types.passthrough_endpoints.vertex_ai import *
    ...
    if isinstance(config, dict):
        for key, value in config.items():
            if isinstance(value, str) and value.startswith("os.environ/"):
                config[key] = llm.get_secret(value)
...
def exception_handler(e: Exception):
    verbose_proxy_logger.error(
        "llm.proxy.proxy_server.v1/projects/tuningJobs(): Exception occurred - {}".format(
            str(e)
        )
    )
