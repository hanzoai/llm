...
Setup Prompt Injection Detection, PII Masking on Hanzo Proxy (AI Gateway)

## 1. Define guardrails on your Hanzo config.yaml
...
1. Connect Hanzo to a [supported logging provider](../logging)
...
The `guardrails` parameter can be passed to any Hanzo Proxy endpoint (`/chat/completions`, `/completions`, `/embeddings`).
