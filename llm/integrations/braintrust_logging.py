...
from llm.utils import print_verbose
...
class BraintrustLogger(CustomLogger):
    ...
    def add_metadata_from_header(llm_params: dict, metadata: dict) -> dict:
        ...
        proxy_headers = llm_params.get("proxy_server_request", {}).get("headers", {}) or {}
    ...
    async def create_default_project_and_experiment(self):
        project = await global_braintrust_http_handler.post(
            f"{self.api_base}/project", headers=self.headers, json={"name": "llm"}
        )
    ...
    def log_success_event(  # noqa: PLR0915
        ...
        try:
            ...
            if response_obj is not None and (
                kwargs.get("call_type", None) == "embedding" or isinstance(response_obj, llm.EmbeddingResponse)
            ):
                output = None
            elif response_obj is not None and isinstance(response_obj, llm.ModelResponse):
                output = response_obj["choices"][0]["message"].json()
                choices = response_obj["choices"]
            elif response_obj is not None and isinstance(response_obj, llm.TextCompletionResponse):
                output = response_obj.choices[0].text
                choices = response_obj.choices
            elif response_obj is not None and isinstance(response_obj, llm.ImageResponse):
                output = response_obj["data"]

            llm_params = kwargs.get("llm_params", {})
            metadata = llm_params.get("metadata", {}) or {}  # if llm_params['metadata'] == None
            ...
            if usage_obj and isinstance(usage_obj, llm.Usage):
                llm.utils.get_logging_id(start_time, response_obj)
                metrics = {
