...
async def new_organization(
    ...
    organization_row = LLM_OrganizationTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LLM_OrganizationTableWithMembers,
)
async def update_organization(
    data: LLM_OrganizationTableUpdate,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
):
    """
...
@router.delete(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[LLM_OrganizationTableWithMembers],
)
...
async def info_organization(organization_id: str):
    ...
    response_pydantic_obj = LLM_OrganizationTableWithMembers(
        **response.model_dump()
    )
...
async def find_member_if_email(
    ...
    """
    Find a member if the user_email is in LLM_UserTable
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Unique user not found for user_email={user_email}. Potential duplicate OR non-existent user_email in LLM_UserTable. Use 'user_id' instead."
            },
        )
    existing_user_email_row_pydantic = LLM_UserTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=LLM_OrganizationMembershipTable,
)
@management_endpoint_wrapper
async def organization_member_update(
    ...
    try:
        ...
        final_organization_membership_pydantic = LLM_OrganizationMembershipTable(
...
) -> Tuple[LLM_UserTable, LLM_OrganizationMembershipTable]:
    """
    Add a member to an organization

    - Checks if member.user_id or member.user_email is in LLM_UserTable
    - If not found, create a new user in LLM_UserTable
    - Add user to organization in LLM_OrganizationMembership
    """

    try:
        user_object: Optional[LLM_UserTable] = None
        ...
        if existing_user_id_row is None and member.user_email is not None:
            try:
                ...
                raise ValueError(
                    f"Potential NON-Existent or Duplicate user email in DB: Error finding a unique instance of user_email={member.user_email} in LLM_UserTable.: {e}"
                )
        ...
        if existing_user_id_row is None and existing_user_email_row is None:
            ...
            if _returned_user is not None:
                user_object = LLM_UserTable(**_returned_user.model_dump())
            ...
            user_object = LLM_UserTable(**existing_user_email_row.model_dump())
        elif existing_user_id_row is not None:
            user_object = LLM_UserTable(**existing_user_id_row.model_dump())
        else:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if user_object is None:
            raise ValueError(
                f"User does not exist in LLM_UserTable. user_id={member.user_id} and user_email={member.user_email}"
            )
        ...
        organization_membership = LLM_OrganizationMembershipTable(
