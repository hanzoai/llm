...
def generate_feedback_box():
    ...
    print(  # noqa
        "\033[1;37m"
        + "# {:^59} #\033[0m".format("https://github.com/BerriAI/llm/issues/new")
    )  # noqa
    print("\033[1;37m" + "#" + " " * box_width + "#\033[0m")  # noqa
    print("\033[1;37m" + "#" + "-" * box_width + "#\033[0m")  # noqa
    print()  # noqa
    print(" Thank you for using Hanzo! - Krrish & Ishaan")  # noqa
    print()  # noqa
    print()  # noqa
    print()  # noqa
    print(  # noqa
        "\033[1;31mGive Feedback / Get Help: https://github.com/BerriAI/llm/issues/new\033[0m"
    )  # noqa
    print()  # noqa
    print()  # noqa
...
from llm.proxy.auth.auth_checks import log_db_metrics
...
from llm.proxy.management_endpoints.team_endpoints import update_team
...
from llm.proxy.vertex_ai_endpoints.vertex_endpoints import router as vertex_router
from llm.proxy.vertex_ai_endpoints.vertex_endpoints import set_default_vertex_config
...
ui_message = (
    f"👉 [```Hanzo Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO"
)
ui_message += "\n\n💸 [```Hanzo Model Cost Map```](https://models.llm.ai/)."
...
async def proxy_shutdown_event():
    ...
    verbose_proxy_logger.info("Shutting down Hanzo Proxy Server")
    if prisma_client:
...
def custom_openapi():
    ...
    openai_routes = HanzoRoutes.openai_routes.value
    ...
    try:
        ...
        async def _update_user_db():
            ...
            if existing_user_obj is not None and isinstance(existing_user_obj, dict):
                existing_user_obj = Hanzo_UserTable(**existing_user_obj)
            try:
...
async def update_cache(  # noqa: PLR0915
    ...
    async def _update_key_cache(token: str, response_cost: float):
        ...
        existing_spend_obj: Hanzo_VerificationTokenView = await user_api_key_cache.async_get_cache(key=hashed_token)  # type: ignore
        verbose_proxy_logger.debug(
    ...
    async def _update_end_user_cache():
        ...
        try:
            ...
            existing_spend_obj = await user_api_key_cache.async_get_cache(key=_id)
            if existing_spend_obj is None:
                # if user does not exist in Hanzo_UserTable, create a new user
    ...
    async def _update_team_cache():
        ...
        try:
            # Fetch the existing cost for the given user
            existing_spend_obj: Optional[Hanzo_TeamTable] = (
...
def run_ollama_serve():
    try:
        ...
        verbose_proxy_logger.debug(
            f"""
            Hanzo Warning: proxy started with `ollama` model\n`ollama serve` failed with Exception{e}. \nEnsure you run `ollama serve`
        """
        )
...
class ProxyConfig:
    ...
    async def load_config(  # noqa: PLR0915
        ...
        if llm_settings:
            ...
            for key, value in llm_settings.items():
                if key == "cache" and value is True:
                    ...
                    if llm.cache is not None:
                        verbose_proxy_logger.debug(
                            f"{blue_color_code}Set Cache on Hanzo Proxy{reset_color_code}"
                        )
                    ...
                    if value is not None and isinstance(value, dict):
                        ...
                        llm.upperbound_key_generate_params = (
                            Hanzo_UpperboundKeyGenerateParams(**value)
                        )
                    else:
                        raise Exception(
        ...
        if model_list:
            ...
            print(  # noqa
                "\033[32mHanzo: Proxy initialized with Config, Set models:\033[0m"
            )  # noqa
    ...
    def get_model_info_with_id(self, model, db_model=False) -> RouterModelInfo:
        ...
        if premium_user is True:
            # seeing "created_at", "updated_at", "created_by", "updated_by" is a Hanzo Enterprise Feature
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        for m in db_models:
            ...
            if isinstance(_llm_params, dict):
                ...
                _llm_params = Hanzo_Params(**_llm_params)
...
class ProxyStartupEvent:
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("llm_jwtauth", None) is not None:
            ...
            llm_jwtauth = Hanzo_JWTAuth(**general_settings["llm_jwtauth"])
        else:
...
async def get_assistants(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as Hanzo adds more providers for acreate_batch
        if llm_router is None:
            raise HTTPException(
...
async def model_info_v2(
    ...
    if llm_model_list is None or not isinstance(llm_model_list, list):
        raise HTTPException(
            status_code=500,
            detail={
                "error": f"No model list passed, models={llm_model_list}. You can add a model through the config.yaml or on the Hanzo Admin UI."
            },
        )
    ...
    if user_model is not None:
        # if user does not use a config.yaml, https://github.com/BerriAI/llm/issues/2061
...
async def model_info_v1(  # noqa: PLR0915
    ...
    if llm_model_list is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Model List not loaded in. Make sure you passed models in your config.yaml or on the Hanzo Admin UI. - https://docs.llm.ai/docs/proxy/configs"
            },
        )
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Router is not loaded in. Make sure you passed models in your config.yaml or on the Hanzo Admin UI. - https://docs.llm.ai/docs/proxy/configs"
            },
        )
...
async def claim_onboarding_link(data: InvitationClaim):
    ...
    if invite_obj.is_accepted is True:
        ...
        raise HTTPException(
            status_code=401,
            detail={
                "error": "The invitation link was never validated. Please file an issue, if this is not intended - https://github.com/BerriAI/llm/issues."
            },
        )
...
async def new_invitation(
    ...
    curl -X POST 'http://localhost:4000/invitation/new' \
        -H 'Content-Type: application/json' \
        -d '{
            "user_id": "1234" // 👈 id of user in 'Hanzo_UserTable'
        }'
    ```
    """
    global prisma_client
    ...
    try:
        ...
        if "Foreign key constraint failed on the field" in str(e):
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "User id does not exist in 'Hanzo_UserTable'. Fix this by creating user via `/user/new`."
                },
            )
...
async def invitation_update(
    ...
    curl -X POST 'http://localhost:4000/invitation/update' \
        -H 'Content-Type: application/json' \
        -d '{
            "invitation_id": "1234" // 👈 id of invitation in 'Hanzo_InvitationTable'
...
async def update_config(config_info: ConfigYAML):  # noqa: PLR0915
    ...
    Currently supports modifying General Settings + Hanzo settings
    """
