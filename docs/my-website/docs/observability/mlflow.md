...
MLflow’s integration with Hanzo supports advanced observability compatible with OpenTelemetry.
...
To enable MLflow auto tracing for Hanzo:
...
# Alternative, you can set the callback manually in Hanzo
...
MLflow integration with Hanzo support tracking tool calls in addition to the messages.

```python
import mlflow

# Enable MLflow auto-tracing for Hanzo
mlflow.llm.autolog()
...
MLflow Hanzo integration allow you to run qualitative assessment against LLM to evaluate or/and monitor your GenAI application.

Visit [Evaluate LLMs Tutorial](../tutorials/eval_suites.md) for the complete guidance on how to run evaluation suite with Hanzo and MLflow.
...
## Combine Hanzo Trace with Your Application Trace

Hanzo is often part of larger LLM applications, such as agentic models. MLflow Tracing allows you to instrument custom Python code, which can then be combined with Hanzo traces.
...
This approach generates a unified trace, combining your custom Python code with Hanzo calls.
