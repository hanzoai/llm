...
from llm.proxy.proxy_server import Hanzo_TeamTable, UserAPIKeyAuth
...
def can_modify_guardrails(team_obj: Optional[Hanzo_TeamTable]) -> bool:
    if team_obj is None:
        return True
        ...
        if "guardrails" in data["metadata"]:
            ...
            if isinstance(request_guardrails, dict):
                ...
                for _guardrail_name, should_run in request_guardrails.items():
                    ...
                    guardrail_item: GuardrailItem = llm.guardrail_name_config_map[
                        _guardrail_name
                    ]
