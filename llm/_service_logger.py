...
class ServiceLogging(CustomLogger):
    """
    Separate class used for monitoring health of llm-adjacent services (redis/postgres).
    """
    ...
    def __init__(self, mock_testing: bool = False) -> None:
        ...
        if "prometheus_system" in llm.service_callback:
    ...
    async def async_service_success_hook(
        ...
        for callback in llm.service_callback:
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        """
        Hook to track latency for llm proxy llm api calls
        """
        try:
