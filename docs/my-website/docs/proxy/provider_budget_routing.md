...
# Budget Routing
Hanzo Supports setting the following budgets:
...
router_settings:
  ...
  # OPTIONAL: Set Redis Host, Port, and Password if using multiple instance of Hanzo
...
Hanzo will emit the following metric on Prometheus to track the remaining budget for each provider

This metric indicates the remaining budget for a provider in dollars (USD)

```
llm_provider_remaining_budget_metric{api_provider="openai"} 10
```
...
{
    "error": {
        "message": "No deployments available - crossed budget: Exceeded budget for deployment model_name: gpt-4o, llm_params.model: openai/gpt-4o, model_id: dbe80f2fe2b2465f7bfa9a5e77e0f143a2eb3f7d167a8b55fb7fe31aed62587f: 0.00015250000000000002 >= 1e-12",
        "type": "None",
        "param": "None",
        "code": "429"
    }
...
If you are using a multi-instance setup, you will need to set the Redis host, port, and password in the `proxy_config.yaml` file. Redis is used to sync the spend across Hanzo instances.
...
router_settings:
  ...
  # 👇 Add this: Set Redis Host, Port, and Password if using multiple instance of Hanzo
  redis_host: os.environ/REDIS_HOST
  redis_port: os.environ/REDIS_PORT
  redis_password: os.environ/REDIS_PASSWORD
...
- **Key**: Provider name (string) - Must be a valid [Hanzo provider name](https://docs.llm.ai/docs/providers)
