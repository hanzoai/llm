...
def test_pre_call_rule():
    try:
        llm.pre_call_rules = [my_pre_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
...
def test_post_call_rule():
    try:
        llm.pre_call_rules = []
        llm.post_call_rules = [my_post_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
        ...
        print(type(e))
        print(vars(e))
        assert e.message == "This violates LLM Proxy Rules. Response too short"
        pass
...
def test_post_call_rule_streaming():
    try:
        llm.pre_call_rules = []
        llm.post_call_rules = [my_post_call_rule_2]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "say sorry"}],
        ...
        print(type(e))
        print(vars(e))
        assert "This violates LLM Proxy Rules. Response too short" in e.message
...
async def test_post_call_processing_error_async_response():
    try:
        response = await acompletion(
            ...
            api_base="https://openai-proxy.hanzoai.repl.co",  # Just used as an example
