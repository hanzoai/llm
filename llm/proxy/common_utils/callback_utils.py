...
from llm.proxy._types import CommonProxyErrors, HanzoPromptInjectionParams
...
def initialize_callbacks_on_proxy(  # noqa: PLR0915
    ...
    if isinstance(value, list):
        ...
        for callback in value:  # ["presidio", <my-custom-callback>]
            if (
                ...
                presidio_logging_only: Optional[bool] = llm_settings.get(
                ...
                from llm.proxy.guardrails.guardrail_hooks.lakera_ai import (
                    lakeraAI_Moderation,
                )
                ...
                from llm.proxy.guardrails.guardrail_hooks.aporia_ai import (
                    AporiaGuardrail,
                )
                ...
                if "prompt_injection_params" in llm_settings:
                    prompt_injection_params_in_config = llm_settings[
                        "prompt_injection_params"
                    ]
                    prompt_injection_params = HanzoPromptInjectionParams(
                ...
                from llm.proxy.hooks.batch_redis_get import (
                ...
                from llm.proxy.hooks.azure_content_safety import (
                    _PROXY_AzureContentSafety,
                )

                azure_content_safety_params = llm_settings[
        ...
        if isinstance(llm.callbacks, list):
            llm.callbacks.extend(imported_list)
        else:
            llm.callbacks = imported_list  # type: ignore
    ...
    verbose_proxy_logger.debug(
        f"{blue_color_code} Initialized Callbacks - {llm.callbacks} {reset_color_code}"
    )


def get_model_group_from_llm_kwargs(kwargs: dict) -> Optional[str]:
    _llm_params = kwargs.get("llm_params", None) or {}
    _metadata = _llm_params.get("metadata", None) or {}
...
def get_remaining_tokens_and_requests_from_request_data(data: Dict) -> Dict[str, str]:
    """
    Helper function to return x-llm-key-remaining-tokens-{model_group} and x-llm-key-remaining-requests-{model_group}
    ...
    # Remaining Requests
    remaining_requests_variable_name = f"llm-key-remaining-requests-{model_group}"
    remaining_requests = _metadata.get(remaining_requests_variable_name, None)
    if remaining_requests:
        headers[f"x-llm-key-remaining-requests-{model_group}"] = remaining_requests
    ...
    if remaining_tokens:
        headers[f"x-llm-key-remaining-tokens-{model_group}"] = remaining_tokens
...
def get_logging_caching_headers(request_data: Dict) -> Optional[Dict]:
    ...
    if "applied_guardrails" in _metadata:
        headers["x-llm-applied-guardrails"] = ",".join(
            _metadata["applied_guardrails"]
        )

    if "semantic-similarity" in _metadata:
        headers["x-llm-semantic-similarity"] = str(_metadata["semantic-similarity"])
