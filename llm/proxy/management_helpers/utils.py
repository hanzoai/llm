...
from llm.proxy._types import (  # key request types; user request types; team request types; customer request types
...
def get_new_internal_user_defaults(
    ...
    user_info = llm.default_internal_user_params or {}

    returned_dict: SSOUserDefinedValues = {
        "models": user_info.get("models", None),
        "max_budget": user_info.get("max_budget", llm.max_internal_user_budget),
        "budget_duration": user_info.get(
            "budget_duration", llm.internal_user_budget_duration
        ),
...
async def add_new_member(
...
) -> Tuple[Hanzo_UserTable, Optional[Hanzo_TeamMembership]]:
    """
    ...
    """
    returned_user: Optional[Hanzo_UserTable] = None
    returned_team_membership: Optional[Hanzo_TeamMembership] = None
    ...
    if new_member.user_id is not None:
        ...
        if _returned_user is not None:
            returned_user = Hanzo_UserTable(**_returned_user.model_dump())
    ...
    if (
        ...
        returned_team_membership = Hanzo_TeamMembership(
...
async def send_management_endpoint_alert(
    ...
    """
    from llm.proxy.proxy_server import premium_user, proxy_logging_obj
