...
import llm
from llm.secret_managers.main import get_secret, get_secret_str
...
def _is_non_openai_azure_model(model: str) -> bool:
    try:
        ...
        if (
            model_name in llm.cohere_chat_models
            or f"mistral/{model_name}" in llm.mistral_chat_models
        ):
            return True
    except Exception:
        return False
...
def handle_cohere_chat_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if custom_llm_provider == "cohere" and model in llm.cohere_chat_models:
    ...
    if "/" in model:
        ...
        if (
            ...
            and _model in llm.cohere_chat_models
        ):
...
def handle_anthropic_text_model_custom_llm_provider(
    ...
    if custom_llm_provider:
        if (
            ...
            and llm.AnthropicTextConfig._is_anthropic_text_model(model)
        ):
    ...
    if "/" in model:
        ...
        if (
            ...
            and llm.AnthropicTextConfig._is_anthropic_text_model(_model)
        ):
...
def get_llm_provider(  # noqa: PLR0915
    ...
    llm_params: Optional[LLM_Params] = None,
) -> Tuple[str, str, Optional[str], Optional[str]]:
    """
    Returns the provider for a given model name - e.g. 'azure/chatgpt-v-2' -> 'azure'

    For router -> Can also give the whole llm param dict -> this function will extract the relevant details
    ...
    try:
        ...
        if llm_params is not None:
            ...
            ), "Either pass in llm_params or the custom_llm_provider/api_base/api_key. Otherwise, these values will be overriden."
        ...
        if (
            ...
            and len(model.split("/"))
            > 1  # handle edge case where user passes in `llm --model mistral` https://github.com/hanzoai/llm/issues/1351
        ):
        ...
        elif model.split("/", 1)[0] in llm.provider_list:
        ...
        if api_base:
            for endpoint in llm.openai_compatible_endpoints:
        ...
        if (
            ...
            custom_llm_provider = "openai"
        elif model in llm.open_ai_text_completion_models:
            custom_llm_provider = "text-completion-openai"
        ## anthropic
        elif model in llm.anthropic_models:
            if llm.AnthropicTextConfig._is_anthropic_text_model(model):
                custom_llm_provider = "anthropic_text"
            else:
                custom_llm_provider = "anthropic"
        ## cohere
        elif model in llm.cohere_models or model in llm.cohere_embedding_models:
            custom_llm_provider = "cohere"
        ## cohere chat models
        elif model in llm.cohere_chat_models:
            custom_llm_provider = "cohere_chat"
        ## replicate
        elif model in llm.replicate_models or (":" in model and len(model) > 64):
        ...
        ## maritalk
        elif model in llm.maritalk_models:
        ...
        ## vertex - text + chat + language (gemini) models
            ...
            or model in llm.vertex_embedding_models
            or model in llm.vertex_vision_models
            or model in llm.vertex_ai_image_models
        ):
        ...
        ## petals
        elif model in llm.petals_models:
            custom_llm_provider = "petals"
        ## bedrock
        elif (
            model in llm.bedrock_models
            or model in llm.bedrock_embedding_models
            or model in llm.bedrock_converse_models
        ):
            custom_llm_provider = "bedrock"
        elif model in llm.watsonx_models:
            custom_llm_provider = "watsonx"
        # openai embeddings
        elif model in llm.open_ai_embedding_models:
            custom_llm_provider = "openai"
        elif model in llm.empower_models:
            custom_llm_provider = "empower"
        elif model == "*":
            custom_llm_provider = "openai"
        if not custom_llm_provider:
            if llm.suppress_debug_info is False:
                print()  # noqa
                print(  # noqa
                    "\033[1;31mProvider List: https://docs.hanzo.ai/docs/providers\033[0m"  # noqa
                )  # noqa
                print()  # noqa
            error_str = f"LLM Provider NOT provided. Pass in the LLM provider you are trying to call. You passed model={model}\n Pass model as E.g. For 'Huggingface' inference endpoints pass in `completion(model='huggingface/starcoder',..)` Learn more: https://docs.hanzo.ai/docs/providers"
            # maps to openai.NotFoundError, this is raised when openai does not recognize the llm
            raise llm.exceptions.BadRequestError(  # type: ignore
                message=error_str,
                model=model,
        ...
        if isinstance(e, llm.exceptions.BadRequestError):
            raise e
        else:
            error_str = (
...
def _get_openai_compatible_provider_info(  # noqa: PLR0915
    ...
    if custom_llm_provider == "perplexity":
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.PerplexityChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.DeepInfraConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.GroqChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.HostedVLLMChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.LMStudioChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.FireworksAIConfig()._get_openai_compatible_provider_info(
        ...
        (
            ...
            dynamic_api_key,
            custom_llm_provider,
        ) = llm.AzureAIStudioConfig()._get_openai_compatible_provider_info(
    ...
    elif custom_llm_provider == "llm_proxy":
        api_base = api_base or get_secret_str("LLM_PROXY_API_BASE")
        dynamic_api_key = api_key or get_secret_str("LLM_PROXY_API_KEY")

    elif custom_llm_provider == "mistral":
        (
            api_base,
            dynamic_api_key,
        ) = llm.MistralConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            ...
            api_base,
            dynamic_api_key,
        ) = llm.JinaAIEmbeddingConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
        ...
        (
            api_base,
            dynamic_api_key,
        ) = llm.XAIChatConfig()._get_openai_compatible_provider_info(
            api_base, api_key
        )
