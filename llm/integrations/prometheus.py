# used for /metrics endpoint on Hanzo Proxy
#### What this does ####
...
from llm.proxy._types import Hanzo_TeamTable, UserAPIKeyAuth
from llm.types.integrations.prometheus import *
...
class PrometheusLogger(CustomLogger):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            from llm.proxy.proxy_server import CommonProxyErrors, premium_user

            if premium_user is not True:
                verbose_logger.warning(
                    f"🚨🚨🚨 Prometheus Metrics is on Hanzo Enterprise\n🚨 {CommonProxyErrors.not_premium_user.value}"
                )
                self.llm_not_a_premium_user_metric = Counter(
                    name="llm_not_a_premium_user_metric",
                    documentation=f"🚨🚨🚨 Prometheus Metrics is on Hanzo Enterprise. 🚨 {CommonProxyErrors.not_premium_user.value}",
                )
                return

            self.llm_proxy_failed_requests_metric = Counter(
                name="llm_proxy_failed_requests_metric",
                documentation="Total number of failed responses from proxy - the client did not get a success response from llm proxy",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_proxy_failed_requests_metric"
                ),
            )

            self.llm_proxy_total_requests_metric = Counter(
                name="llm_proxy_total_requests_metric",
                documentation="Total number of requests made to the proxy server - track number of client side requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_proxy_total_requests_metric"
                ),
            )

            # request latency metrics
            self.llm_request_total_latency_metric = Histogram(
                "llm_request_total_latency_metric",
                "Total latency (seconds) for a request to Hanzo",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_request_total_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            )

            self.llm_llm_api_latency_metric = Histogram(
                "llm_llm_api_latency_metric",
                "Total latency (seconds) for a models LLM API call",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_llm_api_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            )

            self.llm_llm_api_time_to_first_token_metric = Histogram(
                "llm_llm_api_time_to_first_token_metric",
            ...
            self.llm_input_tokens_metric = Counter(
                "llm_input_tokens",
                "Total number of input tokens from LLM requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_input_tokens_metric"
                ),
            )

            self.llm_output_tokens_metric = Counter(
                "llm_output_tokens",
                "Total number of output tokens from LLM requests",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_output_tokens_metric"
                ),
            )

            # Remaining Budget for Team
            self.llm_remaining_team_budget_metric = Gauge(
                "llm_remaining_team_budget_metric",
                "Remaining budget for team",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_remaining_team_budget_metric"
                ),
            )

            # Max Budget for Team
            self.llm_team_max_budget_metric = Gauge(
                "llm_team_max_budget_metric",
                "Maximum budget set for team",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_team_max_budget_metric"
                ),
            )

            # Team Budget Reset At
            self.llm_team_budget_remaining_hours_metric = Gauge(
                "llm_team_budget_remaining_hours_metric",
                "Remaining days for team budget to be reset",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_team_budget_remaining_hours_metric"
                ),
            )

            # Remaining Budget for API Key
            self.llm_remaining_api_key_budget_metric = Gauge(
                "llm_remaining_api_key_budget_metric",
                "Remaining budget for api key",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_remaining_api_key_budget_metric"
                ),
            )

            # Max Budget for API Key
            self.llm_api_key_max_budget_metric = Gauge(
                "llm_api_key_max_budget_metric",
                "Maximum budget set for api key",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_api_key_max_budget_metric"
                ),
            )

            self.llm_api_key_budget_remaining_hours_metric = Gauge(
                "llm_api_key_budget_remaining_hours_metric",
                "Remaining hours for api key budget to be reset",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_api_key_budget_remaining_hours_metric"
                ),
            )

            ########################################
            # Hanzo Virtual API KEY metrics
            ########################################
            # Remaining MODEL RPM limit for API Key
            self.llm_remaining_api_key_requests_for_model = Gauge(
                "llm_remaining_api_key_requests_for_model",
                "Remaining Requests API Key can make for model (model based rpm limit on key)",
                labelnames=["hashed_api_key", "api_key_alias", "model"],
            )

            # Remaining MODEL TPM limit for API Key
            self.llm_remaining_api_key_tokens_for_model = Gauge(
                "llm_remaining_api_key_tokens_for_model",
            ...
            self.llm_remaining_requests_metric = Gauge(
            ...
            self.llm_remaining_tokens_metric = Gauge(
                "llm_remaining_tokens",
            ...
            self.llm_overhead_latency_metric = Histogram(
                ...
                "Latency overhead (milliseconds) added by Hanzo processing",
                labelnames=[
                    "model_group",
                    "api_provider",
                    "api_base",
            ...
            self.llm_provider_remaining_budget_metric = Gauge(
                "llm_provider_remaining_budget_metric",
            ...
            self.llm_deployment_state = Gauge(
                "llm_deployment_state",
                "LLM Deployment Analytics - The state of the deployment: 0 = healthy, 1 = partial outage, 2 = complete outage",
                labelnames=_logged_llm_labels,
            )

            self.llm_deployment_cooled_down = Counter(
                "llm_deployment_cooled_down",
                "LLM Deployment Analytics - Number of times a deployment has been cooled down by Hanzo load balancing logic. exception_status is the status of the exception that caused the deployment to be cooled down",
            ...
            self.llm_deployment_failure_by_tag_responses = Counter(
                "llm_deployment_failure_by_tag_responses",
            ...
            self.llm_deployment_total_requests = Counter(
                ...
                documentation="LLM Deployment Analytics - Total number of LLM API calls via llm - success + failure",
                labelnames=[REQUESTED_MODEL] + _logged_llm_labels + team_and_key_labels,
            )
            ...
            self.llm_deployment_latency_per_output_token = Histogram(
                ...
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_deployment_latency_per_output_token"
                ),
            )
            ...
            self.llm_llm_api_failed_requests_metric = Counter(
                name="llm_llm_api_failed_requests_metric",
                documentation="deprecated - use llm_proxy_failed_requests_metric",
                labelnames=[
                    "end_user",
                    "hashed_api_key",
                    "api_key_alias",
            ...
            self.llm_requests_metric = Counter(
                name="llm_requests_metric",
                documentation="deprecated - use llm_proxy_total_requests_metric. Total number of LLM calls to llm - track total per API Key, team, user",
                labelnames=PrometheusMetricLabels.get_labels(
                    label_name="llm_requests_metric"
                ),
            )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _metadata = llm_params.get("metadata", {})
        end_user_id = get_end_user_id_for_cost_tracking(
            llm_params, service_type="prometheus"
        )
        ...
        if (
            ...
            self.llm_proxy_total_requests_metric.labels(**_labels).inc()
    ...
    def _increment_token_metrics(
        ...
        self.llm_input_tokens_metric.labels(**_labels).inc(
        ...
        self.llm_output_tokens_metric.labels(**_labels).inc(
    ...
    async def _increment_remaining_budget_metrics(
        ...
        _team_spend = llm_params.get("metadata", {}).get(
            "user_api_key_team_spend", None
        )
        _team_max_budget = llm_params.get("metadata", {}).get(
            "user_api_key_team_max_budget", None
        )

        _api_key_spend = llm_params.get("metadata", {}).get(
            "user_api_key_spend", None
        )
        _api_key_max_budget = llm_params.get("metadata", {}).get(
    ...
    def _set_virtual_key_rate_limit_metrics(
        ...
        from llm.proxy.common_utils.callback_utils import (
            get_model_group_from_llm_kwargs,
        )

        # Set remaining rpm/tpm for API Key + model
        # see parallel_request_limiter.py - variables are set there
        model_group = get_model_group_from_llm_kwargs(kwargs)
        remaining_requests_variable_name = (
            f"llm-key-remaining-requests-{model_group}"
        )
        remaining_tokens_variable_name = f"llm-key-remaining-tokens-{model_group}"
        ...
        self.llm_remaining_api_key_requests_for_model.labels(
            user_api_key, user_api_key_alias, model_group
        ).set(remaining_requests)

        self.llm_remaining_api_key_tokens_for_model.labels(
            user_api_key, user_api_key_alias, model_group
    ...
    def _set_latency_metrics(
        ...
        if (
            ...
            self.llm_llm_api_time_to_first_token_metric.labels(
                model,
                user_api_key,
                user_api_key_alias,
                user_api_team,
        ...
        if api_call_total_time_seconds is not None:
            ...
            self.llm_llm_api_latency_metric.labels(**_labels).observe(
        ...
        if total_time_seconds is not None:
            ...
            self.llm_request_total_latency_metric.labels(**_labels).observe(
                total_time_seconds
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            self.llm_llm_api_failed_requests_metric.labels(
                end_user_id,
                user_api_key,
                user_api_key_alias,
                model,
    ...
    async def async_post_call_failure_hook(
        ...
        try:
            ...
            self.llm_proxy_failed_requests_metric.labels(**_labels).inc()
    ...
    def set_llm_deployment_failure_metrics(self, request_kwargs: dict):
        ...
        try:
            ...
            _llm_params = request_kwargs.get("llm_params", {}) or {}
            llm_model_name = request_kwargs.get("model", None)
            ...
            llm_provider = _llm_params.get("custom_llm_provider", None)
            ...
            if standard_logging_payload is not None and isinstance(
                ...
                _tags = standard_logging_payload["request_tags"]
                for tag in _tags:
                    self.llm_deployment_failure_by_tag_responses.labels(
                        **{
                            UserAPIKeyLabelNames.REQUESTED_MODEL.value: model_group,
                            UserAPIKeyLabelNames.TAG.value: tag,
                            UserAPIKeyLabelNames.v2_LLM_MODEL_NAME.value: llm_model_name,
    ...
    def set_llm_deployment_success_metrics(
        ...
        try:
            ...
            _llm_params = request_kwargs.get("llm_params", {}) or {}
            _metadata = _llm_params.get("metadata", {})
            llm_model_name = request_kwargs.get("model", None)
            llm_provider = _llm_params.get("custom_llm_provider", None)
            ...
            if remaining_requests:
                ...
                "llm_model_name"
                """
                self.llm_remaining_requests_metric.labels(
                    model_group,
                    llm_provider,
                    api_base,
                    llm_model_name,
            ...
            if remaining_tokens:
                self.llm_remaining_tokens_metric.labels(
                    model_group,
                    llm_provider,
                    api_base,
                    llm_model_name,
            ...
            """
            log these labels
            ["llm_model_name", "requested_model", model_id", "api_base", "api_provider"]
            """
    ...
    async def log_success_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=PrometheusMetricLabels.get_labels(
                label_name="llm_deployment_successful_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.llm_deployment_successful_fallbacks.labels(**_labels).inc()
    ...
    async def log_failure_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=PrometheusMetricLabels.get_labels(
                label_name="llm_deployment_failed_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.llm_deployment_failed_fallbacks.labels(**_labels).inc()

    def set_llm_deployment_state(
        self,
        state: int,
        llm_model_name: str,
        ...
        self.llm_deployment_state.labels(
            llm_model_name, model_id, api_base, api_provider
        ).set(state)
    ...
    def set_deployment_healthy(
        ...
        self.set_llm_deployment_state(
            0, llm_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_partial_outage(
        ...
        self.set_llm_deployment_state(
            1, llm_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_complete_outage(
        ...
        self.set_llm_deployment_state(
            2, llm_model_name, model_id, api_base, api_provider
        )
    ...
    def increment_deployment_cooled_down(
        ...
        """
        increment metric when llm.Router / load balancing logic places a deployment in cool down
        """
        self.llm_deployment_cooled_down.labels(
    ...
    def track_provider_remaining_budget(
        ...
        """
        self.llm_provider_remaining_budget_metric.labels(provider).set(
    ...
    def _initialize_prometheus_startup_metrics(self):
        ...
        """
        if llm.prometheus_initialize_budget_metrics is not True:
    ...
    async def _initialize_budget_metrics(
        ...
        """
        from llm.proxy.proxy_server import prisma_client
    ...
    async def _initialize_team_budget_metrics(self):
        ...
        ) -> Tuple[List[Hanzo_TeamTable], Optional[int]]:
    ...
    async def _initialize_api_key_budget_metrics(self):
        ...
        from llm.constants import UI_SESSION_TOKEN_TEAM_ID
    ...
    async def _set_team_list_budget_metrics(self, teams: List[Hanzo_TeamTable]):
    ...
    async def _set_team_budget_metrics_after_api_request(
        ...
        - Assemble a Hanzo_TeamTable object
    ...
    async def _assemble_team_object(
        ...
        - `budget_reset_at`
        """
        from llm.proxy.auth.auth_checks import get_team_object
        from llm.proxy.proxy_server import prisma_client, user_api_key_cache
    ...
    def _set_team_budget_metrics(
        ...
        self.llm_remaining_team_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if team.max_budget is not None:
            ...
            self.llm_team_max_budget_metric.labels(**_labels).set(team.max_budget)
        ...
        if team.budget_reset_at is not None:
            ...
            self.llm_team_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    def _set_key_budget_metrics(self, user_api_key_dict: UserAPIKeyAuth):
        ...
        self.llm_remaining_api_key_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if user_api_key_dict.max_budget is not None:
            ...
            self.llm_api_key_max_budget_metric.labels(**_labels).set(
                user_api_key_dict.max_budget
            )

        if user_api_key_dict.budget_reset_at is not None:
            self.llm_api_key_budget_remaining_hours_metric.labels(**_labels).set(
                self._get_remaining_hours_for_budget_reset(
    ...
    async def _assemble_key_object(
        ...
        """
        from llm.proxy.auth.auth_checks import get_key_object
        from llm.proxy.proxy_server import prisma_client, user_api_key_cache
...
def prometheus_label_factory(
    ...
    if UserAPIKeyLabelNames.END_USER.value in filtered_labels:
        filtered_labels["end_user"] = get_end_user_id_for_cost_tracking(
            llm_params={"user_api_key_end_user_id": enum_values.end_user},
...
def get_custom_labels_from_metadata(metadata: dict) -> Dict[str, str]:
    ...
    """
    keys = llm.custom_prometheus_metadata_labels
