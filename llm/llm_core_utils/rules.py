...
class Rules:
    ...
    def pre_call_rules(self, input: str, model: str):
        for rule in llm.pre_call_rules:
            if callable(rule):
                decision = rule(input)
                if decision is False:
                    raise llm.APIResponseValidationError(message="LLM Response failed post-call-rule check", llm_provider="", model=model)  # type: ignore
        return True

    def post_call_rules(self, input: Optional[str], model: str) -> bool:
        if input is None:
            return True
        for rule in llm.post_call_rules:
            if callable(rule):
                decision = rule(input)
                if isinstance(decision, bool):
                    ...
                    if decision_val is False:
                        raise llm.APIResponseValidationError(message=decision_message, llm_provider="", model=model)  # type: ignore
        return True
