...
def _clean_endpoint_data(endpoint_data: dict, details: Optional[bool] = True):
    ...
    """
    endpoint_data.pop("llm_logging_obj", None)
    return (
    ...
    for is_healthy, model in zip(results, model_list):
        ...
        if isinstance(is_healthy, dict) and "error" not in is_healthy:
            healthy_endpoints.append(
                _clean_endpoint_data({**llm_params, **is_healthy}, details)
            )
            ...
            unhealthy_endpoints.append(_clean_endpoint_data(llm_params, details))

    return healthy_endpoints, unhealthy_endpoints


def _update_llm_params_for_health_check(
    model_info: dict, llm_params: dict
) -> dict:
    """
    Update the llm params for health check.

    - gets a short `messages` param for health check
    - updates the `model` param with the `health_check_model` if it exists Doc: https://docs.hanzo.ai/docs/proxy/health#wildcard-routes
    """
    llm_params["messages"] = _get_random_llm_message()
    _health_check_model = model_info.get("health_check_model", None)
    if _health_check_model is not None:
        llm_params["model"] = _health_check_model
    return llm_params
...
async def perform_health_check(
    ...
    if not model_list:
        if cli_model:
            model_list = [
                {"model_name": cli_model, "llm_params": {"model": cli_model}}
            ]
        else:
            return [], []
    ...
    if model is not None:
        _new_model_list = [
            x for x in model_list if x["llm_params"]["model"] == model
        ]
        if _new_model_list == []:
