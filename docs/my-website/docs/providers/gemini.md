...
| Provider Route on Hanzo | `gemini/` |
...
### Response schema 
Hanzo supports sending `response_schema` as a param for Gemini-1.5-Pro on Google AI Studio. 
...
To pass additional GenerationConfig params - e.g. `topK`, just pass it in the request body of the call, and Hanzo will pass it straight through as a key-value pair in the request body. 
...
Hanzo follows the OpenAI format and accepts sending inline data as an encoded base64 string. 
