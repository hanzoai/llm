...
import llm
import llm.types
import llm.types.llms
from llm import verbose_logger
from llm.llms.custom_httpx.http_handler import HTTPHandler, get_async_httpx_client
from llm.types.llms.anthropic import *
from llm.types.llms.bedrock import MessageBlock as BedrockMessageBlock
from llm.types.llms.custom_http import httpxSpecialProvider
from llm.types.llms.ollama import OllamaVisionModelObject
...
from llm.types.llms.vertex_ai import FunctionCall as VertexFunctionCall
from llm.types.llms.vertex_ai import FunctionResponse as VertexFunctionResponse
from llm.types.llms.vertex_ai import PartType as VertexPartType
from llm.types.utils import GenericImageParsingChunk
...
}  # similar to autogen. Only used if `llm.modify_params=True`.
...
def hf_chat_template(  # noqa: PLR0915
    ...
    if chat_template is None:
        ...
        if model in llm.known_tokenizer_config:
            tokenizer_config = llm.known_tokenizer_config[model]
        else:
            tokenizer_config = _get_tokenizer_config(model)
            llm.known_tokenizer_config.update({model: tokenizer_config})
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1. Anthropic supports roles like "user" and "assistant", (here llm translates system-> assistant)
    2. The first message always needs to be of role "user"
    3. Each message must alternate between "user" and "assistant" (this is not addressed as now by llm)
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if llm.modify_params:
            new_messages.insert(
                0, {"role": "user", "content": [{"type": "text", "text": "."}]}
            )
        else:
            raise Exception(
                "Invalid first message. Should always start with 'role'='user' for Anthropic. System prompt is sent separately for Anthropic. set 'llm.modify_params = True' or 'llm_settings:modify_params = True' on proxy, to insert a placeholder user message - '.' as the first message, "
            )
...
from llm.types.llms.cohere import (
    CallObject,
    ChatHistory,
    ChatHistoryChatBot,
    ChatHistorySystem,
...
def amazon_titan_pt(
    messages: list,
):  # format - https://github.com/hanzoai/llm/issues/1896
    """
...
from llm.types.llms.bedrock import ContentBlock as BedrockContentBlock
from llm.types.llms.bedrock import DocumentBlock as BedrockDocumentBlock
from llm.types.llms.bedrock import ImageBlock as BedrockImageBlock
from llm.types.llms.bedrock import SourceBlock as BedrockSourceBlock
from llm.types.llms.bedrock import ToolBlock as BedrockToolBlock
from llm.types.llms.bedrock import (
    ToolInputSchemaBlock as BedrockToolInputSchemaBlock,
)
from llm.types.llms.bedrock import ToolResultBlock as BedrockToolResultBlock
from llm.types.llms.bedrock import (
    ToolResultContentBlock as BedrockToolResultContentBlock,
)
from llm.types.llms.bedrock import ToolSpecBlock as BedrockToolSpecBlock
from llm.types.llms.bedrock import ToolUseBlock as BedrockToolUseBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        supported_image_formats = (
            llm.AmazonConverseConfig().get_supported_image_types()
        )
        supported_doc_formats = (
            llm.AmazonConverseConfig().get_supported_document_types()
        )
...
def _insert_assistant_continue_message(
    ...
    Conversation blocks and tool result blocks cannot be provided in the same turn. Issue: https://github.com/hanzoai/llm/issues/6053
    """
...
def get_user_message_block_or_continue_message(
    ...
    Relevant Issue: https://github.com/hanzoai/llm/issues/7169
    """
    content_block = message.get("content", None)

    # Handle None case
    if content_block is None or (
        user_continue_message is None and llm.modify_params is False
    ):
...
def get_assistant_message_block_or_continue_message(
    ...
    # Handle Base case
    if content_block is None or (
        assistant_continue_message is None and llm.modify_params is False
    ):
        return skip_empty_text_blocks(message=message)
                ...
                if isinstance(message_block["content"], list):
                    ...
                    for element in message_block["content"]:
                        if isinstance(element, dict):
                            ...
                            _cache_point_block = (
                                llm.AmazonConverseConfig()._get_cache_point_block(
                                    message_block=cast(
...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    if input_tool_name != valid_string:
        ...
        llm.bedrock_tool_name_mappings.set_cache(
...
def _bedrock_tools_pt(tools: List) -> List[BedrockToolBlock]:
    ...
    for tool in tools:
        ...
        # related issue: https://github.com/hanzoai/llm/issues/5007
...
def response_schema_prompt(model: str, response_schema: dict) -> str:
    ...
    if f"{model}/response_schema_prompt" in llm.custom_prompt_dict:

        custom_prompt_details = llm.custom_prompt_dict[
            f"{model}/response_schema_prompt"
        ]  # allow user to define custom response schema prompt by model
    elif "response_schema_prompt" in llm.custom_prompt_dict:
        custom_prompt_details = llm.custom_prompt_dict["response_schema_prompt"]
...
def prompt_factory(
    ...
    if custom_llm_provider == "ollama":
        ...
        if (
            ...
            or llm.supports_vision(model=model)
            or llm.supports_vision(model=custom_llm_provider + "/" + model)
        ):
            return _gemini_vision_convert_messages(messages=messages)
        else:
            return gemini_text_image_pt(messages=messages)
    elif custom_llm_provider == "mistral":
        return llm.MistralConfig()._transform_messages(
