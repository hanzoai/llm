...
from llm.types.utils import ModelResponse, PromptTokensDetailsWrapper, Usage
from llm.utils import add_dummy_tool, has_tool_call_blocks
...
class AmazonConverseConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> List[str]:
        ...
        if llm.utils.supports_tool_choice(
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            if llm.drop_params is True or drop_params is True:
                ...
                raise llm.utils.UnsupportedParamsError(
                    message="Bedrock doesn't support tool_choice={}. To drop it from the call, set `llm.drop_params = True.".format(
                        tool_choice
                    ),
                    status_code=400,
                )
            ...
            raise llm.utils.UnsupportedParamsError(
                message="Bedrock doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `llm.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def _transform_request_helper(
        ...
        if (
            ...
            if llm.modify_params:
                ...
                raise llm.UnsupportedParamsError(
                    message="Bedrock doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `llm.modify_params = True` // `llm_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
    ...
    async def _async_transform_request(
        ...
        bedrock_messages = (
            await BedrockConverseMessagesProcessor._bedrock_converse_messages_pt_async(
                ...
                user_continue_message=llm_params.pop("user_continue_message", None),
            )
        )
    ...
    def _transform_response(
        ...
        model_response.choices = [
            llm.Choices(
                ...
                index=0,
                message=llm.Message(**chat_completion_message),
            )
        ]
