...
from llm.caching.caching import Cache, DualCache, RedisCache, InMemoryCache
from llm.caching.llm_caching_handler import LLMClientCache
from llm.types.llms.bedrock import COHERE_EMBEDDING_INPUT_TYPES
...
email: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/hanzoai/llm/discussions/648
)
token: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/hanzoai/llm/discussions/648
)
...
caching: bool = (
    False  # Not used anymore, will be removed in next MAJOR release - https://github.com/hanzoai/llm/discussions/648
)
caching_with_models: bool = (
    False  # # Not used anymore, will be removed in next MAJOR release - https://github.com/hanzoai/llm/discussions/648
)
cache: Optional[Cache] = (
    None  # cache object <- use this - https://docs.hanzo.ai/docs/caching
)
...
default_soft_budget: float = (
    50.0  # by default all llm proxy keys have a soft budget of 50.0
)
...
upperbound_key_generate_params: Optional[LLM_UpperboundKeyGenerateParams] = None
...
force_ipv4: bool = (
    False  # when True, llm will force ipv4 for all LLM requests. Some users have seen httpx ConnectionError when using ipv6.
)
...
def add_known_models():
    for key, value in model_cost.items():
        ...
        elif value.get("llm_provider") == "vertex_ai-language-models":
            vertex_language_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-vision-models":
        ...
        elif value.get("llm_provider") == "vertex_ai-embedding-models":
            vertex_embedding_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-anthropic_models":
            key = key.replace("vertex_ai/", "")
            vertex_anthropic_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-llama_models":
            key = key.replace("vertex_ai/", "")
            vertex_llama3_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-mistral_models":
            key = key.replace("vertex_ai/", "")
            vertex_mistral_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-ai21_models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_ai21_models.append(key)
        elif value.get("llm_provider") == "vertex_ai-image-models":
            key = key.replace("vertex_ai/", "")
        ...
        elif value.get("llm_provider") == "bedrock_converse":
            bedrock_converse_models.append(key)
        elif value.get("llm_provider") == "deepinfra":
            deepinfra_models.append(key)
        elif value.get("llm_provider") == "perplexity":
            perplexity_models.append(key)
        elif value.get("llm_provider") == "watsonx":
            watsonx_models.append(key)
        elif value.get("llm_provider") == "gemini":
            gemini_models.append(key)
        elif value.get("llm_provider") == "fireworks_ai":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key and "fireworks-ai-default" not in key:
                fireworks_ai_models.append(key)
        elif value.get("llm_provider") == "fireworks_ai-embedding-models":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key:
                fireworks_ai_embedding_models.append(key)
        elif value.get("llm_provider") == "text-completion-codestral":
            text_completion_codestral_models.append(key)
        elif value.get("llm_provider") == "xai":
            xai_models.append(key)
        elif value.get("llm_provider") == "deepseek":
            deepseek_models.append(key)
        elif value.get("llm_provider") == "azure_ai":
            azure_ai_models.append(key)
        elif value.get("llm_provider") == "voyage":
            voyage_models.append(key)
        elif value.get("llm_provider") == "databricks":
            databricks_models.append(key)
        elif value.get("llm_provider") == "cloudflare":
            cloudflare_models.append(key)
        elif value.get("llm_provider") == "codestral":
            codestral_models.append(key)
        elif value.get("llm_provider") == "friendliai":
            friendliai_models.append(key)
        elif value.get("llm_provider") == "palm":
            palm_models.append(key)
        elif value.get("llm_provider") == "groq":
            groq_models.append(key)
        elif value.get("llm_provider") == "azure":
            azure_models.append(key)
        elif value.get("llm_provider") == "anyscale":
            anyscale_models.append(key)
        elif value.get("llm_provider") == "cerebras":
            cerebras_models.append(key)
        elif value.get("llm_provider") == "galadriel":
            galadriel_models.append(key)
        elif value.get("llm_provider") == "sambanova_models":
            sambanova_models.append(key)
        elif value.get("llm_provider") == "assemblyai":
            assemblyai_models.append(key)
        elif value.get("llm_provider") == "jina_ai":
            jina_ai_models.append(key)
        elif value.get("llm_provider") == "snowflake":
...
from llm.llm_core_utils.llm_logging import Logging, modify_integration
from llm.llm_core_utils.get_llm_provider_logic import get_llm_provider
from llm.llm_core_utils.core_helpers import remove_index_from_tool_calls
from llm.llm_core_utils.token_counter import get_modified_max_tokens
from .utils import (
    client,
    exception_type,
    get_optional_params,
...
from llm.llms.openai.completion.transformation import OpenAITextCompletionConfig
...
from .llms.llm_proxy.chat.transformation import LLMProxyChatConfig
