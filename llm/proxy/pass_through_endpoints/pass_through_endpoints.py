...
from llm.llms.custom_httpx.http_handler import get_async_httpx_client
...
from llm.types.utils import StandardLoggingUserAPIKeyMetadata
...
async def set_env_variables_in_header(custom_headers: Optional[dict]) -> Optional[dict]:
    ...
    for key, value in custom_headers.items():
        # langfuse Api requires base64 encoded headers - it's simpleer to just ask llm users to set their langfuse public and secret keys
...
async def chat_completion_pass_through_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if "api_key" in data:
            llm_response = asyncio.create_task(llm.aadapter_completion(**data))
        elif (
            ...
            llm_response = asyncio.create_task(llm_router.aadapter_completion(**data))
        elif user_model is not None:  # `llm --model <your-model-name>`
            llm_response = asyncio.create_task(llm.aadapter_completion(**data))
        else:
            raise HTTPException(
...
class HttpPassThroughEndpointHelpers:
    ...
    def get_response_headers(
        headers: httpx.Headers, llm_call_id: Optional[str] = None
    ) -> dict:
        ...
        if llm_call_id:
            return_headers["x-llm-call-id"] = llm_call_id
...
async def pass_through_request(  # noqa: PLR0915
    ...
    try:
        ...
        logging_obj.update_environment_variables(
            ...
            user="unknown",
            optional_params={},
            llm_params=kwargs["llm_params"],
            call_type="pass_through_endpoint",
        )
        logging_obj.model_call_details["llm_call_id"] = llm_call_id
        ...
        verbose_proxy_logger.exception(
            "llm.proxy.proxy_server.pass_through_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
def _init_kwargs_for_pass_through_endpoint(
    ...
    _llm_metadata: Optional[dict] = _parsed_body.pop("llm_metadata", None)
    _metadata = dict(
    ...
    if _llm_metadata:
        _metadata.update(_llm_metadata)
...
def create_pass_through_route(
    ...
    try:
        ...
        llm.adapters = [{"id": adapter_id, "adapter": adapter}]
...
async def initialize_pass_through_endpoints(pass_through_endpoints: list):
    ...
    from llm.proxy._types import CommonProxyErrors, HanzoRoutes
    from llm.proxy.proxy_server import app, premium_user
    ...
    for endpoint in pass_through_endpoints:
        ...
        if _auth is not None and str(_auth).lower() == "true":
            ...
            HanzoRoutes.openai_routes.value.append(_path)
...
async def get_pass_through_endpoints(
    ...
    """
    from llm.proxy.proxy_server import get_config_general_settings
