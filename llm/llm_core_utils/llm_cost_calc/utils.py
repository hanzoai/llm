...
from llm.types.utils import ModelInfo, Usage
...
def _generic_cost_per_character(
    ...
    try:
        ...
        verbose_logger.exception(
            "llm.litellm_core_utils.llm_cost_calc.utils.py::cost_per_character(): Exception occured - {}\nDefaulting to None".format(
                str(e)
            )
        )
...
def generic_cost_per_token(
    ...
    Input:
        - model: str, the model name without provider prefix
        - usage: LLM Usage block, containing anthropic caching information
