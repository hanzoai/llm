...
## Getting the LLM Call ID

LLM generates a unique `call_id` for each request. This `call_id` can be
...
setting a request header `LLM-Disable-Message-Redaction: true`.


```shell
curl --location 'http://0.0.0.0:4000/chat/completions' \
    --header 'Content-Type: application/json' \
    --header 'LLM-Disable-Message-Redaction: true' \
    --data '{
    "model": "gpt-3.5-turbo",
    "messages": [
        {
...
```
LLM.Info: "no-log request, skipping logging"
```
...
### LLM Tags - `cache_hit`, `cache_key`

Use this if you want to control which LLM-specific fields are logged as tags by the LLM proxy. By default LLM Proxy logs no LLM-specific fields

| LLM specific field               | Description                                           | Example Value                                       |
|------------------------|-------------------------------------------------------|------------------------------------------------|
...
| `user_api_key_alias`   | An alias for the LLM Virtual Key.| `prod-app1`        |
...
### View POST sent from LLM to provider

Use this when you want to view the RAW curl request sent from LLM to the LLM API 
...
You will see `raw_request` in your Langfuse Metadata. This is the RAW CURL command sent from LLM to your LLM API provider
...
1. Make Request to LLM Proxy with `traceparent` header
...
| Property | Details |
|----------|---------|
| Description | Log LLM `SpendLogs Table` to Google Cloud Storage PubSub Topic |

When to use `gcs_pubsub`?

- If your LLM Database has crossed 1M+ spend logs and you want to send `SpendLogs` to a PubSub Topic that can be consumed by GCS BigQuery


#### Usage

1. Add `gcs_pubsub` to LLM Config.yaml
```yaml
model_list:
- llm_params:
    api_base: https://exampleopenaiendpoint-production.up.railway.app/
...
1. Add `azure_storage` to LLM Config.yaml
```yaml
model_list:
  - model_name: fake-openai-endpoint
    llm_params:
...
LLM Supports logging to the following Datdog Integrations:
...
LLM supports customizing the following Datadog environment variables
...
class MyCustomHandler(CustomLogger):
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        metadata = llm_params.get("metadata", {})   # headers passed to LLM proxy, can be found here
...
Here's a reference FastAPI Server that is compatible with LLM Proxy:
...
Start the LLM Proxy and make a test request to verify the logs reached your callback API 
