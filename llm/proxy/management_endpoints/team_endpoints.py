...
async def get_all_team_memberships(
    ...
    returned_tm: List[LLM_TeamMembership] = []
    for tm in team_memberships:
        returned_tm.append(LLM_TeamMembership(**tm.model_dump()))
...
async def new_team(  # noqa: PLR0915
    ...
    try:
        ...
        ## ADD TO TEAM TABLE
        complete_team_data = LLM_TeamTable(
            **data.json(),
            model_id=_model_id,
        )
        ...
        team_row: LLM_TeamTable = await prisma_client.db.llm_teamtable.create(
...
async def update_team(
    ...
    _team_metadata_fields = LLM_ManagementEndpoint_MetadataFields_Premium
    ...
    team_row: Optional[LLM_TeamTable] = (
    ...
    await _cache_team_object(
        ...
        team_table=LLM_TeamTableCachedObj(**team_row.model_dump()),
...
def team_member_add_duplication_check(
    data: TeamMemberAddRequest,
    existing_team_row: LLM_TeamTable,
):
...
async def team_member_add(
    ...
    complete_team_data = LLM_TeamTable(**existing_team_row.model_dump())
...
async def team_member_delete(
    ...
    existing_team_row = LLM_TeamTable(**_existing_team_row.model_dump())
...
async def delete_team(
    ...
    for team_id in data.team_ids:
        ...
        team_row_pydantic = LLM_TeamTable(**team_row_base.model_dump())
    ...
    if llm.store_audit_logs is True:
        ...
        for team_id in data.team_ids:
            team_row: Optional[LLM_TeamTable] = await prisma_client.get_data(  # type: ignore
...
def validate_membership(
    user_api_key_dict: UserAPIKeyAuth, team_table: LLM_TeamTable
):
    if (
        user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN.value
...
def _unfurl_all_proxy_models(
    team_info: LLM_TeamTable, llm_router: Router
) -> LLM_TeamTable:
    if (
    ...
    try:
        if prisma_client is None:
            raise HTTPException(
                ...
                detail={
                    "error": "Database not connected. Connect a database to your proxy - https://docs.hanzo.ai/docs/simple_proxy#managing-auth---virtual-keys"
                },
            )
        if team_id is None:
            raise HTTPException(
        ...
        validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=LLM_TeamTable(**team_info.model_dump()),
        )
        ...
        if isinstance(team_info, dict):
            _team_info = LLM_TeamTable(**team_info)
        elif isinstance(team_info, BaseModel):
            _team_info = LLM_TeamTable(**team_info.model_dump())
        else:
            _team_info = LLM_TeamTable()
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=List[LLM_TeamTable],
):
    from llm.proxy.proxy_server import prisma_client
    ...
    if available_teams is None:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "No available teams for user to join. See how to set available teams here: https://docs.hanzo.ai/docs/proxy/self_serve#all-settings-for-self-serve--sso-flow"
            },
        )
    ...
    user_info_correct_type = LLM_UserTable(**user_info.model_dump())
    ...
    available_teams_correct_type = [
        LLM_TeamTable(**team.model_dump()) for team in available_teams_db
    ]
...
async def list_team(
    ...
    for team in filtered_response:
        _team_memberships: List[LLM_TeamMembership] = []
...
async def get_paginated_teams(
    ...
    Returns:
        Tuple[List[LLM_TeamTable], int] - (list of teams, total count)
    """
    try:
...
async def ui_view_teams(
    ...
    Returns:
        List[LLM_SpendLogs]: Paginated list of matching user records
    """
    from llm.proxy.proxy_server import prisma_client
...
async def team_model_add(
    ...
    team_obj = LLM_TeamTable(**team_row.model_dump())
