...
export const handleEditModelSubmit = async (formValues: Record<string, any>, accessToken: string | null, setEditModalVisible: (visible: boolean) => void, setSelectedModel: (model: any) => void) => {
  ...
  let newHanzoParams: Record<string, any> = {};
  ...
  for (const [key, value] of Object.entries(formValues)) {
    if (key !== "model_id") {
      ...
      newHanzoParams[key] = value === "" ? null : value;
    } else {
  ...
  let payload: {
    ...
    llm_params: Object.keys(newHanzoParams).length > 0 ? newHanzoParams : undefined,
...
const EditModelModal: React.FC<EditModelModalProps> = ({
  ...
  return (
    <Modal
      title={"Edit '" + model_name + "' Hanzo Params"}
      visible={visible}
      width={800}
      footer={null}
      onOk={handleOk}
