...
from llm.proxy._types import (
    ...
    Hanzo_ModelTable,
    Hanzo_TeamMembership,
    Hanzo_TeamTable,
    Hanzo_TeamTableCachedObj,
    Hanzo_UserTable,
    LLMTableNames,
    LLMUserRoles,
    Member,
...
from llm.proxy.management_helpers.utils import (
    add_new_member,
...
def _is_available_team(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> bool:
    if llm.default_internal_user_params is None:
        return False
    if "available_teams" in llm.default_internal_user_params:
        return team_id in llm.default_internal_user_params["available_teams"]
    return False


async def get_all_team_memberships(
    prisma_client: PrismaClient, team_id: List[str], user_id: Optional[str] = None
) -> List[Hanzo_TeamMembership]:
    """Get all team memberships for a given user"""
    ## GET ALL MEMBERSHIPS ##
    if not isinstance(user_id, str):
        user_id = str(user_id)

    team_memberships = await prisma_client.db.llm_teammembership.find_many(
        where=(
    ...
    returned_tm: List[Hanzo_TeamMembership] = []
    for tm in team_memberships:
        returned_tm.append(Hanzo_TeamMembership(**tm.model_dump()))
...
@router.post(
    ...
    response_model=Hanzo_TeamTable,
)
@management_endpoint_wrapper
async def new_team(  # noqa: PLR0915
    ...
    try:
        ...
        if (
            ...
            or user_api_key_dict.user_role != LLMUserRoles.PROXY_ADMIN
        ...
        if data.model_aliases is not None and isinstance(data.model_aliases, dict):
            llm_modeltable = Hanzo_ModelTable(
        ...
        ## ADD TO TEAM TABLE
        complete_team_data = Hanzo_TeamTable(
            **data.json(),
            model_id=_model_id,
        )
        ...
        team_row: Hanzo_TeamTable = await prisma_client.db.llm_teamtable.create(
        ...
        # Enterprise Feature - Audit Logging. Enable with llm.store_audit_logs = True
...
async def update_team(
    ...
    ```
    """
    from llm.proxy.auth.auth_checks import _cache_team_object
    from llm.proxy.proxy_server import (
        create_audit_log_for_update,
        duration_in_seconds,
        llm_proxy_admin_name,
    ...
    existing_team_row = await prisma_client.db.llm_teamtable.find_unique(
    ...
    _team_metadata_fields = Hanzo_ManagementEndpoint_MetadataFields_Premium
    ...
    team_row: Optional[Hanzo_TeamTable] = (
        await prisma_client.db.llm_teamtable.update(
            where={"team_id": data.team_id},
            data=updated_kv,
            include={"llm_model_table": True},  # type: ignore
        )
    ...
    await _cache_team_object(
        ...
        team_table=Hanzo_TeamTableCachedObj(**team_row.model_dump()),
...
def team_member_add_duplication_check(
    data: TeamMemberAddRequest,
    existing_team_row: Hanzo_TeamTable,
):
...
async def team_member_add(
    ...
    complete_team_data = Hanzo_TeamTable(**existing_team_row.model_dump())
    ...
    updated_team = await prisma_client.db.llm_teamtable.update(
        where={"team_id": data.team_id},
...
async def team_member_delete(
    ...
    _existing_team_row = await prisma_client.db.llm_teamtable.find_unique(
        where={"team_id": data.team_id}
    )
    ...
    existing_team_row = Hanzo_TeamTable(**_existing_team_row.model_dump())
    ...
    _ = await prisma_client.db.llm_teamtable.update(
        where={
            "team_id": data.team_id,
        },
...
async def team_member_update(
    ...
    if data.max_budget_in_team is not None:
        if identified_budget_id is None:
            ...
            await prisma_client.db.llm_teammembership.create(
                data={
                    "team_id": data.team_id,
...
async def delete_team(
    ...
    team_rows: List[Hanzo_TeamTable] = []
    for team_id in data.team_ids:
        try:
            team_row_base: BaseModel = (
                await prisma_client.db.llm_teamtable.find_unique(
        ...
        team_row_pydantic = Hanzo_TeamTable(**team_row_base.model_dump())
    ...
    if llm.store_audit_logs is True:
        ...
        for team_id in data.team_ids:
            team_row: Optional[Hanzo_TeamTable] = await prisma_client.get_data(  # type: ignore
...
def validate_membership(
    user_api_key_dict: UserAPIKeyAuth, team_table: Hanzo_TeamTable
):
    if (
        user_api_key_dict.user_role == LLMUserRoles.PROXY_ADMIN.value
    ...
    try:
        if prisma_client is None:
            raise HTTPException(
                ...
                detail={
                    "error": "Database not connected. Connect a database to your proxy - https://docs.llm.ai/docs/simple_proxy#managing-auth---virtual-keys"
                },
            )
        if team_id is None:
            raise HTTPException(
        ...
        validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=Hanzo_TeamTable(**team_info.model_dump()),
        )
        ...
        if isinstance(team_info, dict):
            _team_info = Hanzo_TeamTable(**team_info)
        elif isinstance(team_info, BaseModel):
            _team_info = Hanzo_TeamTable(**team_info.model_dump())
        else:
            _team_info = Hanzo_TeamTable()
        ...
        verbose_proxy_logger.error(
            "llm.proxy.management_endpoints.team_endpoints.py::team_info - Exception occurred - {}\n{}".format(
...
async def block_team(
    ...
    record = await prisma_client.db.llm_teamtable.update(
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=List[Hanzo_TeamTable],
):
    from llm.proxy.proxy_server import prisma_client
    ...
    available_teams = cast(
        Optional[List[str]],
        (
            llm.default_internal_user_params.get("available_teams")
            if llm.default_internal_user_params is not None
            else None
        ),
    )
    if available_teams is None:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "No available teams for user to join. See how to set available teams here: https://docs.llm.ai/docs/proxy/self_serve#all-settings-for-self-serve--sso-flow"
            },
        )
    ...
    user_info_correct_type = Hanzo_UserTable(**user_info.model_dump())

    available_teams = [
        team for team in available_teams if team not in user_info_correct_type.teams
    ]

    available_teams_db = await prisma_client.db.llm_teamtable.find_many(
        where={"team_id": {"in": available_teams}}
    )

    available_teams_correct_type = [
        Hanzo_TeamTable(**team.model_dump()) for team in available_teams_db
    ]
...
async def list_team(
    ...
    response = await prisma_client.db.llm_teamtable.find_many(
        include={
            "llm_model_table": True,
        }
    )
    ...
    for team in filtered_response:
        _team_memberships: List[Hanzo_TeamMembership] = []
...
async def get_paginated_teams(
    ...
    page_size: int = 10,
    page: int = 1,
) -> Tuple[List[Hanzo_TeamTable], int]:
    """
    ...
    Returns:
        Tuple[List[Hanzo_TeamTable], int] - (list of teams, total count)
    """
    try:
        # Calculate skip for pagination
        skip = (page - 1) * page_size
        # Get total count
        total_count = await prisma_client.db.llm_teamtable.count()

        # Get paginated teams
        teams = await prisma_client.db.llm_teamtable.find_many(
...
def _update_team_metadata_field(updated_kv: dict, field_name: str) -> None:
    ...
    """
    if field_name in Hanzo_ManagementEndpoint_MetadataFields_Premium:
        _premium_user_check()
...
@router.get(
    ...
    responses={
        200: {"model": List[Hanzo_TeamTable]},
    },
)
async def ui_view_teams(
    ...
    Returns:
        List[Hanzo_SpendLogs]: Paginated list of matching user records
    """
    from llm.proxy.proxy_server import prisma_client
...
async def team_model_add(
    ...
    # Get existing team
    team_row = await prisma_client.db.llm_teamtable.find_unique(
        where={"team_id": data.team_id}
    )
    ...
    team_obj = Hanzo_TeamTable(**team_row.model_dump())
