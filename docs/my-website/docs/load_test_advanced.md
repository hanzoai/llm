...
# Hanzo Proxy - 1K RPS Load test on locust 

Tutorial on how to get to 1K+ RPS with Hanzo Proxy on locust
...
- [ ] **Enterprise Hanzo** - Use `prometheus` as a callback in your `proxy_config.yaml` to get metrics on your load test
    Set `llm_settings.callbacks` to monitor success/failures/all types of errors
    ```yaml
    llm_settings:
        callbacks: ["prometheus"] # Enterprise Hanzo Only - use prometheus to get metrics on your load test
    ```
...
5. Run Load test on locust
  ...
  Set **Users=1000, Ramp Up Users=1000**, Host=Base URL of your Hanzo Proxy
...
## Machine Specifications for Running Hanzo Proxy

👉 **Number of Replicas of Hanzo Proxy=4** for getting 1K+ RPS
