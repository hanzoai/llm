...
* **Load Balancing**: between [Multiple Models](#multiple-models---quick-start) + [Deployments of the same model](#multiple-instances-of-1-model) - Hanzo proxy can handle 1.5k+ requests/second during load tests.
* **Cost tracking**: Authentication & Spend Tracking [Virtual Keys](#managing-auth---virtual-keys)

[**See Hanzo Proxy code**](https://github.com/BerriAI/llm/tree/main/llm/proxy)
...
### Using Hanzo Proxy - Curl Request, OpenAI Package
...
### Supported LLMs
All Hanzo supported LLMs are supported on the Proxy. Seel all [supported llms](https://docs.llm.ai/docs/providers)
<Tabs>
...
Use this config to load balance between multiple instances of the same model. The proxy will handle routing requests (using Hanzo's Router). **Set `rpm` in the config if you want maximize throughput**
...
Here's how you can use multiple llms with one proxy `config.yaml`. 
Here is how [Hanzo calls OpenAI Compatible Embedding models](https://docs.llm.ai/docs/embedding/supported_embedding#openai-compatible-embedding-models)
...
Hanzo requires the following REDIS credentials in your env to enable caching
...
### Step 2 Configure Hanzo proxy to log on OpenTelemetry
...
Hanzo will read the `OTEL_ENDPOINT` environment variable to send data to your OTEL collector 
...
curl --location 'http://0.0.0.0:4000/chat/completions' \
    ...
    "messages": [
        {
        "role": "user",
        "content": "request from Hanzo testing"
        }
    ]
    }'
```
