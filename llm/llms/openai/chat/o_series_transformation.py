...
Translations handled by Hanzo:
- modalities: image => drop param (if user opts in to dropping param)  
- role: system ==> translate to role 'user' 
- streaming => faked by Hanzo 
...
from llm.types.llms.openai import AllMessageValues, ChatCompletionUserMessage
...
class OpenAIOSeriesConfig(OpenAIGPTConfig):
                ...
                if temperature_value == 1:
                    ...
                    if llm.drop_params is True or drop_params is True:
                        ...
                        raise llm.utils.UnsupportedParamsError(
                            message="O-series models don't support temperature={}. Only temperature=1 is supported. To drop unsupported openai params from the call, set `llm.drop_params = True`".format(
                                temperature_value
                            ),
                            status_code=400,
                        )
    ...
    def is_model_o_series_model(self, model: str) -> bool:
        if model in llm.open_ai_chat_completion_models and (
