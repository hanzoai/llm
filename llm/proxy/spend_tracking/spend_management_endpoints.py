...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "LLM_SpendLogs" s
        LEFT JOIN
            "LLM_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def ui_view_spend_logs(  # noqa: PLR0915
    ...
    Returns:
        {
            "data": List[LLM_SpendLogs],  # Paginated spend logs
...
-H "Authorization: Bearer sk-1234"
    ...
    ```
    curl -X GET "http://0.0.0.0:8000/spend/logs?user_id=z@hanzo.ai" \
-H "Authorization: Bearer sk-1234"
    ```
    """
    from llm.proxy.proxy_server import prisma_client
...
async def global_spend_key_internal_user(
    ...
    sql_query = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "LLM_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query = """
    SELECT DISTINCT end_user FROM "LLM_SpendLogs"
    """
...
async def provider_budgets() -> ProviderBudgetResponse:
    ...
    try:
        ...
        if provider_budget_config is None:
            raise ValueError(
                "No provider budget config found. Please set a provider budget config in the router settings. https://docs.hanzo.ai/docs/proxy/provider_budget_routing"
            )
