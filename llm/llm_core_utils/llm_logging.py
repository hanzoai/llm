...
try:
    ...
    verbose_logger.debug(
        f"[Non-Blocking] Unable to import GenericAPILogger - Hanzo Enterprise Feature - {str(e)}"
    )
...
class Logging(HanzoLoggingBaseClass):
        ...
        try:
            ...
            if self.logger_fn and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "Hanzo.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(
                            str(e)
                        )
                    )
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.debug(
                        f"Hanzo.Logging: is sentry capture exception initialized {capture_exception}"
                    )
    ...
    def _print_llm_call_debugging_log(
        ...
        Prints the RAW curl command sent from Hanzo
        """
        if _is_debugging_on():
            if json_logs:
                ...
                self.model_call_details["llm_params"]["api_base"] = masked_api_base
    ...
    def _get_masked_headers(
        ...
        Masks the headers of the request sent from Hanzo
        """
        sensitive_keywords = [
            "authorization",
            "token",
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "Hanzo.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
        ...
        try:
            ...
            if (
                ...
                if (
                    ...
                    or isinstance(result, HanzoBatch)
            ...
            raise Exception(f"[Non-Blocking] Hanzo.Success_Call Error: {str(e)}")

    def success_handler(  # noqa: PLR0915
        self, result=None, start_time=None, end_time=None, cache_hit=None, **kwargs
    ):
        verbose_logger.debug(
            f"Logging Details Hanzo-Success Call: Cache_hit={cache_hit}"
        )
        ...
        try:
            ...
            if complete_streaming_response is not None:
                verbose_logger.debug(
                    "Logging Details Hanzo-Success Call streaming complete"
                )
            ...
            for callback in callbacks:
                try:
                    ...
                    print_verbose(
                        f"Hanzo.LoggingError: [Non-Blocking] Exception occurred while success logging with integrations {traceback.format_exc()}"
                    )
                    print_verbose(
                        f"Hanzo.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "Hanzo.LoggingError: [Non-Blocking] Exception occurred while success logging {}".format(
                    str(e)
                ),
            )
    ...
    async def async_success_handler(  # noqa: PLR0915
        ...
        """
        print_verbose(
            "Logging Details Hanzo-Async Success Call, cache_hit={}".format(cache_hit)
        )
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.error(
                    f"Hanzo.LoggingError: [Non-Blocking] Exception occurred while success logging {traceback.format_exc()}"
                )
                pass
    ...
    def failure_handler(  # noqa: PLR0915
        ...
        verbose_logger.debug(
            f"Logging Details Hanzo-Failure Call: {llm.failure_callback}"
        )
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    print_verbose(
                        f"Hanzo.LoggingError: [Non-Blocking] Exception occurred while failure logging with integrations {str(e)}"
                    )
                    print_verbose(
                        f"Hanzo.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "Hanzo.LoggingError: [Non-Blocking] Exception occurred while failure logging {}".format(
                    str(e)
                )
            )
    ...
    async def async_failure_handler(
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(
                    "Hanzo.LoggingError: [Non-Blocking] Exception occurred while failure \
                        logging {}\nCallback={}".format(
                        str(e), callback
                    )
                )
    ...
    def _remove_internal_llm_callbacks(self, callbacks: List) -> List:
        """
        Creates a filtered list of callbacks, excluding internal Hanzo callbacks.
