...
from llm.types.router import LLMParamsTypedDict
...
def _check_non_standard_fallback_format(fallbacks: Optional[List[Any]]) -> bool:
    ...
    - List[Dict[<LLMParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for key in LLMParamsTypedDict.__annotations__.keys():
