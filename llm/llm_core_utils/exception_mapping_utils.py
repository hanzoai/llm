...
def exception_type(  # type: ignore  # noqa: PLR0915
    ...
    if any(
        ...
        for exc_type in llm.LLM_EXCEPTION_TYPES
    ):
    ...
    if llm.suppress_debug_info is False:
        ...
        print(  # noqa
            "Hanzo.Info: If you need to debug this error, use `llm._turn_on_debug()'."  # noqa
        )  # noqa
        print()  # noqa
    ...
    try:
        ...
        if model:
            ...
            try:
                ...
                messages = llm.get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if (
                    messages
                    and len(messages) > 0
                    and llm.redact_messages_in_exceptions is False
                ):
                    extra_information += f"\nMessages: `{messages}`"
                ...
                # on llm proxy add key name + team to exceptions
            ...
            if (
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 401:
                        ...
                        exception_mapping_worked = True
                        raise llm.InternalServerError(
                            message=f"AnthropicException - {error_str}. Handle with `llm.InternalServerError`.",
                            llm_provider="anthropic",
                            model=model,
                        )
                    elif original_exception.status_code == 503:
                        exception_mapping_worked = True
                        raise llm.ServiceUnavailableError(
                            message=f"AnthropicException - {error_str}. Handle with `llm.ServiceUnavailableError`.",
                            llm_provider="anthropic",
                            model=model,
                        )
            ...
            elif custom_llm_provider in llm._openai_like_providers:
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Enable 'llm.modify_params=True' (for PROXY do: `llm_settings::modify_params: True`) to insert a dummy assistant message and fix this error.",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Pass in default user message via `completion(..,user_continue_message=)` or enable `llm.modify_params=True`.\nFor Proxy: do via `llm_settings::modify_params: True` or user_continue_message under `llm_params`",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                ...
                if "Unable to locate credentials" in error_str:
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"llm.BadRequestError: SagemakerException - {error_str}",
                        model=model,
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"llm.BadRequestError: VertexAIException - {error_str}",
                        model=model,
                ...
                if "400 Request payload size exceeds" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise llm.InternalServerError(
                        message=f"llm.InternalServerError: VertexAIException - {error_str}",
                        model=model,
                        llm_provider="vertex_ai",
                        response=httpx.Response(
                            status_code=500,
                    ...
                    exception_mapping_worked = True
                    raise RateLimitError(
                        message=f"llm.RateLimitError: VertexAIException - {error_str}",
                        model=model,
                        llm_provider="vertex_ai",
                        llm_debug_info=extra_information,
                        response=httpx.Response(
                ...
                if "Internal server error" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise ContentPolicyViolationError(
                        message=f"llm.ContentPolicyViolationError: AzureException - {message}",
                        llm_provider="azure",
                        model=model,
                        llm_debug_info=extra_information,
                        response=getattr(original_exception, "response", None),
    ...
    try:
        ...
        if logger_fn and callable(logger_fn):
            try:
                ...
                verbose_logger.debug(
                    f"Hanzo.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
                )
    except Exception:
        verbose_logger.debug(
            f"Hanzo.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
...
def _add_key_name_and_team_to_alert(request_info: str, metadata: dict) -> str:
    """
    Internal helper function for llm proxy
