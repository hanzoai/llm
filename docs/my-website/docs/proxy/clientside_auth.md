...
Allow your end-users to pass their model list, api base, OpenAI API key (any Hanzo supported provider) to make requests 
...
Allows your users to pass in their OpenAI API key/API base (any Hanzo supported provider) to make requests 
...
Any value passed in the Proxy's request body, will be checked by Hanzo against the mapped openai / llm auth params. 
