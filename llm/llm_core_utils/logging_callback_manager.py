...
from llm.integrations.additional_logging_utils import AdditionalLoggingUtils
from llm.integrations.custom_logger import CustomLogger


class LoggingCallbackManager:
    """
    A centralized class that allows easy add / remove callbacks for llm.
    ...
    def add_llm_input_callback(self, callback: Union[CustomLogger, str]):
        """
        Add a input callback to llm.input_callback
        """
    ...
    def add_llm_service_callback(
        ...
        """
        Add a service callback to llm.service_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=llm.service_callback
        )

    def add_llm_callback(self, callback: Union[CustomLogger, str, Callable]):
        """
        Add a callback to llm.callbacks
    ...
    def add_llm_success_callback(
        ...
        """
        Add a success callback to `llm.success_callback`
        """
        self._safe_add_callback_to_list(
    ...
    def add_llm_failure_callback(
        ...
        """
        Add a failure callback to `llm.failure_callback`
        """
        self._safe_add_callback_to_list(
    ...
    def add_llm_async_success_callback(
        ...
        """
        Add a success callback to llm._async_success_callback
        """
        self._safe_add_callback_to_list(
    ...
    def add_llm_async_failure_callback(
        ...
        """
        Add a failure callback to llm._async_failure_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=llm._async_failure_callback
        )
    ...
    def _get_all_callbacks(self) -> List[Union[CustomLogger, Callable, str]]:
        """
        Get all callbacks from llm.callbacks, llm.success_callback, llm.failure_callback, llm._async_success_callback, llm._async_failure_callback
        """
        return (
            llm.callbacks
            + llm.success_callback
